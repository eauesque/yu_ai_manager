...
def _doctor_report_dir() -> Path:
    ...
    side effect instead of skipping the route entirely. Rust honours the same
    variable in ``routes/diagnostics.rs::doctor_report_dir``; the two must stay
    in step or the harness silently compares one server that wrote and one that
    did not.
    """
    override = os.environ.get("YU_DOCTOR_REPORT_DIR")
    if override:
...
async def _run_doctor_job(job_id: str) -> None:
    try:
        ...
        md_path, json_path = await asyncio.to_thread(write_report_files, _doctor_report_dir(), report_md, report_json)
        _register_job(
            job_id,
            {
                "status": "done",
