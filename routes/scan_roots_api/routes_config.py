...
# `require_admin_scope` / `require_local` already return a finished
# `(Response, status)` pair -- `api_error` builds it with `jsonify`. Returning
# `api_result(*auth_err)` therefore passed the *Response object* as the payload
# and produced `{"ok": false, "error": "<Response [200 OK]>"}`: the status was
# still 403, so every test that checked only the number stayed green while the
# sentence an operator reads was destroyed -- and replaced with text claiming
# 200. Return the pair unchanged.
