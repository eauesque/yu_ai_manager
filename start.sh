...
# Marks this launch as launcher-driven, so web_ui.py knows an exit code 75
# (dist rebuilt, now fast-mode eligible) will be caught and retried here
# rather than just killing a direct invocation (IDE, Tauri sidecar, bare
# `uv run python web_ui.py`) that has no retry loop of its own.
#
# `export` mutates the shell's own environment, not just this script's: a
# normal `./start.sh` run gets a fresh process from the shebang, so it dies
# with the script, but `source start.sh` / `. ./start.sh` runs this in the
# caller's interactive shell and the export would otherwise persist there
# after the script exits, silently making a later direct `python web_ui.py`
# in that same shell behave as if a launcher were present. The EXIT trap
# below unsets it on every exit path. When sourced, the caller's shell may
# already have its own EXIT trap; overwriting it outright would silently
# drop that behavior, so any prior trap is captured and chained after ours.
export YU_LAUNCHER=1
_yu_prev_exit_trap=$(trap -p EXIT | sed -n "s/^trap -- '\(.*\)' EXIT\$/\1/p")
trap "unset YU_LAUNCHER${_yu_prev_exit_trap:+; $_yu_prev_exit_trap}" EXIT
...
# --- Doctor (diagnose only, start nothing) ---
# Placed right after uv is ready and before post_restart_apply / onnx / node:
# a user runs this when startup misbehaves, so it must not depend on the very
# steps that might be failing.
if [ "$YU_DOCTOR" = "1" ]; then
    "$UV_BIN" run --no-project --quiet python scripts/doctor.py "$@"
    exit $?
fi
...
while :; do
    ...
    if [ "$YU_FAST_RC" -eq 0 ] && [ "${#YU_FAST_ARGS[@]}" -gt 0 ]; then
        "${YU_FAST_ARGS[@]}"
        rc=$?
        if [ "$rc" -eq 127 ] || [ "$rc" -eq 126 ] || [ "$rc" -eq 78 ]; then
            ...
            # leave a hole in the fallback net for it. 78 (EX_CONFIG): the
            # database is at an older schema than the binary, and only Python
            # owns the migration chain that brings it up -- decide() cannot
            # foresee this, because resolving the database path the way
            # yu-server does (config.toml, .env, profile, YU_DB) lives in the
            # Rust binary, so the refusal has to come back as an exit code
            # rather than a pre-launch verdict. Either way, fall back
            ...
            #
            # One condition, not one per code, deliberately: the two
            # assignments below suppress the post-rebuild re-resolve at the
            # bottom of this loop, and a separate branch that forgot them
            # would send this launch back through yu-server for the same
            # refusal.
            if [ "$rc" -eq 78 ]; then
                echo "[fast-mode] The database needs migrating; starting the Python version, which owns the migration chain." >&2
            else
