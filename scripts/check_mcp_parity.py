...
def collect_routes(repo: Path = REPO) -> list[RouteEntry]:
    ...
    for base in scan_dirs:
        ...
        for py_file in base.rglob("*.py"):
            # Skip __pycache__ and venv. Test the path RELATIVE to the repo:
            # `py_file.parts` is absolute, so a checkout living under any
            # directory named "tmp" (a worktree in .../jobs/<id>/tmp/, say)
            # matched on its own location and every route file was skipped.
            # collect_routes then returned [], which makes the error check
            # shout that every MCP path is broken -- and makes its sibling
            # coverage check report "all routes covered" over an empty set.
            # The loud half is survivable; the quiet half is a green that
            # measured nothing.
            if any(
                part in (".venv", "__pycache__", "venv", "tmp")
                for part in py_file.relative_to(repo).parts
            ):
                continue
            try:
...
def collect_mcp_paths(repo: Path = REPO) -> list[MCPPathEntry]:
    ...
    for py_file in sorted(mcp_dir.rglob("*.py")):
        # Relative parts, for the same reason as collect_routes above: an
        # absolute `.parts` lets the checkout's own location silence the scan.
        if any(
...
def run_parity_check(
    ...
    # A scan that found nothing is not a clean repo, it is a broken scanner.
    # Measured 2026-09-08: a worktree under `.../jobs/<id>/tmp/merge-npu`
    # matched the "tmp" exclusion on its OWN absolute path, so every route
    # file was skipped. `check_broken_mcp_paths` then shouted that all 571
    # MCP paths were broken, while `check_uncovered_routes` reported
    # "all /api/ routes covered" over an empty set -- a green measuring
    # nothing. The exclusions are now relative; this refuses the empty case
    # outright so no future variant can pass by finding zero.
    if routes and not mcp_paths:
        raise RuntimeError(
            "check_mcp_parity: found routes but zero MCP paths — the mcp_server "
            "scan is broken, not the repo. Refusing to report a verdict."
        )
    if mcp_paths and not routes:
        raise RuntimeError(
            "check_mcp_parity: found MCP paths but zero routes — the route scan "
            "is broken, not the repo. Refusing to report a verdict."
        )
