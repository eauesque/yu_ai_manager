...
# native-dialog: OS folder-picker (rfd) for /api/tools/select-folder. Without it
# the endpoint answers "headless_unsupported" and every folder button in the UI
# is dead. Skipped on Linux: rfd there needs wayland-client dev libs, which a
# headless server lacks and could not put to use anyway.
# Features are package-qualified because two packages are built at once and
# they do not share a feature set: native-dialog exists only on yu-server, so a
# bare name would be applied to whichever package happens to declare it.
# yu-infer-shim must receive the same GPU EP feature as yu-server -- it is the
# sidecar that actually runs ONNX inference (yu-server reaches it over HTTP via
# infer_client), so a sidecar built without it silently falls back to CPU.
FEATURES=""
if [[ "$(uname -s)" != "Linux" ]]; then
    FEATURES="yu-server/native-dialog"
fi
if [[ -n "$GPU_FEATURE" ]]; then
    FEATURES="${FEATURES:+$FEATURES,}yu-server/$GPU_FEATURE,yu-infer-shim/$GPU_FEATURE"
    info "GPU EP: $GPU_FEATURE"
fi

env "${BUILD_ENV[@]}" cargo build --release \
    -p yu-server -p yu-infer-shim --features "$FEATURES"
...
# yu-server spawns the sidecar as current_exe().parent().join("yu-infer"), so it
# has to sit next to the server binary -- which is where cargo just put it.
SIDECAR="$TARGET_DIR/release/yu-infer"
[[ -f "$SIDECAR" ]] || err "$MSG_BINARY_MISSING: $SIDECAR"

if [[ $DO_STRIP -eq 1 ]] && command -v strip &>/dev/null; then
    strip "$BINARY" "$SIDECAR"
    ok "$MSG_STRIPPED"
fi
...
ok "$MSG_DONE $SIDECAR ($SIDECAR_SIZE)"
