...
That rule is about the *decision*, not about the fallback net. One refusal
cannot be decided here at all: whether the database is at an older schema than
the binary. Resolving the database path the way yu-server does -- config.toml,
.env, profile, YU_DB, all of it in Rust -- cannot be mirrored in this module
without a second resolver that would drift, so yu-server makes that call after
it starts and reports it as exit code 78 (EXIT_DB_SCHEMA_BEHIND). The launchers
add that code to the one fallback condition they already have; the net widens,
the judgment stays here.
"""
...
# `.onnx_extra` token (scripts/detect_onnx_extra.py) -> cargo EP feature on the
# Rust side. The names differ because one picks a PyPI wheel and the other a
# cargo feature; "cpu" maps to nothing, which is what an absent key means here.
_EXTRA_TO_GPU_FEATURE = {
    "gpu": "cuda",
    "rocm": "rocm",
    "directml": "directml",
    "silicon": "coreml",
...
# Features that compile ort in load-dynamic mode because no prebuilt ONNX
# Runtime ships with them (see yu-hailo-infer-core/Cargo.toml). A binary built
# with one of these needs ORT_DYLIB_PATH at startup or it cannot run at all.
_FEATURES_NEEDING_DYLIB = frozenset({"rocm", "directml", "openvino"})


def _gpu_feature() -> str | None:
    """The cargo execution-provider feature to build with, or None for CPU.

    Detection is `scripts/detect_onnx_extra.py` -- the same probe that chooses
    which onnxruntime wheel the Python server installs -- so the two servers
    reach for the same accelerator instead of drifting apart on two copies of
    the same "is there a GPU here" question.
    """
    from scripts.detect_onnx_extra import detect
    from scripts.internal.fast_mode_env import ort_dylib_path_set

    feature = _EXTRA_TO_GPU_FEATURE.get(detect())
    if feature is None:
        return None
    if feature in _FEATURES_NEEDING_DYLIB and not ort_dylib_path_set():
        print(
            f"[fast-mode] {feature} には ORT_DYLIB_PATH が要る（未設定のため CPU で建てる）",
            file=sys.stderr,
        )
        return None
    return feature
...
def _run_cargo(repo: Path, jobs: str) -> tuple[bool, str]:
    ...
    feature = _gpu_feature()
    if feature is not None:
        print(f"[fast-mode] EP: {feature}", file=sys.stderr)
        # Both packages have to be named. With more than one `-p`, cargo reads
        # a bare `--features cuda` as ambiguous; and the two must agree anyway,
        # or ort gets built twice under different feature sets.
        argv += ["--features", f"yu-server/{feature},yu-infer-shim/{feature}"]
...
def _will_bind_non_loopback(repo: Path, passthrough: list[str]) -> bool:
    ...
    config.toml *is* covered, contrary to what this note said until
    2026-09-10: resolve_config_path (core/web/startup_args.py) puts
    config.toml at the head of its candidate list, matching main.rs. The
    stale note outlived the fix and was cited as evidence in a later design
    review, which is the cost of leaving one in place.
...
def main(argv: list[str]) -> int:
    ...
    #
    # The launch arguments are read before `--resolve` is even validated, so
    # `--python` works the same whether the launcher or a person invokes this.
    #
    # Called with no argument: existing tests substitute a zero-argument stub
    # for this function, and more importantly the environment switches are what
    # this call is for. The flag is checked separately, right below.
    passthrough_for_optout = argv[argv.index("--") + 1 :] if "--" in argv else []
    if skip_fast_mode_requested() or python_only_requested(passthrough_for_optout):
        if python_only_requested(passthrough_for_optout):
            print(
                "[fast-mode] Python で起動します: --python が指定されました",
                file=sys.stderr,
            )
        return 1
