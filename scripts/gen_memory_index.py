...
def primary_worktree_root(repo_root: Path) -> Path:
    """Return the primary checkout when repo_root is a linked git worktree.

    Claude Code stores memory under a slug of the primary checkout path, so a
    linked worktree must resolve to the same directory. Without this the memory
    dir is never found inside a worktree and the sync gate passes vacuously.
    """
...
def resolve_memory_dir(repo_root: Path) -> Path:
    ...
    if env:
        return Path(env)
    root = primary_worktree_root(Path(repo_root))
    return Path.home() / ".claude" / "projects" / slugify_repo_path(root) / "memory"
