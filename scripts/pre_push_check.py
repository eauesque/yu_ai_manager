...
def check_githooks_installed() -> tuple[bool, str]:
    """Fail when core.hooksPath does not point at the tracked .githooks.

    Was advisory until v4.700.0, and the warning went unread: `.githooks/pre-commit`
    runs `sync_version.py --check` and would have blocked every version bump that
    skipped the sync, but core.hooksPath pointed at an empty `.git/hooks`, so it
    never ran. Measured over the 40 most recent VERSION-touching commits,
    src-tauri/Cargo.toml diverged from VERSION in 20 of them (package.json in 8):
    VERSION and package.json are the two a person edits by hand, and the cargo
    files are the ones only the script remembers. Each divergence broke the next
    nightly Static Checks run.

    Also verifies the hooks are executable: git skips a non-executable hook in
    silence, which looks exactly like the dormant path this check now catches.

    Skipped under CI, where nothing is committed and hooks are never consulted --
    static-checks.yml runs this very script, so failing there would only make the
    gate permanently red without protecting a single commit.
    """
    import subprocess

    if os.environ.get("CI") or os.environ.get("GITHUB_ACTIONS"):
        return True, "(CI — hooks are never consulted here; skipped)"

    def _git(*args: str) -> str:
        return subprocess.run(
            ["git", *args], capture_output=True, text=True, cwd=_REPO_ROOT, check=True
        ).stdout.strip()
    ...
    try:
        ...
        # Ask git where the hook actually lands rather than re-deriving it: git applies
        # core.hooksPath itself, and its answer differs between a worktree and the main
        # checkout for the SAME relative setting.
        hook_path = _git("rev-parse", "--git-path", "hooks/pre-push")
        common_git_dir = Path(_git("rev-parse", "--path-format=absolute", "--git-common-dir"))
    except Exception as e:
        # Cannot verify => cannot claim it is enforced.
        return False, f"could not read core.hooksPath: {e}"

    expected = _REPO_ROOT / ".githooks"
    # The main checkout's `.githooks` is the same tracked directory of the same
    # repository, so a worktree pointed there is enforced too -- see below.
    main_githooks = common_git_dir.parent / ".githooks"
    resolved_dir = (_REPO_ROOT / hook_path).resolve().parent

    accepted = {expected.resolve()}
    if main_githooks.is_dir():
        accepted.add(main_githooks.resolve())

    if resolved_dir not in accepted:
        shown = value if value else "(unset)"
        return False, (
            f"core.hooksPath is {shown}, which resolves to {resolved_dir} — not the "
            "tracked .githooks. The pre-commit/pre-push gate is DORMANT on this clone, "
            "so a VERSION bump without sync_version.py commits clean and breaks the "
            "next nightly CI run.\n"
            "  Fix (once per clone): git config core.hooksPath .githooks\n"
            "  An absolute path to this repository's .githooks is accepted too: a "
            "RELATIVE value resolves against the CWD, so in the main checkout running "
            "git from a subdirectory skips every hook in silence."
        )
    if main_githooks.is_dir() and resolved_dir == main_githooks.resolve() != expected.resolve():
        # Accepted, but say so: this worktree runs the MAIN checkout's hook files, which
        # differ if its branch has changed them.
        note = f" (via the main checkout's .githooks at {main_githooks})"
    else:
        note = ""

    # Scan the directory git will actually use, not the one this worktree owns.
    not_executable = [
        hook.name
        for hook in sorted(resolved_dir.glob("*"))
        if hook.is_file() and not os.access(hook, os.X_OK)
    ]
    if not_executable:
        return False, (
            f"core.hooksPath resolves to {resolved_dir} but "
            f"{', '.join(not_executable)} is not executable — "
            "git skips a non-executable hook without a word.\n"
            f"  Fix: chmod +x {resolved_dir}/{{{','.join(not_executable)}}}"
        )

    return True, f"core.hooksPath -> {resolved_dir} (tracked gate active){note}"
...
try:
    from scripts.internal.pre_push_check_checkers import (
        ...
        check_rust_hook_definitions_pin,
        check_rust_schema_version_pin,
        check_rust_standalone_build,
        check_scan_roots_recovery_filter,
        check_schema_drift,
        check_schema_version_single_owner,
        check_systemd_restart_convergence,
        check_systemd_unit_ordering,
        check_ts,
    )
except ImportError:
...
def check_todo_archived() -> tuple[bool, str]:
    """Is a finished entry still sitting in TODO.md?

    A settled conclusion left in the open list reads as work. See
    `scripts/check_todo_archived.py` for the two occasions that motivated this.
    """
    scripts_dir = str(_REPO_ROOT / "scripts")
    if scripts_dir not in sys.path:
        sys.path.insert(0, scripts_dir)
    from check_todo_archived import _self_check, validate  # 遅延 import（read-only）

    broken = _self_check()
    if broken:
        return False, "検出器自身ガ壊レ居ル:\n" + "\n".join(f"  {b}" for b in broken)

    ok, problems = validate()
    if ok:
        return True, "TODO.md ニ `done(...)` ノ残留無シ"
    return False, (
        "TODO.md ニ `done(...)` ノ項ガ残リ居ル:\n"
        + "\n".join(problems)
        + "\n完了シタル項ハ其ノ都度 docs/development/archives/todo-completed-YYYY-MM.md ヘ移スベシ。"
    )


def check_changelog_canonical() -> tuple[bool, str]:
    """Did a released version reach the canonical changelog?

    `CHANGELOG.md` is canonical and `CHANGELOG.ja.md` is release-facing. Three
    versions (4.711.1/.2/.3) shipped with entries written only into the release
    file; two earlier runs of the same drift went unnoticed for months. See
    `scripts/check_changelog_canonical.py`.
    """
    scripts_dir = str(_REPO_ROOT / "scripts")
    if scripts_dir not in sys.path:
        sys.path.insert(0, scripts_dir)
    from check_changelog_canonical import (  # 遅延 import（read-only）
        _CANONICAL,
        _RELEASE,
        _VERSION,
        _self_check,
    ...
    if not problems:
        return True, "CHANGELOG.md（正本）ハ出荷版ヲ悉ク収ム"
    return False, (
        "正本タル CHANGELOG.md ニ漏レ有リ:\n"
        + "\n".join(f"  {problem}" for problem in problems)
    )


def check_agent_routing() -> tuple[bool, str]:
    """Does every agent the routing tables name actually resolve to something?

    Twice the tables have named agents that could not be dispatched -- once a
    missing `.claude/agents/*.md`, once a disabled plugin -- and the mandatory
    plan gate was among them. See `scripts/check_agent_routing.py`.
    """
    scripts_dir = str(_REPO_ROOT / "scripts")
    if scripts_dir not in sys.path:
        sys.path.insert(0, scripts_dir)
    from check_agent_routing import _self_check, validate  # 遅延 import（read-only）

    broken = _self_check()
    if broken:
        return False, "検出器自身ガ壊レ居ル:\n" + "\n".join(f"  {b}" for b in broken)

    ok, problems = validate(_REPO_ROOT)
    if ok:
        return True, "agent routing OK（全 agent 参照ガ実体ヘ解決ス）"
    return False, "\n".join(problems) + "\nrouting 表ヲ実態ヘ合ハスカ、agent／plugin ヲ有効化スベシ"
...
def check_parity_no_python() -> tuple[bool, str]:
    """The other half of the relaxation ratchet: is each `no_python` claim true?

    The relaxation ratchet counts the flag; this asks Python's own url_map
    whether the route it claims does not exist is one Python serves. 168 of the
    289 claims are currently unproven, so this is frozen as a ratchet rather
    than a blocking gate -- growth fails, repayment asks for a refresh.
    """
    try:
        from scripts.internal.parity_no_python_gate import check
    except ImportError:
        from internal.parity_no_python_gate import check  # type: ignore[no-redef]
    return check()
...
def main() -> int:
    ...
    parser.add_argument("--skip", default="", help="Comma-separated: githooks-path,ruff,ts,pyright,async-route-io,db-write-rule,i18n,pytest,scan-roots-recovery-filter,schema-drift,genesis-sql-sync,schema-version-pin,hook-definitions-pin,systemd-restart-convergence,systemd-unit-ordering,schema-version-owner,genesis-acceptance,uv-pin,reqs-sync,doc-index-v2,feature-index,dev-overview,settings-schema,agents-md,bat-bom,ps1-bom,feature-parity,scope-gate,mcp-parity-errors,mcp-parity-warnings,bungotai-normalize,bungo-all-normalize,bungo-katakana,bungotai-sync,scope-fence-golden,charter-sync,charter-validators,charter-stranded,skills-structure,skills-index-sync,memory-index-sync,rationale-ref,pair-completeness,dup-rationale-ref,structural-validators,sse-kind-parity,openapi-new-routes,semgrep,ai-coreutils-fresh,rust-tests,rust-fmt,rust-compat,rust-standalone-build,lan-cowork-clippy,lan-cowork-schema-contract,lan-cowork-mirror-drift,parity-allowlist-sync,extension-contract-sync,no-rust-proxy-calls,python-forwarder-ratchet,parity-relaxations,parity-no-python,ruff-ratchet,eslint-ratchet,public-leaks,native-only-endpoints,crate-include-paths,crate-escapes-root,config-read-format,portfolio-forwarder-count")
    ...
    if "launcher-exit-codes" not in skip:
        checks.append(("launcher fallback exit codes (start.sh ↔ start.ps1 ↔ Rust)", check_launcher_fallback_exit_codes))
    if "schema-version-owner" not in skip:
        checks.append(("schema_version single owner (Python ↔ Rust)", check_schema_version_single_owner))
    if "systemd-unit-ordering" not in skip:
        checks.append(("systemd unit ordering (migrate oneshot -> server)", check_systemd_unit_ordering))
    if "systemd-restart-convergence" not in skip:
        checks.append(("systemd restart convergence (deploy/*.service)", check_systemd_restart_convergence))
    if "hook-definitions-pin" not in skip:
        checks.append(("hook definitions pin (Python ↔ Rust)", check_rust_hook_definitions_pin))
    ...
    if "agent-routing" not in skip:
        checks.append(("agent-routing ↔ agents/*.md・plugin 実在性", check_agent_routing))
    if "todo-archived" not in skip:
        checks.append(("TODO.md ニ完了項ノ残留無シ", check_todo_archived))
    if "changelog-canonical" not in skip:
        checks.append(("CHANGELOG.md（正本）↔ 出荷版", check_changelog_canonical))
    ...
    if "parity-no-python" not in skip:
        checks.append(
            ("parity no_python claims (may only shrink)", check_parity_no_python)
        )
    ...
    # Same hazard, same shape, second crate pair: private development with a
    # public mirror, pinned by rev. Registering only one of the two would leave
    # the other free to pin a rev nobody else can fetch.
    if "hailo-infer-pin" not in skip:
        checks.append(
            ("hailo-infer pinned rev is public", check_hailo_infer_pinned_rev_public)
        )
    if "rust-compat" not in skip and _rust_changed:
        checks.append(("rust↔python native route parity", check_rust_compat))
    # The bridge's send side lives in nai_params.py / nai_bridge.rs; a change to
    # either can split the outgoing payload while both still answer 200.
    _nai_send_changed = any(
        f.endswith("nai_params.py")
        or f.endswith("routes/nai_bridge.rs")
        or f == "scripts/verify_nai_send_parity.py"
        for f in _changed_files
    )
    if "nai-send-parity" not in skip and _nai_send_changed:
        checks.append(("NAI bridge send-side parity", check_nai_send_parity))
    ...
    if "ocr-feature-gate" not in skip and _rust_changed:
        checks.append(("OCR feature gate", check_ocr_feature_gate))
