...
Compares:
  - initialize (legacy, 2026-07-28, and no-version negotiation)
  - unsupported protocol version rejection
  - server/discover (except serverInfo.version)
...
def _post(client: httpx.Client, url: str, body: dict, timeout: float = 30.0,
          headers: dict[str, str] | None = None) -> tuple[int, Any]:
    try:
        resp = client.post(url, json=body, timeout=timeout, headers=headers)
        try:
            data = resp.json()
        except Exception:
            data = resp.text
...
def cmp_initialize(py_client: httpx.Client, rust_client: httpx.Client,
                   py_url: str, rust_url: str) -> list[CmpResult]:
    cases = (
        ("2024-11-05", "2024-11-05", False),
        ("2026-07-28", "2026-07-28", True),
        (None, "2025-11-25", False),
    )
    results = []
    for requested, expected, expects_result_type in cases:
        params = {"capabilities": {}, "clientInfo": {"name": "parity-check", "version": "1.0"}}
        if requested:
            params["protocolVersion"] = requested
        headers = {"MCP-Protocol-Version": requested} if requested else None
        body = _req("initialize", params)
        py_status, py_data = _post(py_client, f"{py_url}/mcp", body, headers=headers)
        rust_status, rust_data = _post(rust_client, f"{rust_url}/mcp", body, headers=headers)
        label = requested or "no protocol version"
        r = CmpResult("initialize", f"POST /mcp — initialize ({label})",
        ...
        if isinstance(py_res, dict) and isinstance(rust_res, dict):
            for side, result in (("python", py_res), ("rust", rust_res)):
                if result.get("protocolVersion") != expected:
                    r.notes.append(
                        f"{side} protocolVersion: expected {expected!r}, got {result.get('protocolVersion')!r}"
                    )
                if ("resultType" in result) != expects_result_type:
                    r.notes.append(
                        f"{side} resultType presence: expected {expects_result_type}, got {'resultType' in result}"
                    )
            if py_res.get("protocolVersion") != rust_res.get("protocolVersion"):
                r.notes.append(
                    f"protocolVersion diff: python={py_res.get('protocolVersion')!r} rust={rust_res.get('protocolVersion')!r}"
                )
            ...
            r.notes.append("missing JSON-RPC result")
        results.append(r)
    return results


def cmp_unsupported_version(py_client: httpx.Client, rust_client: httpx.Client,
                            py_url: str, rust_url: str) -> CmpResult:
    body = _req("tools/list")
    headers = {"MCP-Protocol-Version": "1900-01-01"}
    py_status, py_data = _post(py_client, f"{py_url}/mcp", body, headers=headers)
    rust_status, rust_data = _post(rust_client, f"{rust_url}/mcp", body, headers=headers)
    r = CmpResult("unsupported-version", "POST /mcp — unsupported protocol version",
                  py_status, rust_status, py_data, rust_data)
    if not _check_reachability(r):
        return r
    if py_status != 400 or rust_status != 400:
        r.notes.append(f"expected status 400: python={py_status} rust={rust_status}")
    py_err = _extract_error(py_data)
    rust_err = _extract_error(rust_data)
    for side, error in (("python", py_err), ("rust", rust_err)):
        if not isinstance(error, dict) or error.get("code") != -32022:
            actual = error.get("code") if isinstance(error, dict) else error
            r.notes.append(f"{side} error.code: expected -32022, got {actual!r}")
    return r


def cmp_server_discover(py_client: httpx.Client, rust_client: httpx.Client,
                        py_url: str, rust_url: str) -> CmpResult:
    body = _req("server/discover")
    headers = {"MCP-Protocol-Version": "2026-07-28"}
    py_status, py_data = _post(py_client, f"{py_url}/mcp", body, headers=headers)
    rust_status, rust_data = _post(rust_client, f"{rust_url}/mcp", body, headers=headers)
    r = CmpResult("server/discover", "POST /mcp — server/discover",
                  py_status, rust_status, py_data, rust_data)
    if not _check_reachability(r):
        return r
    if py_status != rust_status:
    ...
    py_res = _extract_result(py_data)
    rust_res = _extract_result(rust_data)
    if not isinstance(py_res, dict) or not isinstance(rust_res, dict):
        r.notes.append("missing JSON-RPC result")
        return r
    for key in ("resultType", "supportedVersions", "capabilities", "instructions"):
        if py_res.get(key) != rust_res.get(key):
            r.notes.append(f"{key} diff: python={py_res.get(key)!r} rust={rust_res.get(key)!r}")
    py_info = py_res.get("serverInfo", {})
    rust_info = rust_res.get("serverInfo", {})
    if py_info.get("name") != rust_info.get("name"):
        r.notes.append(f"serverInfo.name diff: python={py_info.get('name')!r} rust={rust_info.get('name')!r}")
...
def main() -> int:
    ...
    ap.add_argument("--rust-token", help="Bearer token for Rust MCP requests")
    ...
    rust_headers = {"Authorization": f"Bearer {args.rust_token}"} if args.rust_token else None
    with httpx.Client() as py_client, httpx.Client(headers=rust_headers) as rust_client:
        print(f"\n比較対象:\n  Python: {py_url}\n  Rust:   {rust_url}\n")

        # 1. initialize
        print("─" * 60)
        init_results = cmp_initialize(py_client, rust_client, py_url, rust_url)
        if init_results[0].rust_status == 401 and not args.rust_token:
            print("Rust MCP requires a Bearer token; pass --rust-token")
            if rust_proc:
                rust_proc.terminate()
            return 2
        ...
        print("─" * 60)
        r = cmp_unsupported_version(py_client, rust_client, py_url, rust_url)
        results.append(r)
        print(f"{r.badge()} {r.label}")
        print(f"   python={r.python_status}  rust={r.rust_status}")
        for n in r.notes:
        ...
        # 3. server/discover
        print("─" * 60)
        r = cmp_server_discover(py_client, rust_client, py_url, rust_url)
        results.append(r)
        print(f"{r.badge()} {r.label}")
        print(f"   python={r.python_status}  rust={r.rust_status}")
        for n in r.notes:
