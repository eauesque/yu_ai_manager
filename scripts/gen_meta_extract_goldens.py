...
def _read_exif_chunks(path: Path) -> dict[str, str]:
    """Read EXIF UserComment from a WebP fixture and lift it to `Comment`.

    Mirrors `exif_chunks_from_tags` in crates/tagdb-core/src/import/fallback_chain.rs
    and the Rust conformance harness. Reading a WebP fixture as PNG yields no
    chunks at all, and a golden that records `unknown` looks like agreement.
    """
    from core.extractors.webp_bytes import extract_webp_metadata

    meta = extract_webp_metadata(path)
    raw = meta.get("raw_meta_json") or ""
    if not raw:
        return {}
    stripped = raw.removeprefix("YU_META:")
    chunks: dict[str, str] = {"Comment": stripped, "UserComment": stripped}
    try:
...
def _read_fixture_chunks(path: Path) -> dict[str, str]:
    """Dispatch on suffix the way the scanner does."""
    if path.suffix.lower() == ".png":
        return _read_png_chunks(path)
    return _read_exif_chunks(path)


def run_a1111(fixture: Path) -> object | None:
    chunks = _read_fixture_chunks(fixture)
