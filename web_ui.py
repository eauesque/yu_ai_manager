                ...
                if _shutil.which("node") and (_base / "build.mjs").exists():
                    ...
                    if _br.returncode == 0:
                        ...
                        try:
                            ...
                            _decision = refresh_after_dist_rebuild(_base)
                            if _decision.use_fast_mode:
                                # Rebuilt bundle now qualifies for yu-server.
                                # This process can't switch to it in place --
                                # os.execv only replaces the process image on
                                # Unix; on Windows it spawns a detached child
                                # and returns here, breaking the launcher's
                                # process ownership and Ctrl+C handling. Hand
                                # off through the same exit-75 retry path
                                # start.sh / start.bat already use, which
                                # rebuilds (a cheap no-op since dist is
                                # already fresh) and re-resolves fast mode.
                                #
                                # Only do this when a launcher is actually
                                # present to catch exit 75 (start.ps1/start.sh
                                # set YU_LAUNCHER=1). A direct invocation --
                                # IDE, Tauri sidecar, bare `uv run
                                # python web_ui.py` -- has no retry loop, so
                                # exiting 75 there would just kill the
                                # process instead of finishing the boot; stay
                                # on Python as this branch always has. The
                                # retry also needs pnpm/npm on PATH.
                                if os.environ.get("YU_LAUNCHER") == "1" and (
                                    _shutil.which("pnpm") or _shutil.which("npm")
                                ):
                                    print(
                                        "[fast-mode] Rebuilt bundle now "
                                        "qualifies -- restarting via the "
                                        "launcher to use yu-server.",
                                        file=sys.stderr,
                                    )
                                    sys.exit(75)
                                print(
                                    "[INFO] yu-server now qualifies, but no "
                                    "launcher is present to retry (or "
                                    "pnpm/npm is missing from PATH) -- "
                                    "continuing with Python this launch.",
                                    file=sys.stderr,
                                )
