...
#
# $env: writes hit the OS process environment block directly -- unlike a
# regular PowerShell variable, they are NOT scope-bound to this script.
# start.bat always launches this file via `powershell -File`, a fresh child
# process where the write dies with the child, but a user running
# `.\start.ps1` straight from their own interactive pwsh session executes it
# in THAT SAME process, so the write would otherwise persist in their shell
# after the script exits (or is Ctrl+C'd) and silently make a later direct
# `python web_ui.py` in that same window behave as if a launcher were
# present. The try/finally below scopes the marker back out on every exit
# path, `exit N` included -- pwsh 7 runs `finally` before an `exit` inside
...
$doctorMode = $LaunchArgs -contains '--doctor'
...
if (-not $usePortable) {
    ...
    # Right after uv is ready and before post_restart_apply / onnx / node: a
    # user runs this when startup misbehaves, so it must not depend on the very
    # steps that might be failing.
    if ($doctorMode) {
        & $uvBin 'run' '--no-project' '--quiet' 'python' 'scripts\doctor.py' @LaunchArgs
        exit $LASTEXITCODE
    }
...
while ($true) {
    if ($yuFastArgs.Count -gt 0) {
        ...
        if ($yuFastFailed -or $rc -eq 127 -or $rc -eq 126 -or $rc -eq 78) {
            ...
            #
            # 78 (EX_CONFIG) mirrors start.sh too: the database is at an older
            # schema than the binary, and only Python owns the migration chain
            # that brings it up. decide() cannot foresee this -- resolving the
            # database path the way yu-server does (config.toml, .env, profile,
            # YU_DB) lives in the Rust binary -- so the refusal comes back as
            # an exit code rather than a pre-launch verdict.
            #
            # One condition, not one per code, deliberately: the two
            # assignments below suppress the post-rebuild re-resolve further
            # down this loop, and a separate branch that forgot them would send
            # this launch back through yu-server for the same refusal.
            if ($rc -eq 78) {
                Write-Host '[fast-mode] The database needs migrating; starting the Python version, which owns the migration chain.'
            } else {
    ...
    Remove-Item Env:\YU_LAUNCHER -ErrorAction SilentlyContinue
}
