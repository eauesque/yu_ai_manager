...
def get_primary_model_dir_for_profile(profile: TaggerProfile) -> Path:
    """Return the primary cache directory for a profile, applying hf_subdir."""
...
def _legacy_model_dir_for_profile(profile: TaggerProfile) -> Path:
    """Return the pre-path-refactor cache directory for a profile."""
    base = Path.home() / ".cache" / "yu_ai_manager" / "wd_tagger"
    base = base / _safe_name(profile.model_id)
    if profile.hf_subdir:
        base = base / profile.hf_subdir
    return base


def _count_required_model_files(profile: TaggerProfile, model_dir: Path) -> int:
    return sum(
        1
        for file_spec in profile.files
        if file_spec.required and (model_dir / file_spec.name).exists()
    )


def get_model_dir_for_profile(profile: TaggerProfile) -> Path:
    """Resolve a profile directory across primary then legacy caches.

    This mirrors CLIP's resolve_existing_model_path() without splitting a
    profile across directories: whichever of primary/legacy holds more of
    the profile's required files wins, ties (including both empty) go to
    primary. Downloads always use the primary directory.
    """
    primary = get_primary_model_dir_for_profile(profile)
    legacy = _legacy_model_dir_for_profile(profile)
    if _count_required_model_files(profile, legacy) > _count_required_model_files(
        profile, primary
    ):
        return legacy
    return primary
...
def _download_one_file(
    ...
    *,
    ...
    base = dest_dir or get_primary_model_dir_for_profile(profile)
...
def download_model_for_profile(
    ...
    """
    base = get_primary_model_dir_for_profile(profile)
    base.mkdir(parents=True, exist_ok=True)
