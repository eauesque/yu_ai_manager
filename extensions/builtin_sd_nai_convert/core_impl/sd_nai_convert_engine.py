...
# Only the prefix is rewritten, so any surrounding syntax -- ``(artist:foo:1.2)``,
# ``1.2::artist:foo ::`` -- keeps its shape.
_ARTIST_PREFIX_RE = re.compile(r"\bartist:\s*", re.IGNORECASE)
...
_AT_ARTIST_RE = re.compile(r"(^|[\s,(\[{|:])@([^\s,)\]}|])")


def convert_artist_to_at(text: str) -> str:
    """Rewrite SD-style ``artist:NAME`` into the anima/NAI ``@NAME`` form."""
    return _ARTIST_PREFIX_RE.sub("@", text or "")


def convert_at_to_artist(text: str) -> str:
    """Inverse of :func:`convert_artist_to_at`."""
    return _AT_ARTIST_RE.sub(r"\1artist:\2", text or "")
