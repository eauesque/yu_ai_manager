...
MAX_CHARACTERS = 6
# NovelAI Diffusion V5 accepts up to twenty-two character prompts; V4 and V4.5
# stop at six. https://docs.novelai.net/en/image/multiplecharacters/
MAX_CHARACTERS_V5 = 22


def max_characters(model: str) -> int:
    """Return how many character prompts ``model`` accepts.
    ...
    """
    return MAX_CHARACTERS_V5 if model_generation(model) == GEN_V5 else MAX_CHARACTERS


def _has_center(center: Any) -> bool:
    """Whether a center object actually carries a usable position.

    ``{}`` is not a position. The Rust side used to treat any present center as
    one while this side treated ``{}`` as absent, so the same request produced
    two different images.
    """
    if not isinstance(center, dict):
        return False
    return isinstance(center.get("x"), (int, float)) and isinstance(center.get("y"), (int, float))
...
def _build_char_captions(
    ...
    """Build positive and negative char_captions, and derive ``use_coords``.

    Returns ``(pos_captions, neg_captions, use_coords)`` with matching
    length/indices. Characters whose positive prompt is empty are skipped, and
    the list is cut to what ``model``'s generation accepts.

    **``use_coords`` is derived from the same set that is emitted.** Scanning
    the unfiltered list, as this module used to, claimed coordinates for
    characters that were never sent -- and disagreed with Rust as a result.
    """
    if not characters:
        return [], [], False
...
def build_request_body(
    ...
    *,
    ...
    pos_chars, neg_chars, use_coords = _build_char_captions(characters, model)
