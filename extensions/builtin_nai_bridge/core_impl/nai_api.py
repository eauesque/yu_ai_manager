...
def create_nai_bridge_blueprint(import_name: str) -> Blueprint:
    ...
    async def api_models():
        ...
        # re-implementing the model->generation mapping in JS. Keeping the
        # judgement in one place is the point: a second hardcoded list is how
        # the two sides drift apart.
        from .nai_uc_presets import model_generation

        items = [
            {
                "id": m,
                "name": MODEL_DISPLAY_NAMES.get(m, m),
                "generation": model_generation(m),
            }
            for m in MODELS
        ]
