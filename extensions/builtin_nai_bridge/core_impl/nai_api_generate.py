...
def _sd_to_nai(text: str) -> str:
    """Rewrite SD prompt syntax pulled in from wildcard lists into NAI syntax.

    The sd_nai_convert extension is optional; the text is returned unchanged when
    it is not installed.
    """
    try:
        ...
        return text
    return engine.convert_sd_to_nai(text, strip_lora=True, strip_embedding=True)
...
def _build_character_params(
    ...
    if characters:
        ...
        def _expand(text: str) -> str:
            # Mirrors expand_request_prompts (Rust): NAI does not interpret
            # `__wc__` or `{a|b}` inside char_captions either, and a field the
            # expansion actually changed is additionally rewritten to NAI syntax.
            expanded = expand_text(text, seed=_wc_seed, extra_wildcards=extra_wc)
            return _sd_to_nai(expanded) if expanded != text else expanded

        for char in characters:
            for key in ("prompt", "negative"):
                if char.get(key):
                    char[key] = _expand(char[key])
...
async def handle_generate(data: dict, get_token_fn) -> tuple:
    ...
    # Wildcard lists are commonly authored in SD syntax, so a field the expansion
    # actually changed is additionally rewritten to NAI syntax. Fields the expansion
    # left alone pass through untouched -- hand-written prompts are never rewritten.
    # ``original_*`` is None when nothing expanded at all.
    if expansion.get("original_prompt") is not None and prompt != expansion["original_prompt"]:
        prompt = _sd_to_nai(prompt)
        expansion["prompt"] = prompt
    if expansion.get("original_negative") is not None and negative != expansion["original_negative"]:
        negative = _sd_to_nai(negative)
        expansion["negative"] = negative
