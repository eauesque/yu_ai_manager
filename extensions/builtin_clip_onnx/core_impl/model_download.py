...
# Shared with text_model_download.py. Must match Rust's CLIP_MODEL_FILES in
# crates/yu-server/src/routes/clip_model.rs exactly, or the two sides can
# resolve to different directories for the same on-disk state.
CLIP_MODEL_FILES = ("text_model.onnx", "tokenizer.json", "vision_model.onnx")


def _count_files_present(directory: Path) -> int:
    return sum(1 for name in CLIP_MODEL_FILES if (directory / name).is_file())


def resolve_clip_model_dir(repo: str = _DEFAULT_REPO) -> Path:
    """Resolve the CLIP model directory across primary then legacy cache.

    Mirrors Rust's `clip_model::model_dir()`: whichever of primary/legacy
    holds more of the three CLIP files wins, without splitting files across
    directories -- ties (including both empty) go to primary.
    """
    primary = get_model_dir(repo)
    legacy = _legacy_clip_onnx_cache() / _safe_name(repo)
    if _count_files_present(legacy) > _count_files_present(primary):
        return legacy
    return primary


def resolve_existing_model_path(repo: str = _DEFAULT_REPO) -> Path | None:
    """Return the vision model path in the resolved (primary-or-legacy) dir."""
    path = resolve_clip_model_dir(repo) / "vision_model.onnx"
