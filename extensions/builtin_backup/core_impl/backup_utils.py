...
def _write_meta(
    ...
    #
    # The key must be applied, and it must be the *caller's* key: the backup
    # is written through a connection keyed the same way the source was, so a
    # bare `sqlite3.connect` cannot read it and every sidecar came out without
    # `schema_version` — silently, because the exception was only logged.
    # `list` then reported `schema_version: null` for every backup ever taken
    # on an encrypted install. Defaulting to the application key fixed that
    # for desktop installs only; a deployment with its own YU_DB_KEY stayed
    # broken, and `_find_reusable_pre_migrate_backup` matches on exactly this
    # field, so it could never reuse a backup either.
    try:
        con = _sc.connect(str(backup_path))
        _apply_key(con, db_key)
        row = con.execute(
