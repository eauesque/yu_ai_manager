"""Core ML CLIP image preprocessing.

Converts images to (1, 3, 224, 224) float32 tensors with CLIP
normalization (ImageNet-style mean/std as used by openai/clip).

Independent from the ONNX preprocess module to avoid cross-extension
dependencies, though the logic is identical.
"""
...
def preprocess_image(path: str) -> np.ndarray:
    """Load and preprocess an image for Core ML CLIP inference.
...
def preprocess_images_batch(paths: list) -> tuple:
    """Preprocess a batch of images for Core ML CLIP inference.
