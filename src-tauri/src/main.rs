...
fn main() {
    ...
    // Why yu-server stood down, when it did. Carried into the Python-detection
    // failure below so a bundle without Python does not report only
    // "Python 未検出" and drop the more useful half of the story.
    let mut yu_server_detail: Option<String> = None;
    ...
    if let Some(yu_bin) = yu_server::find_yu_server_bin() {
        ...
        let mut server_child =
            match yu_server::start_yu_server(&yu_bin, &project_root, port, &log_path) {
                Ok(child) => Some(child),
                Err(e) => {
                    log!(
                        &log_path,
        ...
        // Observe the child as well as the port: a yu-server that refuses the
        // database exits in a fraction of a second, and a port-only wait used
        // to sit through the whole timeout before degrading.
        let readiness = match server_child {
            Some(ref mut child) => yu_server::wait_for_server_or_exit(port, server_timeout, child),
            None => yu_server::ReadinessOutcome::TimedOut,
        };
        let readiness_detail = yu_server::describe_readiness(&readiness, server_timeout.as_secs());
        let became_ready = readiness == yu_server::ReadinessOutcome::Ready;
        ...
        match yu_server::decide_fast_mode_outcome(spawn_succeeded, became_ready) {
            ...
            yu_server::FastModeOutcome::DegradeToPython => {
                log!(&log_path, "{} — Python 経路へ降格します", readiness_detail);
                // A permanent degradation the user cannot see is the thing the
                // comment on EXIT_DB_RUST_SCHEMA_AHEAD warns against. Shown,
                // not fatal: refusing to open leaves a user with nothing to act
                // on, while a dialog leaves them with the repair. The app keeps
                // starting either way.
                if let Some(notice) = yu_server::desktop_degradation_notice(&readiness) {
                    logging::show_message_box(
                        "YU AI Manager - データベースが新しすぎます",
                        &format!("{notice}\n{}", log_path.display()),
                        false,
                    );
                }
                if let Some(mut server_child) = server_child {
                    // Only a still-running child needs killing; one that exited
                    // on its own has already been reaped by `try_wait`.
                    let _ = server_child.kill();
                    let _ = server_child.wait();
                }
                // Carried into the Python-detection failure below: without it a
                // bundle that ships no Python reports "Python 未検出" and says
                // nothing about why yu-server stood down, which is the more
                // useful half of the story.
                yu_server_detail = Some(readiness_detail);
    ...
    let python = match find_python(&project_root) {
        Ok(p) => p,
        Err(msg) => {
            // Name both halves. "Python 未検出" alone sends the user looking
            // for a Python problem when the actual story may be that
            // yu-server refused the database and this bundle ships no Python
            // to migrate it.
            let full = match yu_server_detail {
                Some(ref detail) => format!("{detail}\n\n{msg}"),
                None => msg,
            };
            logging::show_error_and_exit("YU AI Manager - Python 未検出", &full, &log_path)
        }
    };
