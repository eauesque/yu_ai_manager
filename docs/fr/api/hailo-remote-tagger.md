# API Hailo Tagger Distant

API for sending images to a remote Hailo AI HAT inference serveur (e.g. Raspberry Pi 5) over the network, running Danbooru tag inference, and saving results to the database.
...
|-------|------|-------------|
| `enabled` | bool | Si Hailo Remote Tagger is enabled |
...
| `timeout` | int | Requête timeout in seconds |
...
### Réponse (not configured / unreachable)
...
| 502 | `request_failed` | Failed to connect to remote serveur |
...
| `source` | Tag source identifier (`hailo_remote` or `hailo_remote:<serveur_id>` when using the registry) |
...
> **Note**: To manage multiple tagger serveurs, use the [Tagger Server Registry API](tagger-serveurs.md). Legacy configuration can be auto-migrated via `/api/tagger-serveurs/migrate`. The Tagger Server Registry also supports Bearer token authentication.
