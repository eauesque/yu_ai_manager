# API de Análisis de IA

APIs para análisis de imágenes impulsado por IA, análisis de tendencias de prompts y gestión de servidores.

Todos los endpoints de POST/PUT/DELETE requieren el encabezado `X-Requested-With` (no requerido cuando se usa Clave API Bearer).

## Límite de Velocidad

Los endpoints de escritura bajo `/api/analysis/` utilizan el nivel **HEAVY** (~20 req/min, burst 5). Los endpoints de GET son ilimitados.
...
Obtiene la configuración actual de análisis de IA. Las claves API se devuelven enmascaradas.
...
|-------|------|-------------|
| `engine` | string | Tipo de motor actual (`claude_api`, `openai`, `ollama`, `openai_compat`, `hailo_vlm`) |
| `api_key` | string | Clave API de Claude (enmascarada) |
| `model` | string | Nombre del modelo API de Claude |
| `ollama_url` | string | URL del servidor Ollama |
| `ollama_model` | string | Nombre del modelo Ollama |
| `openai_api_key` | string | Clave API de OpenAI (enmascarada) |
| `openai_model` | string | Nombre del modelo OpenAI |
| `openai_compat_url` | string | URL del servidor compatible con OpenAI |
| `openai_compat_api_key` | string | Clave API compatible con OpenAI (enmascarada) |
| `openai_compat_model` | string | Nombre del modelo compatible con OpenAI |
| `hailo_vlm_model` | string | Nombre del modelo VLM de Hailo |
| `fallback_local_only` | boolean | Si se restringe a motores locales solo |
| `language` | string | Idioma para resultados de análisis (`ja`, `en`, etc.) |
| `is_local` | boolean | Si el motor actual es local (gratis) |
| `has_servers` | boolean | Si el registro de servidores está configurado |
| `servers` | array | Lista de servidores (solo cuando `has_servers` es true) |
| `active_server` | string | ID del servidor activo (solo cuando `has_servers` es true) |

### POST /api/analysis/config

Guarda la configuración de análisis de IA. Los valores enmascarados (cadenas que contienen `...`) no se sobrescriben. Las claves API se cifran automáticamente.
...
|-----------|------|----------|-------------|
| `engine` | string | No | Tipo de motor |
| `api_key` | string | No | Clave API de Claude |
| `model` | string | No | Modelo API de Claude |
| `ollama_url` | string | No | URL del servidor Ollama |
| `ollama_model` | string | No | Nombre del modelo Ollama |
| `openai_api_key` | string | No | Clave API de OpenAI |
| `openai_model` | string | No | Nombre del modelo OpenAI |
| `openai_compat_url` | string | No | URL del servidor compatible con OpenAI |
| `openai_compat_api_key` | string | No | Clave API compatible con OpenAI |
| `openai_compat_model` | string | No | Nombre del modelo compatible con OpenAI |
| `hailo_vlm_model` | string | No | Nombre del modelo VLM de Hailo |
| `fallback_local_only` | boolean | No | Restringir a motores locales solo |
| `language` | string | No | Idioma para resultados de análisis |
