...
This guide walks through creating a minimal custom UI and verifying it works.
...
Use `window.detailModalApi.showDetail(id)` as the primary public API. It is safer to write integrations without depending on old global names such as `window.showDetail(id)`.

Notes:

- Prefer feature APIs under `window.<feature>Api.*`
- `window.tr`, `window.apiFetch`, `window.apiUrl`, and `window.escapeHtml` remain available as foundational runtime globals
...
The server auto-detects `ui/custom/` on restart.

```bash
python web_ui.py --db ./tags.db --port 5000
```

You can also specify the UI explicitly in `config.json`:
...
## 5. Supported Page Routes

Flask routing maps to the following template names:

| Route | Template | Description |
|-------|----------|-------------|
| `/` | `index.html` | Main search page |
| `/stats` | `stats.html` | Statistics dashboard |
| `/tools` | `tools.html` | Tools page |
| `/settings` | `settings.html` | Settings page |
| `/extensions` | `extensions.html` | Extension management |
| `/story` | `story.html` | Your Story page |
| `/inspect` | `inspect.html` | Metadata inspector page |

The custom UI displays at the same URLs when you place templates with these names. An error occurs if a user navigates to a route whose template does not exist.

## 6. Statistics Page Example
...
## 7. Handle CSRF Protection

API calls that use POST / PUT / DELETE require the `X-Requested-With` header:

```javascript
// Example: set a rating
...
**Tip**: It is convenient to create a helper function that wraps all API calls:
...
## Generate a UI with AI

Here is a sample prompt you can give Claude or ChatGPT to generate a custom UI:
...
See [docs/api/](../api/README.md) for full API documentation.
