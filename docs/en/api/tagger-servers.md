...
API for managing multiple tag inference workers (Hailo Remote, ONNX Local, Ryzen AI, etc.) as a unified cluster, with distributed batch tagging via a shared-queue work-stealing parallel execution model.

## Overview

The Tagger Server Registry goes beyond a single Hailo Remote Tagger by managing multiple heterogeneous inference backends as a cluster. Each server has a configurable priority, and tasks are distributed according to the selected distribution mode (single / parallel / idle_first).
...
| `onnx_remote` | Remote ONNX inference server |
...
| Mode | Description |
|------|-------------|
| `single` | Use only the highest-priority enabled server |
| `parallel` | Run on all enabled servers in parallel (work-stealing) |
| `idle_first` | Prefer idle servers first |
...
|-------|------|----------|-------------|
| `endpoint_url` | string | Yes | Remote server URL |
...
Communication with remote servers (`hailo_remote` / `onnx_remote`) supports optional Bearer token authentication.
...
`deploy/hailo_tagger_server.py` provides a reference implementation with token verification. Set the token at startup via any of:
...
When no token is configured, the server operates in open access mode (LAN trust model) for backward compatibility. Invalid tokens receive 401/403 responses.

---

## GET /api/tagger-servers

List registered servers and the current distribution mode.
...
| `enabled` | bool | No | Enabled/disabled (default: `true`) |
...
Update an existing server's settings. Partial updates supported.
...
| `enabled` | bool | No | Enabled/disabled |
...
Reorder server priorities in bulk.
...
| Field | Type | Required | Description |
|-------|------|----------|-------------|
| `order` | string[] | Yes | Array of server IDs in priority order |
...
Run distributed batch tagging using the shared-queue work-stealing model. Executes as a background job with progress reported via SSE.
...
| `threshold` | float | No | Override tag confidence threshold (uses per-server config if omitted) |
...
|--------|------|-------------|
| 400 | `no_servers` | No enabled servers available |
...
The `source` field uses the format `{type}:{server_id}` (e.g. `hailo_remote:pi-hailo-a`, `onnx_local:local-onnx`).
...
Migrate legacy `hailo_tagger` configuration to the Tagger Server Registry format. Converts the existing `hailo_tagger` entry in `config.json` into a `tagger_servers` array entry.
...
|-----|------|-------------|
| `tagger_servers` | array | Array of server entries |
| `tagger_servers_mode` | string | Distribution mode (`single` / `parallel` / `idle_first`) |
...
Tags are stored in the `file_hailo_tags` table. The `source` column uses the `{type}:{server_id}` format to identify which server assigned the tag.
...
| `source` | Tag source identifier (`{type}:{server_id}` format, e.g. `hailo_remote:pi-hailo-a`) |
