...
APIs for managing configuration profiles. Profiles are named snapshots of application settings, stored as `profiles/<name>.json`.
...
| `is_active` | boolean | Whether this is the currently active profile |
...
| `label` | string | No | Display label. Defaults to `name` if omitted |
...
| `new_label` | string | No | New display label. Defaults to `new_name` if omitted |
