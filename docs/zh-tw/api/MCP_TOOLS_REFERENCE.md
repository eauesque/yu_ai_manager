...
YU AI Manager 的 MCP (Model Context Protocol) 伺服器提供的全部工具清單。
您可以從 Claude Desktop 或其他 MCP 客戶端呼叫這些工具，以自動化管理、分析和生成您的圖書館。
...
| 變數 | 說明 | 預設值 |
|------|------|-----------|
| `YU_BASE_URL` | YU AI Manager 伺服器的 URL | `http://localhost:5000` |
| `YU_API_KEY` | API 金鑰 (Bearer 認證) | (無) |
| `YU_DEBUG_MODE` | 設為 `1` 啟用除錯工具 | `0` |
...
`wait_for_scan` / `wait_for_batch` 工具支援 MCP Notifications：
- **支援 progressToken 的客戶端**：透過 `notifications/progress` 接收實時進度
- **不支援的客戶端**：進行阻塞等待，完成後返回最終結果
...
|------|-------------|------------|
| `search_images` | 使用各種篩選器搜尋圖像 | `query`: str = '', `sort`: str = 'date', `limit`: int = 20, `cursor`: str = '', `from_date`: str = '', `to_date`: str = '', `file_format`: str = 'all', `min_rating`: str = '', `max_rating`: str = '', `in_prompt`: str = '', `fav_only`: bool = False, `collection_id`: int = 0, `also_path`: bool = False |
| `search_images_grouped` | 使用目錄群組搜尋圖像 | `query`: str = '', `sort`: str = 'date', `limit`: int = 20, `from_date`: str = '', `to_date`: str = '' |
| `search_union` | 多個查詢的聯集搜尋 | `queries`: list |
| `get_image_detail` | 取得圖像的全部中繼資料 | `file_id`: int |
| `get_library_stats` | 圖書館統計 | — |
| `get_file_info` | 檔案路徑和中繼資料資訊 | `file_id`: int |
| `get_groups_index` | 目錄群組的索引 | — |
| `get_group_members` | 群組內的成員清單 | `group`: str |
| `get_container_members` | ZIP/RAR 容器內的成員清單 | `file_id`: int |
| `file_search` | 按路徑或名稱在資料庫中搜尋檔案 | `query`: str, `meta_filter`: str = "all", `limit`: int = 100 |

## Collections (7)

| Tool | Description | Parameters |
|------|-------------|------------|
| `list_collections` | 全部集合清單 | — |
| `create_collection` | 建立集合 | `name`: str |
| `rename_collection` | 重新命名集合 | `collection_id`: int, `name`: str |
| `delete_collection` | 刪除集合 | `collection_id`: int |
| `reorder_collections` | 變更集合順序 | `order`: list |
| `add_to_collection` | 將圖像新增到集合 | `collection_id`: int, `file_ids`: list, `expected_count`: int = 0 |
| `remove_from_collection` | 從集合中移除圖像 | `collection_id`: int, `file_ids`: list, `expected_count`: int = 0 |

## Ratings & Tags (5)

| Tool | Description | Parameters |
|------|-------------|------------|
| `rate_images` | 批次設定多個圖像的評分 | `items`: list, `expected_count`: int = 0 |
| `get_ratings` | 取得檔案的評分 | `file_ids`: str |
| `get_ratings_stats` | 評分統計 | — |
| `set_tags` | 新增/移除多個圖像的使用者標籤 | `items`: list, `expected_count`: int = 0 |
| `normalize_tags` | 正規化資料庫內的標籤 | — |

## Favorites (8)

| Tool | Description | Parameters |
|------|-------------|------------|
| `toggle_favorite` | 切換我的最愛 | `file_id`: int |
| `check_favorite` | 檢查我的最愛狀態 | `file_id`: int |
| `check_favorite_collections` | 檢查我的最愛檔案的集合成員資格 | `file_id`: int |
| `list_favorites` | 我的最愛清單 | `limit`: int = 50, `offset`: int = 0 |
| `fav_batch_add` | 批次將多個檔案新增到我的最愛 | `file_ids`: list, `collection_id`: int = 1 |
| `fav_batch_remove` | 批次從我的最愛移除多個檔案 | `file_ids`: list, `collection_id`: int = 0 |
| `fav_export_folder` | 將我的最愛匯出到伺服器上的資料夾 | `dest_path`: str, `collection_id`: int = 0 |
| `fav_images` | 我的最愛集合內的圖像清單 | `collection_id`: int = 0 |

## Annotations (4)

| Tool | Description | Parameters |
|------|-------------|------------|
| `set_annotations` | 儲存註解 (upsert) | `items`: list, `expected_count`: int = 0 |
| `get_annotations` | 取得圖像的註解 | `file_id`: int, `source`: str = '', `key`: str = '' |
| `search_annotations` | 跨註解搜尋 | `source`: str = '', `key`: str = '', `min_confidence`: str = '', `max_confidence`: str = '', `limit`: int = 100, `offset`: int = 0 |
| `delete_annotations` | 刪除註解 | `source`: str, `file_ids`: Optional = None, `key`: str = '' |

## Scanning (14)

| Tool | Description | Parameters |
|------|-------------|------------|
| `trigger_scan` | 開始掃描所有掃描根目錄 | — |
| `start_scan` | 開始掃描指定路徑或所有根目錄 | `path`: str = '' |
| `get_scan_status` | 取得掃描進度 | — |
| `cancel_scan` | 取消掃描 | — |
| `resume_scan` | 恢復中斷的掃描 | — |
| `dismiss_interrupted_scan` | 捨棄中斷狀態 | — |
| `get_scan_interrupted` | 取得中斷掃描資訊 | — |
| `get_scan_errors` | 掃描錯誤清單 | `error_type`: str = '', `resolved`: str = 'false', `limit`: int = 50 |
| `resolve_scan_error` | 將錯誤標記為已解決 | `error_id`: int |
| `clear_scan_errors` | 清除已解決的錯誤 | — |
| `get_scanned_roots` | 已掃描根目錄清單 | — |
| `scan_queue_list` | 掃描佇列的待處理項目清單 | -- |
| `scan_queue_remove` | 從掃描佇列中移除項目 | `queue_id`: str |
| `scan_queue_clear` | 清除所有掃描佇列 | -- |

## Scan Roots (9)

| Tool | Description | Parameters |
|------|-------------|------------|
| `list_scan_roots` | 掃描根目錄清單 | — |
| `add_scan_root` | 新增掃描根目錄 | `path`: str |
| `edit_scan_root` | 編輯掃描根目錄路徑 | `index`: int, `path`: str |
| `remove_scan_root` | 移除掃描根目錄 | `index`: int |
| `toggle_scan_root` | 切換掃描根目錄啟用/停用 | `index`: int |
| `reorder_scan_roots` | 變更掃描根目錄順序 | `order`: list |
| `scan_directory` | 掃描特定目錄 | `path`: str |
| `get_checkpoints` | 可用的模型檢查點 | — |
| `purge_scanned_roots` | 清除已掃描根目錄記錄 | — |

## Hash & Duplicates (7)

| Tool | Description | Parameters |
|------|-------------|------------|
| `find_duplicates` | 偵測重複檔案 | `method`: str = 'hash' |
| `find_similar` | 使用知覺雜湊搜尋相似圖像 | `file_id`: int, `threshold`: int = 5 |
| `compute_hashes` | 開始檔案雜湊計算工作 | `hash_type`: str = 'both' |
| `delete_duplicates` | 刪除重複檔案 | `groups`: list, `mode`: str = 'soft' |
| `start_hash_backfill` | 開始批次計算未計算的雜湊值 | — |
| `cancel_hash_backfill` | 取消雜湊計算 | — |
| `get_hash_backfill_status` | 取得雜湊計算進度 | — |

## Wait / Progress (2)

| Tool | Description | Parameters |
|------|-------------|------------|
| `wait_for_scan` | スキャン完了まで待機 (進度通知対応) | `timeout`: int = 600 |
| `wait_for_batch` | バッチジョブ完了まで待機 (進度通知対応) | `job_id`: str = 'ai_analysis', `timeout`: int = 600 |

## AI Analysis (25)

| Tool | Description | Parameters |
|------|-------------|------------|
| `analyze_image` | 單一圖像的 AI 分析 | `file_id`: int |
| `analyze_batch` | 多個圖像的批次 AI 分析 | `file_ids`: list, `expected_count`: int = 0, `server_ids`: list = None |
| `analyze_batch_cancel` | 取消執行中的 AI 分析批次工作 | -- |
| `get_analysis_result` | 取得分析結果 | `file_id`: int |
| `get_analysis_stats` | 分析統計 | — |
| `get_analysis_config` | 取得分析設定 | — |
| `save_analysis_config` | 儲存分析設定 | `config`: dict |
| `get_available_engines` | 取得可用引擎清單 | — |
| `get_ollama_models` | 取得 Ollama 模型清單 | — |
| `test_ollama_connection` | Ollama 連線測試 | — |
| `get_openai_compat_models` | 取得 OpenAI 相容 API 模型清單 | — |
| `test_openai_compat_connection` | OpenAI 相容 API 連線測試 | — |
| `list_ai_servers` | 已登錄的 AI 伺服器清單 | — |
| `add_ai_server` | 登錄 AI 伺服器 | `name`: str, `type`: str, `config`: dict, `priority`: int = 50, `enabled`: bool = True |
| `update_ai_server` | 更新 AI 伺服器設定 | `server_id`: str, `name`: str = '', `config`: dict = None, `priority`: int = -1, `enabled`: bool = True |
| `remove_ai_server` | 移除 AI 伺服器 | `server_id`: str |
| `set_active_ai_server` | 切換作用中伺服器 | `server_id`: str |
| `test_ai_server` | AI 伺服器連線測試 | `server_id`: str |
| `reorder_ai_servers` | 變更伺服器優先順序 | `order`: list |
| `migrate_ai_servers` | 從舊設定移轉 | — |
| `analyze_prompt_trends` | 分析提示詞趨勢 | `limit`: int = 100 |
| `get_trend_history` | 取得趨勢分析歷史 | `limit`: int = 20 |
| `delete_trend_history` | 刪除趨勢歷史 | `history_id`: int |
| `analyze_video` | 多關鍵影格動畫分析 (Vision LLM) | `file_id`: int, `engine`: str = "", `model`: str = "", `keyframe_count`: int = 4 |
| `transcribe_audio` | 使用 Whisper 轉錄音訊/視訊檔案 | `file_id`: int, `engine`: str = "", `model`: str = "", `language`: str = "" |
| `get_audio_analysis_status` | 檢查音訊分析的可用性 (ffmpeg, whisper) | -- |

## WD-Tagger (15)

| Tool | Description | Parameters |
|------|-------------|------------|
| `wd_tagger_tag_file` | 對單一檔案進行標籤推論 | `file_id`: int |
...
| `get_md_scan_roots` | 掃描根目錄清單 | — |
| `set_md_scan_roots` | スキャンルート設定 | `roots`: list |
| `remove_md_scan_root` | 移除掃描根目錄 | `index`: int |
...
| `toggle_profile_favorite` | 切換我的最愛 | `name`: str |
...
| Tool | Description | Parameters |
|------|-------------|------------|
| `get_video_analysis_config` | 動画取得分析設定 | — |
| `save_video_analysis_config` | 動画儲存分析設定 | `config`: dict |
...
| Tool | Description | Parameters |
|------|-------------|------------|
| `llm-endpoints-list` | 已設定的 LLM 端點清單 | — |
| `llm-endpoints-set` | 新增或更新 LLM 端點 | `category`: str, `base_url`: str, `model`: str, `api_key`: str = '', `timeout`: int = 60 |
| `llm-endpoints-remove` | 刪除 LLM 端點 | `category`: str |
| `llm-endpoints-test` | 測試 LLM 端點連線 | `category`: str |
| `llm-chat` | 將聊天委派給已設定的 LLM | `category`: str, `message`: str, `system_prompt`: str = '', `max_tokens`: int = 1024, `temperature`: float = 0.7 |

## Server Mode (2)

| Tool | Description | Parameters |
|------|-------------|------------|
| `server-mode-get` | 取得目前的伺服器模式 | — |
| `server-subsystems-status` | 取得子系統的狀態清單 | — |

## MCP 不支援的功能

以下因 MCP 的限制而未工具化：

- **二進位返回**: 縮圖 (`/api/thumbnail/`)、原始圖像 (`/api/original/`)、ZIP 下載、動畫檔案
- **OS 對話框**: 資料夾選取對話框 (`/api/tools/select-folder`)、檔案管理員啟動 (`/api/open-folder/`)
- **SSE 串流**: 日誌串流 (`/api/logs/stream`)
- **驗證頁面**: PIN 輸入畫面、LAN Share 來賓頁面
