...
| `YU_MCP_PROFILES` | 登録するツール群をプロファイルで絞り込む | (なし = 全ツール) |
| `YU_MCP_FACADE` | `0` で facade を無効化し、全ツールを直接登録する | `1` (有効) |

### ツール facade (`yu_*`)

関連するツール群は 1 つの `yu_<domain>` ツールに畳まれ、元のツール名は `action`
引数として渡す。ツール名はそのまま使える。

```json
{"action": "wd_tagger_tag_file", "args": {"file_id": 123}}
```

各 facade の説明には配下 action の一覧・引数名・1 行要約が載る。
`?` は任意引数、`+N?` はさらに N 個の任意引数があることを示す。
全引数と詳細は `yu_help(action="...")` で取得する。

```
- wd_tagger_tag_file(file_id): Run WD Tagger on a single image file.
- wd_tagger_untagged(limit?, offset?): List files that have no WD Tagger tags.
```
...
| `yu_tagger` | WD Tagger |
| `yu_github` | GitHub |
| `yu_comfyui` | ComfyUI ブリッジ |
| `yu_hailo` | Hailo (chat / genai / tagger) |
| `yu_sd` | SD / NovelAI ブリッジ・NAI 変換 |
| `yu_yolo` | YOLO 検出・ストリーム |
| `yu_misc` | ファイル・メディア・サーバー雑務 |
| `yu_analysis` | 画像解析・タガーサーバー |
| `yu_extension` | 拡張 |
| `yu_prompt` | プロンプトライブラリ |
| `yu_scan` | スキャンルート・重複 |
| `yu_settings` | 設定・プロファイル・UI |
| `yu_chatlog` | チャットログ |
| `yu_ocr` | OCR・音声認識 |
| `yu_lora` | LoRA データセット |
| `yu_share` | SNS 共有・Markdown ビューア |
| `yu_llm` | LLM エンドポイント・ルーター |
| `yu_search` | セマンティック検索・横断検索 |
| `yu_automation` | Webhook・スケジューラ・MCP クライアント |
| `yu_debug` | デバッグ・検証 |
| `yu_fleet` | fleet・mesh・gateway・LAN 共有 |
| `yu_maintenance` | バックアップ・アーカイブ・更新・レポート |

facade 化せず直接登録されるもの: `server_tools` (22、頻用のため)、
`wait_tools` (2、`Context` を取る)、`diagnostics_tools` (3、`dict` を返す)。

実測 (`scripts/internal/measure_facade_surface.py`):
**633 → 51 ツール、259,815 → 82,211 chars (-68.4%)**。
`YU_MCP_PROFILES=core` と併用すれば 33 ツール・約 9,809 tokens。
必須引数を全て公開しているため `yu_help` 往復が要る action は 0 件。

`YU_MCP_FACADE=0` で従来どおり全ツールを直接登録する。

### ツールプロファイル (`YU_MCP_PROFILES`)

全ツールを公開すると `tools/list` が 633 件・約 81k tokens に達し、クライアント側の
ツール選択精度が落ちる。用途に応じてカンマ区切りでプロファイルを指定すると、
該当するツール群のみを登録する。未設定または `all` で従来どおり全件登録。

| プロファイル | 内容 | ツール数 |
|---|---|---:|
| `core` | ライブラリ・検索・設定・スキャン・統計 (常に登録される基本ツールを含む) | 201 |
| `tagging` | WD / Hailo / YOLO タガーと画像解析 | 104 |
| `generate` | SD / ComfyUI / NAI ブリッジとプロンプトライブラリ | 104 |
| `llm` | チャット・チャットログ・OCR・音声認識 | 106 |
| `agent` | エージェント安全機構・拡張・スケジューラ・Webhook | 106 |
| `devops` | GitHub・fleet・gateway・mesh・デバッグ | 83 |
| `social` | SNS 共有・Markdown ビューア | 45 |
| `training` | LoRA データセット構築 | 39 |
| (未設定) / `all` | 全ツール | 633 |

`server_tools` (基本 22 ツール) はプロファイル指定に関わらず常に登録される。
そのため各プロファイルのツール数にはこの 22 件が含まれる。

```json
"env": { "YU_MCP_PROFILES": "core,tagging" }
```

未知のプロファイル名を指定すると起動時にエラーになる (タイプミスで
ツールが黙って消えるのを防ぐため)。
