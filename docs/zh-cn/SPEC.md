...
9. [扩展（Extensions）](#9-扩展extensions)
10. [配置（config.json）](#10-配置configjson)
11. [文件结构](#11-文件结构)
12. [开发规范](#12-开发规范)
...
**Yu AI Manager** 是一套 AI 生成图片、视频、音频、文字的本地数据库管理系统。  
设计理念为边缘优先、不依赖云端，优先在本地/LAN 内完成所有操作。
...
| MCP サーバー | Claude Desktop 等から直接操作可能な 180+ ツール |
...
| POST | `/api/agent/scope/<session_id>` | スコープ更新 |
...
| PUT | `/api/analysis/servers/<server_id>` | サーバー更新 |
...
| PUT | `/api/collections/<int:collection_id>` | 更新 |
...
| builtin-mcp-client | integration | 外部 MCP サーバー接続 |
...
- `feat:` → minor バージョンアップ
- `fix:` → patch バージョンアップ
- 作業のたびに `VERSION` / `CHANGELOG.md` / `TODO.md` を更新
