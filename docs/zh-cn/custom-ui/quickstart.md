...
服务器重启时会自动检测 `ui/custom/`。

```bash
python web_ui.py --db ./tags.db --port 5000
```

也可以在 `config.json` 中显式指定 UI：
...
|------|------|------|
| `/` | `index.html` | 主搜索页面 |
| `/stats` | `stats.html` | 统计仪表板 |
| `/tools` | `tools.html` | 工具页面 |
| `/settings` | `settings.html` | 设置页面 |
| `/extensions` | `extensions.html` | 扩展管理 |
| `/story` | `story.html` | Your Story 页面 |
| `/inspect` | `inspect.html` | 元数据检查器页面 |

放置这些名称的模板后，自定义 UI 将在相同的 URL 显示。如果用户访问不存在模板的路由，将会出现错误。
...
使用 POST / PUT / DELETE 的 API 调用需要 `X-Requested-With` 头：
...
以下是可以交给 Claude 或 ChatGPT 来生成自定义 UI 的示例提示：
...
完整 API 文档请参阅 [docs/api/](../api/README.md)。
