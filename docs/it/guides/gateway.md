# Gateway — Guida ai Limiti di Autenticazione LAN

> Versione di destinazione: Gateway Phase 1 (v4.75.0 以降) / Aggiunta di Gradio (v4.255.11 以降)

## Cos'è Gateway

Gateway は、SD WebUI・ComfyUI・Ollama・Gradio アプリなどの**strumenti backend senza funzionalità di autenticazione**へのアクセスを、  
**modello Bearer token + scope**unificato.
...
### Differenza dall'LLM Router

| | Gateway | LLM Router |
|---|---|---|
| **Destinazione** | SD WebUI・ComfyUI・Ollama・Gradio をまとめて | LLM (Ollama) のみ |
| **Autenticazione** | Bearer basato su scope obbligatorio | loopback は bypass 可 |
| **Proxy verso** | `/sd/*`, `/comfy/*`, `/v1/*`, `/gradio/<name>/*` | `/v1/*` のみ |
| **Uso principale** | 外部 / LAN に生成ツールを安全公開 | AI コーディングツールのバックエンド |

È possibile abilitare entrambi sulla stessa macchina。

---

## Configurazione

### 1. Creazione della prima chiave API (CLI)
...
### 2. Aggiunta della configurazione in config.json
...
### 3. Riavvio dell'app e verifica del funzionamento
...
そのまま透過転送するプロキシです。`OLLAMA_HOST` の向き先を Gateway に変えるだけでAutenticazioneが付きます。
...
Gradio ベースの WebUI（Irodori-TTS 等）を Gateway 経由でAutenticazione付きアクセス可能にします。  
...
```
/agentmemory/livez       → Autenticazione不要（ヘルスチェック）
...
| `loopback` | loopback (127.0.0.1) からはAutenticazione不要。LAN からは `api_key` と同等 |
| `none` | Autenticazioneなし（開発・テスト専用。本番不可）|
...
## Non-Goal（Phase 1 のDestinazione外）
...
## Gestione delle chiavi API nell'interfaccia web
