# Yu AI Manager — Полная спецификация

> **Целевая аудитория**: Claude Desktop などの AI エージェント  
> **Версия**: v4.91.15  
...
4. [Аутентификация и безопасность](#4-認証セキュリティ)
5. [REST API конечные точки](#5-rest-api-エンドポイント)
6. [MCP сервер](#6-mcp-サーバー)
7. [SSE события](#7-sse-イベント)
8. [Схема БД](#8-db-スキーマ)
9. [Расширения (Extensions)](#9-拡張extensions)
10. [Конфигурация (config.json)](#10-設定configjson)
11. [Структура файлов](#11-Структура файлов)
12. [Стандарты разработки](#12-Стандарты разработки)

---

## 1. Обзор проекта

**Yu AI Manager**  — это AI 生成画像・動画・音声・テキストного локальногоライブラリуправления.  
Приоритет граничных вычислений и независимость от облакакак принцип проектированияローカル/LAN при завершении.

### Основные функции

| Функция | Описание |
|------|------|
| Управление библиотекой | Сканирование, тегирование и поиск изображений/видео/аудио/текста |
...
| MCP сервер | Claude Desktop 等から直接操作可能な 180+ ツール |
...
| POST | `/api/agent/scope/<session_id>` | スコープОбновлено |
...
| POST | `/api/analysis/config` | 設定Обновлено |
...
| PUT | `/api/analysis/servers/<server_id>` | サーバーОбновлено |
...
| PUT | `/api/collections/<int:collection_id>` | Обновлено |
...
| GET | `/api/events/stream` | SSE событияストリーム |
...
| builtin-mcp-client | integration | 外部 MCP сервер接続 |
...
| builtin-tag-dictionary | utility | タグ辞書・Описание |
...
- `feat:` → minor Версияアップ
- `fix:` → patch Версияアップ
- 作業のたびに `VERSION` / `CHANGELOG.md` / `TODO.md` をОбновлено
...
- 新Функция: `docs/{ja,en,zh-tw,zh-cn,ko}/` に全言語作成（スタブ禁止）
