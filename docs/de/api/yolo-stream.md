...
> **Hinweis:** Die native Rust-Antwort listet USB-Kameras nur unter Linux und öffnet sie nie; `resolution` ist immer `null`. Windows und macOS geben `devices: []` zurück, und die Registrierung über einen numerischen Kameraindex wird dort nicht unterstützt.
>
> Auch der Event-Fan-out wird reduziert: keine implizite Wildcard-Zustellung an konfigurierte Webhook-Erweiterungen, kein LAN-Relay bei Übereinstimmung eines benutzerdefinierten Event-Namens mit `RELAY_TYPES` und kein eigener MCP-Event-Empfänger. `mcp_event` wird über den gemeinsamen SSE-Hub zugestellt.
