...
Enable the scheduler and configure built-in job schedules in `config.json`:
...
| `data.running` | boolean | Whether the scheduler is running |
...
|-------|------|-------------|
| `ok` | boolean | Success flag |
| `data.jobs` | array | Array of job objects |
...
| `data.jobs[].paused` | boolean | Whether the job is paused |
...
|-------|------|-------------|
| `ok` | boolean | Success flag |
| `data.history` | array | Array of execution history entries |
