...
(function () {
  ...
  /* Single entry point for every folder-browse button: try the native OS
   * dialog first, fall back to the server-side browser when the server has no
   * dialog (headless build without the `native-dialog` feature, Linux host
   * where rfd is not compiled in, or any non-desktop host). */
  window.pickFolder = function (initialPath, onSelect) {
    var current = initialPath || '';
    var qs = current ? '?initial=' + encodeURIComponent(current) : '';
    fetch('/api/tools/select-folder' + qs)
      .then(function (r) {
        if (!r.ok) throw new Error(String(r.status));
        return r.json();
      })
      .then(function (data) {
        ...
        window.dirPickerFallback(current, onSelect);
      })
      .catch(function () {
        window.dirPickerFallback(current, onSelect);
      });
