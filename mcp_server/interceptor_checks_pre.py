...
def pre_check(tool_name: str, facade_name: str | None = None) -> str | None:
    """Run the pre-execution checks for one tool call.

    ``facade_name`` is the yu_* tool a facade call arrived on, when the effective
    name differs. Only the scope fence is evaluated for it as well, so an operator
    can still deny a whole domain by name; kill switch, circuit breaker and budget
    must not run twice or the budget would be double-charged for one call.
    """
    for check in (check_kill_switch, check_circuit_breaker, check_budget, check_scope):
        ...
        if message:
            return message
    if facade_name:
        return check_scope(facade_name)
    return None
