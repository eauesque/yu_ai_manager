...
use meta_extract::{models::PngTextChunks, parse_metadata, read_exif_tags, read_png_text_chunks};
...
/// Read a fixture's text chunks the way the real scanner does.
///
/// PNG carries them as tEXt/iTXt; WebP and the other EXIF-bearing formats carry
/// them in `UserComment`, which the scanner lifts to `Comment`. Reading every
/// fixture as PNG made WebP fixtures parse as `unknown` -- and a golden that
/// records `unknown` looks like agreement.
///
/// Mirrors `exif_chunks_from_tags` in crates/tagdb-core/src/import/fallback_chain.rs.
fn read_fixture_chunks(path: &Path) -> PngTextChunks {
    let ext = path
        .extension()
        .and_then(|e| e.to_str())
        .unwrap_or("")
        .to_ascii_lowercase();
    if ext == "png" {
        return read_png_text_chunks(path);
    }

    let mut chunks = PngTextChunks::default();
    for (key, value) in read_exif_tags(path) {
        chunks.entries.insert(key.clone(), value.clone());
        if key.ends_with("UserComment") {
            ...
            if let Ok(Value::Object(map)) = serde_json::from_str::<Value>(&stripped) {
                for (json_key, json_value) in map {
                    if let Value::String(text) = json_value {
                        chunks.entries.entry(json_key).or_insert(text);
                    }
                }
            }
        }
...
fn run_conformance(parser_id: &str) {
    ...
    let chunks = read_fixture_chunks(&fixture_path);
