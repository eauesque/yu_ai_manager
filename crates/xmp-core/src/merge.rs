...
mod tests {
    ...
    fn merge_into_file_matches_uppercase_extension() {
        ...
        // `is_ok()` alone would pass a dispatcher that recognised `.PNG` and
        // then wrote nothing: the claim is that an upper-case extension takes
        // the same path as a lower-case one, so read the attribute back --
        // which is what the WebP test below already does.
        assert!(result.is_ok(), "{result:?}");
        let raw = crate::io::png::read_xmp(&path).expect("the merged PNG must carry XMP");
        let parsed = crate::packet::parse(&raw);
        assert_eq!(
            parsed.get_attrs("wdtag").get("model"),
            Some(&"m".to_string()),
            "an upper-case extension took a path that wrote nothing"
        );
    }
