...
/// Cache-busting token appended to bundle URLs as `?v=`.
///
/// Reads the file `build.mjs` actually writes: `.build-info.json`, dot-prefixed.
/// This looked for `dist_info.json`, which nothing has ever written, so the
/// token was permanently the `"dev"` fallback — and a browser that had loaded
/// `tools-app.js?v=dev` once kept serving it from cache through every later
/// rebuild. `scripts/check_dist_freshness.py` reads the same file and key.
///
/// `scripts/fast_mode.py::expected_ui_contract` still reads the old name *and*
/// a `v` key the build never writes; it therefore always yields `None`. That
/// one is left alone here because its callers have not been traced.
pub fn dist_v(project_root: &Path) -> String {
    let info_path = project_root.join("ui/default/static/dist/.build-info.json");
...
mod tests {
    ...
    /// The token must come from the file `build.mjs` writes.
    ///
    /// Pinning the *filename* is the point: the previous name
    /// (`dist_info.json`) was never written by anything, so `dist_v` silently
    /// returned the `"dev"` fallback forever and browsers cached one bundle
    /// across every rebuild. A test that only checked "returns a string" would
    /// have passed throughout.
    #[test]
    fn the_token_is_the_build_hash_when_the_build_info_is_present() {
        let root = tempfile::tempdir().unwrap();
        let dist = root.path().join("ui/default/static/dist");
        std::fs::create_dir_all(&dist).unwrap();
        std::fs::write(
            dist.join(".build-info.json"),
            r#"{"src_hash": "abcdef1234567890", "built_at": 1}"#,
        )
        .unwrap();

        assert_eq!(dist_v(root.path()), "abcdef12", "first 8 chars of src_hash");
    }

    #[test]
    fn a_rebuild_changes_the_token() {
        // The whole purpose: a new bundle must reach a browser that cached the
        // old one. Equal tokens across differing sources is the bug.
        let root = tempfile::tempdir().unwrap();
        let dist = root.path().join("ui/default/static/dist");
        std::fs::create_dir_all(&dist).unwrap();
        let info = dist.join(".build-info.json");

        std::fs::write(&info, r#"{"src_hash": "1111111111", "built_at": 1}"#).unwrap();
        let before = dist_v(root.path());
        std::fs::write(&info, r#"{"src_hash": "2222222222", "built_at": 2}"#).unwrap();
        let after = dist_v(root.path());

        assert_ne!(before, after, "a rebuilt bundle must get a new token");
    }

    #[test]
    fn a_missing_build_info_falls_back_to_dev() {
        // Keeps the fallback deliberate rather than accidental: a source tree
        // that was never built has no hash to report.
        let root = tempfile::tempdir().unwrap();
        assert_eq!(dist_v(root.path()), "dev");
    }

    #[test]
    fn the_legacy_filename_is_not_consulted() {
        // Writing only the old name must NOT produce a token. Without this,
        // restoring the old path would go unnoticed.
        let root = tempfile::tempdir().unwrap();
        let dist = root.path().join("ui/default/static/dist");
        std::fs::create_dir_all(&dist).unwrap();
        std::fs::write(
            dist.join("dist_info.json"),
            r#"{"src_hash": "deadbeefcafe", "built_at": 1}"#,
        )
        .unwrap();
        ...
        assert_eq!(
            dist_v(root.path()),
            "dev",
            "only .build-info.json is the contract"
        );
    }
}
