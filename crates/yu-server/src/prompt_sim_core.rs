...
#[cfg(test)]
mod tests {
    /// The DP group field is named `expression`, matching Python and the UI.
    ///
    /// Rust emitted `expr` until v4.713.5. Python's `dp_probability.py` emits
    /// `expression` and the simulator template reads `grp.expression`
    /// (`simulator_parts/_script.html:204`), so on a standalone Rust server
    /// every DP group header rendered as `undefined`. The parity entry could
    /// not catch it: it claimed `python_path: None`, so the two bodies were
    /// never compared.
    #[test]
    fn dp_groups_use_the_field_name_the_ui_reads() {
        let groups = analyze_dp_choices("{a|b|c}");
        assert!(!groups.is_empty(), "the fixture must produce a group");
        for group in &groups {
            assert!(
                group.get("expression").is_some(),
                "the UI reads grp.expression: {group}"
            );
            assert!(
                group.get("expr").is_none(),
                "the old name must not come back alongside it: {group}"
            );
        }
    }
