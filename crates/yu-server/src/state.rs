...
pub struct Config {
    ...
    /// Proxies the rate limiter trusts when resolving a client IP, read from
    /// `server.trusted_proxy_ips`.
    ///
    /// Deliberately separate from `trusted_ips` / `trusted_proxy_enabled`, which
    /// fuse two concerns Python keeps apart: `trusted_proxy_ips` decides *who is
    /// a proxy* (Python installs ProxyFix on it, rewriting the client address),
    /// while `trusted_proxy_auth` decides *whether that proxy's `X-Remote-User`
    /// is honoured*. Rust binds both to `trusted_proxy_auth`, so on the ordinary
    /// reverse-proxy deployment -- `trusted_proxy_ips` set, `trusted_proxy_auth`
    /// off -- `resolve_client_ip` returns the TCP peer, i.e. the proxy's own
    /// address. Keying the limiter on that would put an entire LAN in one
    /// bucket and hand DESTRUCTIVE's 12/min to everyone jointly: a rate limiter
    /// that is itself the denial of service.
    ///
    /// Reusing `trusted_ips` instead would mean feeding it from config, which
    /// also opens `X-Remote-User` delegation on deployments that set both keys
    /// -- fail-closed to fail-open. That belongs in its own ticket with a
    /// security review, not inside a port. See
    /// `tmp/trusted-proxy-config-sources.md`.
    ///
    /// Empty by default, which makes `resolve_client_ip` return the TCP peer:
    /// identical to today's behaviour on any deployment without a proxy.
    pub rate_limit_trusted_proxies: HashSet<String>,
    ...
    /// WD-Tagger cache root resolved once at startup (primary if it holds
    /// anything, else legacy `~/.cache/yu_ai_manager/wd_tagger`), matching
    /// `routes::wd_tagger::wd_tagger_root(&cache_dir)`. Every producer that
    /// needs "the" WD-Tagger root -- the yu-infer sidecar spawn, its
    /// crash-supervisor respawn, the in-process ONNX fallback, and LAN mesh
    /// capability reporting -- reads this single value instead of
    /// recomputing it, so a filesystem change mid-run (or the up-to-25s
    /// window while the sidecar is starting) cannot make them disagree about
    /// which directory is authoritative (arch-constraints.yaml's
    /// single-computation rule).
    pub wd_tagger_root: PathBuf,
    /// CLIP ONNX model directory resolved once at startup, matching
    /// `routes::clip_model::model_dir(&cache_dir)` (including the
    /// `HAILO_CLIP_TEXT_MODEL_DIR` override). Shared the same way as
    /// `wd_tagger_root`.
...
pub struct AppState {
    ...
    /// Per-IP three-tier limiter for `/api/` and `/ext/` requests.
    ///
    /// Distinct from `rate_limiter` above, which counts failed PIN attempts and
    /// nothing else. Held here rather than in statics so each test builds its
    /// own: a shared static would make "the third request is refused" depend on
    /// which tests ran first.
    pub api_rate_limiters: crate::auth::api_rate_limit::RateLimiters,
    ...
    /// Requests parked while a scan is running, persisted to
    /// `data/scan_queue.json` (shared with Python).
    ...
    /// The DB signature the stats caches above were populated under.
    ///
    /// A TTL hit alone is no evidence the cached aggregate still agrees with the
    /// DB: within the window the DB can change underneath it. `build_stats_all`
    /// compares the live signature against this and drops the stats caches when
    /// it moves, so a hit always agrees with the DB. See stats.rs.
    pub stats_db_sig: tokio::sync::Mutex<Option<serde_json::Value>>,
}
...
impl AppState {
    ...
    pub async fn new_with_infer_and_vectors(
        ...
        let scan_queue = Arc::new(crate::scan_queue::ScanQueue::load(&config.project_root));
        Self {
            config,
            effective_port: 5000,
...
/// Test state for the restart route/server-info blockers, which (unlike
/// `semantic_test_state_with_root`) need control over the trusted-proxy
/// settings that feed `is_local_request`'s trusted-set decision (spec
/// `決定 2`).
#[cfg(test)]
pub(crate) async fn restart_test_state(
    pin_auth_enabled: bool,
    ...
    Arc::new(
        AppState::new(
            Config {
                ...
                config_path: PathBuf::from(".").join("config.json"),
...
/// Same as [`semantic_test_state_with`] but lets a test set `project_root` too.
#[cfg(test)]
