...
pub(crate) fn validate_base_url(s: &str) -> Result<String, &'static str> {
    ...
    // `core/gateway/headroom_proxy.py::validate_base_url` also rejects userinfo, and
    // a base_url carrying credentials would put them in every proxied request's URL.
