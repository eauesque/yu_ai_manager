...
static API_KEY: LazyLock<Regex> =
    LazyLock::new(|| Regex::new(r"sk_[0-9a-f]{32}").expect("api key pattern is a literal"));
...
pub(crate) fn scrub_secrets(text: &str) -> String {
    ...
    let step = KV.replace_all(&step, format!("${{key}}${{sep}}{MASK}"));
    // A freshly issued key is readable exactly once, in the POST /api/apikeys
    // response. The rules above key off a surrounding name or scheme, so a bare
    // `sk_...` in a message body would survive all three.
    API_KEY.replace_all(&step, MASK).into_owned()
}
...
mod tests {
    ...
    const LEAKS: &[(&str, &str)] = &[
        ...
        // A key straight out of POST /api/apikeys: no key name and no scheme,
        // so the KV/BEARER/CLI_FLAG rules all miss it.
        (
            "issued sk_0123456789abcdef0123456789abcdef for label dev",
