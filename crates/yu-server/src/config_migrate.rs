...
/// Strip matched surrounding quotes from every stored scan root, once, at startup.
///
/// Windows Explorer's "Copy as path" yields `"D:\\pictures"`, and a config
/// written before the input was normalised keeps those quotes forever: the
/// entry then fails every path check, so the folder silently scans nothing.
/// Adding and editing a root already unquote, but the entries already on disk
/// are only reachable here -- and healing the file, rather than each reader,
/// fixes the scanner and the watcher too, not just the settings list.
///
/// Returns the number of roots rewritten.
pub(crate) fn normalize_scan_root_quotes(config_path: &Path) -> usize {
    let raw = match std::fs::read_to_string(config_path) {
        Ok(raw) => raw,
        Err(_) => return 0,
    };
    let mut config = match config_io::parse(config_path, &raw) {
        Some(config) => config,
        None => return 0,
    };
    let Some(roots) = config.get_mut("scan_roots").and_then(Value::as_array_mut) else {
        return 0;
    };

    let mut fixed = 0usize;
    for root in roots.iter_mut() {
        // Roots are usually objects, but a bare string is accepted elsewhere.
        let current = root
            .get("path")
            .and_then(Value::as_str)
            .or_else(|| root.as_str());
        let Some(current) = current else { continue };
        let unquoted = crate::routes::scan_roots::unquote_path(current);
        if unquoted == current {
            continue;
        }
        if let Some(map) = root.as_object_mut() {
            map.insert("path".to_string(), Value::String(unquoted));
        } else {
            *root = Value::String(unquoted);
        }
        fixed += 1;
    }
    ...
    if let Err(error) = config_io::write(config_path, &config) {
        tracing::warn!(%error, "failed to rewrite unquoted scan roots");
        return 0;
    }
    fixed
}
...
mod tests {
    ...
    /// A root stored with the quotes Explorer's "Copy as path" supplies scans
    /// nothing until they are stripped, and only this startup pass can reach
    /// entries already written to disk.
    #[test]
    fn startup_strips_quotes_from_already_stored_scan_roots() {
        let root = tempfile::tempdir().unwrap();
        let path = root.path().join("config.json");
        config_io::write(
            &path,
            &json!({"scan_roots": [
                {"path": "\"D:\\pictures\"", "enabled": true},
                {"path": "/srv/plain"},
                {"path": "/srv/one\"quote"},
            ]}),
        )
        ...
        assert_eq!(normalize_scan_root_quotes(&path), 1);
        let after = config_io::load(&path);
        assert_eq!(after["scan_roots"][0]["path"], json!("D:\\pictures"));
        assert_eq!(after["scan_roots"][0]["enabled"], json!(true));
        assert_eq!(after["scan_roots"][1]["path"], json!("/srv/plain"));
        // A lone quote is part of a legitimate POSIX filename -- keep it.
        assert_eq!(after["scan_roots"][2]["path"], json!("/srv/one\"quote"));
        // Second run has nothing left to do.
        assert_eq!(normalize_scan_root_quotes(&path), 0);
    }
