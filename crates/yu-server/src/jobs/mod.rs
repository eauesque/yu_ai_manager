...
impl JobManager {
    ...
    pub fn update_progress(&self, job_id: &str, current: u64, total: u64, message: Option<String>) {
        if let Some(j) = self.jobs.lock().unwrap().get_mut(job_id) {
            ...
            j.total = Some(total);
            // Truncate to a whole percent, matching Python's
            // `int((current / total) * 100)` in core/jobs_core/jobs_model.py.
            // Serializing the raw f64 leaks IEEE 754 precision to every job UI
            // (e.g. "2.4532345634564%"), since no caller rounds on display.
            j.percent = if total > 0 {
                Some((current as f64 / total as f64 * 100.0).trunc())
            } else {
                None
            };
            j.message = message;
    ...
    /// Labels of every currently running job, sorted.
    ///
    /// Exists for the doctor's runtime snapshot: a backlog of stuck jobs is
    /// invisible from outside the process, so it cannot be diagnosed by the
    /// pre-launch `scripts/doctor.py`.
...
mod tests {
    ...
    #[test]
    fn test_job_update_progress_truncates_to_whole_percent() {
        // 27/1100 is 2.4545...% — serializing the raw f64 is what leaked
        // "2.4532345634564%"-style values into the job progress UIs.
        let mgr = JobManager::new();
        mgr.start("j6b", "Progress");
        mgr.update_progress("j6b", 27, 1100, None);
        let d = mgr.get_job("j6b").unwrap();
        assert_eq!(d.percent.unwrap(), 2.0);
    }
