...
//! syncs deletions. Archive traversal (`scan_zips`) is handled by
...
pub async fn run_scan_root(
    ...
    let config = read_config(&state.config.config_path).unwrap_or_default();
    let toggles = crate::ext_config::parser_toggles(&config);
    let mut extensions = scan_exts();
    if scan_zips {
        extensions.extend(
            crate::scan_archive::ARCHIVE_EXTS
                .iter()
    ...
    for (idx, path) in files.iter().enumerate() {
        ...
        if crate::scan_archive::archive_kind(path).is_some() {
            // Only reachable when scan_zips put the archive extensions in the
            // walk filter.
            let archive_outcome =
                crate::scan_archive::import_archive(&state.db, path, force, toggles).await;
            outcome.added += archive_outcome.imported;
            outcome.errors += archive_outcome.errors;
        ...
        let ok = match native_import_one_with(&state.db, &path_str, force, toggles).await {
            Ok(_) => true,
            Err(e) => {
    ...
    if scan_zips {
        outcome.deleted += crate::scan_archive::sync_deleted_archive_members(&state.db, root).await;
    }
    outcome
}
...
pub async fn run_scan_all(
    ...
    for (idx, r) in roots.iter().enumerate() {
        ...
        let root_outcome = run_scan_root(
            state,
            &r.path,
            r.recursive,
            force,
