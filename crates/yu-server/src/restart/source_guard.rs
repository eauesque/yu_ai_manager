...
#[cfg(all(unix, not(target_os = "macos")))]
#[test]
fn linux_exec_restart_marks_inherited_descriptors_cloexec_before_exec() {
    // Structural, because the effect is not observable from inside the test
    // process: measured, emptying this loop leaves every restart unit test
    // green. `exec()` replaces the process with no fork, so a listening
    // socket that survives here makes the restarted server's bind fail with
    // EADDRINUSE -- the failure this guard exists to keep out.
    //
    // This was previously a `libc::close()` loop, mirroring Python's
    // `os.closerange()` unconditional close. That form was measured unsafe
    // in this process: the real-machine restart probe hit
    // `tokio-rt-worker panicked ... worker thread panicking; aborting
    // process` in 4 of 5 runs, because closing tokio's epoll/eventfd out
    // from under a still-running multi-threaded runtime lets another
    // worker touch a now-closed fd before execv lands. This is the same
    // class of failure `core/platform/process_restart.py:1-11` documents
    // for why macOS is routed away from `closerange()` (libdispatch /
    // CoreFoundation aborting before execv runs) -- we had copied the
    // Linux-shaped close loop into an environment (multi-threaded tokio)
    // that has the property which made macOS unsafe.
    //
    // FD_CLOEXEC achieves closerange's actual intent -- the new image
    // inherits nothing -- atomically at execve, so no window exists for a
    // live worker to observe a closed fd. This guard now says the range is
    // still marked close-on-exec and still ordered before the exec; it does
    // not prove the listener is gone, that needs a live restart, see
    // QA-rust-restart-debug-log.md §4.1/§5.2.
    let body = linux_exec_restart_body();
    let Some(fcntl_at) = body.find("libc::fcntl(") else {
        panic!(
            "exec_restart must mark inherited descriptors FD_CLOEXEC before exec\nbody was:\n{body}"
        );
    };
    assert!(
        body.contains("libc::F_SETFD") && body.contains("libc::FD_CLOEXEC"),
        "the fcntl call must set FD_CLOEXEC via F_SETFD, not some other \
         fcntl command\nbody was:\n{body}",
    );
    assert!(
        !body.contains("libc::close("),
        "a bare libc::close() loop must not return here -- that is the \
         closerange-shaped form measured to abort the process 4/5 times \
         under a live multi-threaded tokio runtime\nbody was:\n{body}",
    );
    ...
    assert!(
        fcntl_at < exec_at,
        "descriptors must be marked close-on-exec BEFORE exec replaces the \
         process; doing it after would never run\nbody was:\n{body}",
    );
    assert!(
    ...
    // here green -- the fcntl call is still written, it just never runs. Pin
    // the bounds too, or the guard passes an implementation that marks
    // nothing.
    assert!(
        body.contains("for fd in first..last"),
        "the fcntl loop must span the whole inherited range (first..last); \
         an empty range marks no descriptor while still looking correct\n\
         body was:\n{body}",
    );
}
