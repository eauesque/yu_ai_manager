//! Restart-API infrastructure, ported from Python.
//!
//! Stage A1 (settings resolution, local-request and token checks, the
//! cooldown gate, `exec_restart`) plus stage A2 (`routes::server_restart`,
//! mounted as `POST /api/server/restart`) have landed -- this module is
//! reachable in production now. `restart-with-config` and `switch-profile`
//! (stage A3) are still absent: no route calls them yet.
//!
//! The nine CLI flags that would turn on the A3 endpoints are still
//! deliberately absent from `Cli`, on purpose. yu-server has
//! `/api/server/restart` but not yet `/api/server/restart-with-config` or
//! `/api/server/switch-profile`, so a `--allow-remote-restart` that clap
//! accepted today would still half-enable them: the flag would parse fine,
//! but the routes it is meant to gate do not all exist. Accepting it would
//! *also* stop `scripts/fast_mode.py` from declining fast mode -- and that
//! decline is what currently routes such a launch to the Python server,
//! which does implement all three. Adding the flags before every route they
//! gate exists would therefore turn a working launch into a silently broken
//! one. They go in together with stage A3, not before.
//!
//! See `docs/development/development_docs/QA-rust-restart-debug-log.md` for
//! what is still unimplemented and unverified.

// Module-wide allow removed after A2 wired the route in: most items here are
// now reachable from `routes::server_restart` and no longer need it. What
// remains genuinely unreachable on every platform (`RestartReservation::commit`,
// `launch_restart`'s `Ok` else-branch) is annotated with a narrow `#[allow(dead_code)]`
// at its own definition, with the reason recorded there -- see the doc comments
// on `commit` and in `launch_restart`.
...
/// Reads the `pin_ok` session flag directly, matching Python's
/// `session_obj.get("pin_ok")` (`restart_route_helpers.py:41-43`). Shared by
/// the restart route and `server_info_body`'s `restart_blockers` computation
/// (spec `決定 1`/`決定 7`) so the two call sites cannot silently diverge on
/// what counts as an authenticated PIN session.
///
/// Deliberately NOT derived from `AuthContext`: an API-key holder must not
/// count as PIN-authenticated for restart purposes.
pub async fn session_pin_ok(session: Option<Extension<Session>>) -> bool {
    match session {
...
/// Holds the restart gate until the handoff is known to be successful.
/// Dropping an uncommitted reservation restores the exact prior state.
#[derive(Debug)]
pub struct RestartReservation<'a> {
    state: &'a RestartState,
    previous: RestartStateInner,
    committed: bool,
}

impl RestartReservation<'_> {
    /// Marks the reservation as successfully handed off so `Drop` does not
    /// restore prior state. Currently unreachable in practice: every
    /// `exec_restart` variant (unix non-macos and macos exec the new image
    /// in place, windows calls `process::exit(0)`) only ever returns on
    /// `Err`, so `launch_restart`'s `Ok` arm below never runs on any
    /// supported platform today. Kept (not removed) as the correct RAII
    /// contract for any future platform or dry-run/test path whose
    /// `exec_restart` could legitimately return `Ok` -- deleting it would
    /// silently make the reservation un-committable if one is ever added.
    pub fn commit(mut self) {
        self.committed = true;
    }
}

impl Drop for RestartReservation<'_> {
    fn drop(&mut self) {
        if !self.committed {
            *self
                .state
                .inner
                .lock()
                .unwrap_or_else(|err| err.into_inner()) = RestartStateInner {
                in_progress: self.previous.in_progress,
                last_requested_at: self.previous.last_requested_at,
            };
        }
    }
}
...
impl RestartState {
    pub fn enforce_restart_cooldown(&self) -> Result<RestartReservation<'_>, RestartCooldown> {
        ...
        let previous = RestartStateInner {
...
pub fn restart_args() -> io::Result<Vec<String>> {
    restart_args_from_current_exe(std::env::current_exe(), std::env::args().skip(1))
}

pub fn restart_args_from_current_exe(
    executable: io::Result<std::path::PathBuf>,
    args: impl IntoIterator<Item = String>,
) -> io::Result<Vec<String>> {
    Ok(std::iter::once(executable?.to_string_lossy().into_owned())
        .chain(args)
        .collect())
}

pub fn launch_restart(reservation: RestartReservation<'static>, args: Vec<String>) {
    std::thread::spawn(move || {
        ...
        if let Err(error) = exec_restart(&args) {
            ...
            // Unreachable on every current platform (see `commit`'s doc
            // comment) -- kept for the same reason: a future `exec_restart`
            // that can return `Ok` must still see the reservation committed.
            reservation.commit();
        }
    });
}
...
pub fn exec_restart(args: &[String]) -> io::Result<()> {
    ...
    #[allow(
        clippy::exit,
        reason = "Windows has no execve; the freshly spawned child replaces this process"
    )]
    std::process::exit(0);
}
...
pub fn exec_restart(args: &[String]) -> io::Result<()> {
    ...
    #[allow(
        clippy::exit,
        reason = "pre_exec's setsid detaches the child; this process still exits in its place"
    )]
    std::process::exit(0);
}
...
pub fn exec_restart(args: &[String]) -> io::Result<()> {
    ...
    // still open here is inherited by the new server unless marked
    // close-on-exec. The listening socket is the one that matters: if it
    // survives, the restarted server's bind fails with EADDRINUSE and the
    // restart dies without a visible cause. Rust's std sets CLOEXEC on the
    // descriptors it opens, but that has not been measured for every
    // descriptor this process holds (tokio's internals, SQLCipher's file
    // handles, any C library loaded at runtime), so this sets FD_CLOEXEC on
    // the whole inherited range itself rather than trusting that assumption.
    //
    // This does NOT close the descriptors with `close()` (which the earlier
    // version of this function did, mirroring Python's `os.closerange()`).
    // That was measured to be unsafe here: the real-machine restart probe hit
    // `tokio-rt-worker panicked ... worker thread panicking; aborting process`
    // in 4 of 5 runs, because closing tokio's epoll/eventfd out from under a
    // still-running multi-threaded runtime lets another worker touch a
    // now-closed fd before execv lands -- the exact class of failure
    // `core/platform/process_restart.py:1-11` documents for why macOS is
    // routed away from `closerange()` in the first place (libdispatch /
    // CoreFoundation aborting before execv runs). We had copied the
    // Linux-shaped close loop into an environment (multi-threaded tokio) that
    // has the property which made macOS unsafe. FD_CLOEXEC achieves
    // closerange's actual intent -- the new image inherits nothing -- but
    // atomically at execve, so there is no window for a live worker to
    // observe a closed fd. It also removes the old accepted cost below: if
    // `exec` now fails, the descriptors are still open and usable, so the
    // process is left undamaged rather than unrecoverable.
    let (first, last) = inherited_fd_range();
    for fd in first..last {
        // SAFETY: fcntl() on an integer descriptor; an already-closed or
        // never opened fd returns EBADF, which is ignored on purpose -- this
        // only needs to mark real, still-open descriptors close-on-exec.
        unsafe {
...
mod tests {
    ...
    fn cooldown_reports_the_time_actually_left_while_a_restart_is_in_progress() {
        ...
        let reservation = state.enforce_restart_cooldown().expect("first call passes");
        let err = state
    ...
    #[test]
    fn a_dropped_reservation_releases_the_gate() {
        let state = RestartState::default();
        drop(state.enforce_restart_cooldown().unwrap());
        assert!(state.enforce_restart_cooldown().is_ok());
    }

    #[test]
    fn restart_args_maps_current_exe_failure_to_an_error() {
        assert!(
            restart_args_from_current_exe(Err(io::Error::other("injected")), Vec::new()).is_err()
        );
    }
