...
impl InferClient {
    ...
    /// Retries once on a transport-level send failure (observed on Windows as
    /// a spurious `WSAECONNABORTED` reset immediately after a slow handler
    /// releases a stalled response) rather than surfacing a fatal error for
    /// what is, from the caller's perspective, a single flaky attempt.
    /// Bodies built from `.json(...)` are always cloneable, so `try_clone()`
    /// only returns `None` for streaming bodies no caller here uses.
    ...
    /// yu-infer's `/healthz` (unauthenticated on the sidecar side). Carries
    /// `hailo_stub` from yu-hailo-infer 0.4.0 onwards: true when that binary
    /// was built without HailoRT headers and therefore links the stub shim.
    /// Callers must bound this themselves -- the shared client has no request
    /// timeout, so a wedged sidecar would otherwise hang them.
    pub async fn healthz(&self) -> Result<serde_json::Value, InferClientError> {
    ...
    pub async fn infer_clip_image_onnx(
        &self,
        image_base64: String,
    ) -> Result<serde_json::Value, InferClientError> {
        let response = self
            .http
            .post(self.endpoint("/v1/infer/clip-image-onnx"))
            .bearer_auth(&self.auth_token)
            .json(&json!({"image_base64": image_base64}))
            .send()
            .await?;
...
mod tests {
    ...
    fn hailo_endpoints_match_yu_infer_routes() {
        ...
        assert_eq!(
            client.endpoint("/v1/infer/clip-image-onnx"),
            "http://127.0.0.1:18771/v1/infer/clip-image-onnx"
        );
        assert_eq!(
