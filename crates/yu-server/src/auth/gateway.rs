    ...
    for entry in entries {
        ...
        keys.push(GatewayKey {
            ...
            scopes: entry
                ...
                .unwrap_or_default(),
            allowed_models: entry.get("allowed_models").cloned().unwrap_or(Value::Null),
        });
    }
    keys
}

/// List only usable gateway keys, without exposing their encrypted secrets.
pub fn list_keys(auth_cfg: &Value, project_root: &Path) -> Vec<Value> {
    load_keys(auth_cfg, project_root)
        .into_iter()
        .map(|key| {
            let mut scopes = key.scopes;
            scopes.sort();
            scopes.dedup();
            json!({
                "id": key.id,
...
mod tests {
    ...
    #[test]
    fn gateway_key_listing_skips_undecryptable_secret() {
        let (root, _key) = temp_root("listing-undecryptable");
        let foreign = base64::engine::general_purpose::URL_SAFE.encode([7_u8; 32]);
        let cfg = json!({"api_keys": [{
            "id": "lost", "secret_enc": seal("secret", &foreign), "scopes": [SCOPE_OLLAMA_PROXY],
        }]});

        assert!(list_keys(&cfg, &root).is_empty());
    }

    #[test]
    fn gateway_key_listing_skips_empty_secret() {
        let (root, key) = temp_root("listing-empty");
        let cfg = json!({"api_keys": [{
            "id": "empty", "secret_enc": seal("", &key), "scopes": [SCOPE_OLLAMA_PROXY],
        }]});
    ...
    #[test]
    fn gateway_key_listing_returns_only_safe_fields() {
        let (root, key) = temp_root("listing-safe-fields");
        let cfg = json!({"api_keys": [{
            "id": "live", "secret_enc": seal("secret", &key),
            "scopes": ["zeta", "alpha", "zeta"], "allowed_models": ["model-a"],
        }]});

        let listed = list_keys(&cfg, &root);
        assert_eq!(
            listed,
            vec![json!({
                "id": "live", "scopes": ["alpha", "zeta"], "allowed_models": ["model-a"],
            })]
        );
