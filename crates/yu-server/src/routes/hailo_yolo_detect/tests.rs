...
fn hailo_yolo_hef_path_resolves_non_default_models_explicitly() {
    ...
    // Component-wise, not string, comparison: PathBuf::join inserts the
    // platform's native separator, so this ends_with("a/b") literal only
    // holds as written on Unix.
    assert!(
        std::path::Path::new(&path).ends_with("hailo_models/yolov11n.hef"),
        "{path}"
    );
}
...
fn hailo_yolo_hef_path_uses_special_filename_for_yolov5m() {
    ...
    assert!(
        std::path::Path::new(&path).ends_with("hailo_models/yolov5m_wo_spp.hef"),
        "{path}"
    );
}
...
/// The first inference call: announce that the worker is inside it, then wait
/// for the test to cancel before answering.
///
/// Two earlier forms both raced, in opposite directions:
///
/// * Cancelling inside the handler BEFORE writing the response (until
///   v4.732.15). `token.cancel()` ran first, so whenever the response then lost
///   its own race the worker took the "in-flight request failed while
///   cancelled" arm of run_detect_worker -- which reports a cancellation with
///   `processed: 0`. That arm's own comment records the same race costing
///   "roughly two runs in five". Measured: 1 failure in 3 runs even in
///   isolation, so reading three green standalone runs as "load-flaky" was
///   simply wrong.
/// * Cancelling from the test after polling the DB for file 1's row. The worker
///   finishes both files in well under the 10ms poll interval, so the cancel
///   landed after file 2: `processed: 2`, 18 failures in 20.
///
/// Neither raced on purpose; both tried to *win* a race. This form removes it:
/// the response cannot be written until the token is already cancelled, so the
/// worker always finishes file 1 and always meets the cancelled token at the
/// loop's next `is_cancelled()` -- the file boundary this test is about.
async fn detect_once_cancelled(State(gate): State<std::sync::Arc<DetectGate>>) -> Json<Value> {
    gate.inside_inference.notify_one();
    gate.cancel.cancelled().await;
...
struct DetectGate {
    ...
    inside_inference: tokio::sync::Notify,
}

#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
async fn run_detect_worker_stops_at_file_boundary_when_cancelled() {
    let cancel = tokio_util::sync::CancellationToken::new();
    let gate = std::sync::Arc::new(DetectGate {
        cancel: cancel.clone(),
        inside_inference: tokio::sync::Notify::new(),
    });
    ...
    let server = tokio::spawn(async move {
        axum::serve(
            listener,
            Router::new()
                .route("/v1/infer/yolo/detect", post(detect_once_cancelled))
                .with_state(handler_gate),
        )
        .await
        .unwrap();
    });
    ...
    // Register the job so `get_job` below has something to report on. The
    // worker is driven by the token this test cancels, not by this one.
    let _registered = state
        .job_manager
        .start(HAILO_YOLO_JOB_ID, "Hailo YOLO detection");
    ...
    // Wait until the worker is inside file 1's inference call, then cancel. The
    // handler will not answer until this cancel has landed, so the ordering is
    // guaranteed rather than raced: file 1 completes, file 2 is never started.
    gate.inside_inference.notified().await;
    cancel.cancel();
    worker.await.unwrap();
    server.abort();
    ...
    // Report the whole tally, not just the field that tripped: a bare
    // `assert_eq!(result["processed"], 1)` could not say which branch swallowed
    // the file, and that cost two rounds of guessing before the cause was found.
    let tally = format!("{result}");

    // The claim under test is the FILE BOUNDARY: once cancelled, file 2 is never
    // started. Assert that against the DB, not against `processed`.
    //
    // `processed` cannot carry this claim. run_detect_worker has two correct
    // endings for a cancelled run and the test must forbid neither:
    //
    //   * the response arrives     -> processed 1, then the loop's
    //                                 `is_cancelled()` breaks before file 2
    //   * the in-flight request fails while already cancelled -> processed 0,
    //     reported as a cancellation, not an error (that arm exists on purpose)
    //
    // Pinning `processed == 1` made the second ending a failure: 1 run in 3
    // before v4.732.15. Relaxing it to `<= 1` then forbade nothing at all --
    // fault injection proved it, the test stayed GREEN with the loop's cancel
    // check replaced by `if false`. The row count is the check that actually
    // fails when the worker stops honouring the cancel.
    let file2_rows: i64 =
        sqlx::query_scalar("SELECT COUNT(*) FROM file_annotations WHERE file_id = 2")
            .fetch_one(&state.db_read)
            .await
            .unwrap();
    assert_eq!(
        file2_rows, 0,
        "file 2 must never be written after the cancel; full tally: {tally}"
    );
    let processed = result["processed"].as_i64().expect("processed is a number");
    assert!(processed <= 1, "processed; full tally: {tally}");
    assert_eq!(result["skipped"], 0, "skipped; full tally: {tally}");
    assert_eq!(result["errors"], 0, "errors; full tally: {tally}");
    assert_eq!(result["total"], 2, "total; full tally: {tally}");
}

/// Cancel the job from inside the request, then fail it.
///
/// This is what makes the sibling test deterministic: the worker is already
/// past the loop's `is_cancelled()` check and inside the inference call when
/// the cancellation lands, and the call then fails. No sleeps, no ordering
/// assumptions.
async fn cancel_then_fail(
    State(token): State<tokio_util::sync::CancellationToken>,
...
#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
async fn a_cancelled_job_whose_inference_fails_is_still_cancelled() {
    // The sibling test cancels while a request is in flight and then releases
    // it, so the request succeeds. Which outcome you got there depended on
    // timing: if the client gave up on the stalled request before the release
    // landed, the worker took the Err branch and reported "error" even though
    // the operator had cancelled. That is what made it flaky -- two runs in
    // five, always the slow ones.
    //
    // Here the request itself cancels the job and then fails, so the Err
    // branch is always reached with the token already set. No sleeps, no
    // ordering assumptions. "cancelled" must win over the failure.
    //
    // The token is made here rather than taken from the job manager only to
    // break a circular dependency (the handler needs it before the state that
    // would hand it out exists). `run_detect_worker` takes the token as a
    // parameter and uses it for nothing but `is_cancelled`, so this is the
    // same signal the real flow carries.
    let cancel = tokio_util::sync::CancellationToken::new();
    let listener = tokio::net::TcpListener::bind("127.0.0.1:0").await.unwrap();
    let address = listener.local_addr().unwrap();
    ...
    let server = tokio::spawn(async move {
        axum::serve(
            listener,
            Router::new()
                .route("/v1/infer/yolo/detect", post(cancel_then_fail))
                .with_state(handler_token),
        )
        .await
        .unwrap();
    });
    ...
    .await
    .unwrap();
    let only = dirs.root.join("only.png");
    image::RgbImage::new(1, 1).save(&only).unwrap();
    sqlx::query("UPDATE files SET path = ? WHERE id = 1")
        .bind(only.to_string_lossy().to_string())
        .execute(&state.db)
        .await
        .unwrap();
    // Register the job so `get_job` below has something to report on.
    let _registered = state
        .job_manager
        .start(HAILO_YOLO_JOB_ID, "Hailo YOLO detection");
    ...
    assert_eq!(
        job.phase.as_deref(),
        Some("cancelled"),
        "an inference failure must not overwrite a cancellation"
    );
}
