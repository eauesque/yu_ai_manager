...
/// Mirror of `routes/settings_manage.py`'s `SettingValueRequest(ApiModel)`:
/// `value` is required, `op_uri` is an optional string, and `extra="forbid"` makes
/// an unknown key a 400. serde's default does the opposite on both counts -- a
/// misspelled field was silently dropped and a missing `value` was accepted here
/// and rejected further in with a different body.
///
/// The error body is this route's own: it calls
/// `api_error(err["error"], 400, code="validation_error")` and never forwards
/// pydantic's `detail`, so there is no `detail` key. The `ratings.rs` precedent
/// does send one; copying it would add a key Python does not.
fn parse_setting_value_request(raw: &Value) -> Result<SettingValueRequest, Response> {
    let mut errors: Vec<String> = Vec::new();
    if let Some(map) = raw.as_object() {
        for key in map.keys() {
            if key != "value" && key != "op_uri" {
                errors.push(format!("{key}: Extra inputs are not permitted"));
            }
        }
    } else {
        // Python's `require_json_dict` rejects a non-object body before the model.
    ...
    // `value: Any` with no default: absent is "Field required", but an explicit
    // null satisfies it (Any accepts None).
    let value = match raw.get("value") {
        None => {
            errors.push("value: Field required".to_string());
            None
        }
        Some(v) => Some(v.clone()),
    };

    // `op_uri: StrictStr | None` -- a non-string is a type error, null is allowed.
    let op_uri = match raw.get("op_uri") {
        None | Some(Value::Null) => None,
        Some(Value::String(s)) => Some(s.clone()),
        Some(_) => {
            errors.push("op_uri: Input should be a valid string".to_string());
            None
        }
    };
    ...
    Ok(SettingValueRequest { value, op_uri })
}
...
pub async fn api_settings_put(
    ...
    // Python looks the key up first (404 for an unknown one) and validates the body
    // after, so `update_setting`'s Ok(false) arm still has to come before the
    // parse errors -- but a bad body on a KNOWN key must report the pydantic shape.
    let body = match parse_setting_value_request(&raw) {
        Ok(parsed) => parsed,
        Err(response) => {
            // Only after confirming the key exists, to keep Python's ordering.
            return match setting_key_exists(&state, &key) {
                Ok(false) => api_error("Unknown setting key", StatusCode::NOT_FOUND, "not_found"),
                Ok(true) => response,
                Err(error) => internal_error(error, "failed to read the settings schema"),
            };
        }
    };
...
/// Does the generated schema know this key? Python answers 404 for an unknown key
/// BEFORE it validates the body, so a malformed body against a nonexistent key is a
/// 404 there, not a 400. `update_setting` already makes this check, but the parse
/// happens first now, so the ordering needs it on its own.
fn setting_key_exists(state: &SharedState, key: &str) -> Result<bool, std::io::Error> {
    let schema = load_schema_defs(&state.config.project_root)?;
    Ok(schema.iter().any(|setting| setting.key == key))
}
...
fn executable_on_path(program: &str) -> bool {
    if program.contains(std::path::MAIN_SEPARATOR) || program.contains('/') {
        return is_program_file(Path::new(program));
    }
    let on_path = std::env::var_os("PATH")
        .map(|paths| {
            std::env::split_paths(&paths).any(|path| search_dir_for_program(&path, program))
        })
        .unwrap_or(false);
    #[cfg(windows)]
    {
        return on_path || windows_app_alias_exists(program);
    }
    #[allow(unreachable_code)]
    on_path
}

fn search_dir_for_program(dir: &Path, program: &str) -> bool {
    if is_program_file(&dir.join(program)) {
        return true;
    }
    #[cfg(windows)]
    {
        return windows_path_extensions()
            .any(|ext| is_program_file(&dir.join(format!("{program}{ext}"))));
    }
    #[allow(unreachable_code)]
    false
}

/// App Execution Aliases (winget/Store installs such as 1Password's `op`) live in a fixed
/// per-user directory. `where op` and a %PATH% scan can both miss them, so look there
/// directly -- CreateProcess resolves the alias even when nothing on %PATH% does.
#[cfg(windows)]
fn windows_app_alias_exists(program: &str) -> bool {
    std::env::var_os("LOCALAPPDATA")
        .map(|local| {
            let dir = Path::new(&local).join("Microsoft").join("WindowsApps");
            search_dir_for_program(&dir, program)
        })
        .unwrap_or(false)
}
...
fn is_program_file(path: &Path) -> bool {
    ...
    #[cfg(windows)]
    {
        // App Execution Aliases are APPEXECLINK reparse points: metadata() cannot open
        // them, so is_file() is false even though CreateProcess resolves them fine.
        // symlink_metadata() reads the link itself and succeeds.
        return std::fs::symlink_metadata(path).is_ok_and(|meta| !meta.is_dir());
    }
    #[allow(unreachable_code)]
    false
}

/// Executable suffixes from %PATHEXT%, so `bw.cmd` (npm shim) and `op.exe` both resolve.
#[cfg(windows)]
...
mod tests {
    ...
    async fn single_unknown_key_returns_python_compatible_not_found() {
        ...
        assert_eq!(
            value,
            json!({"ok": false, "error": "Unknown setting key", "code": "not_found"})
        );
    }
    ...
    /// Python's `SettingValueRequest(ApiModel)` is `extra="forbid"`, so a misspelled
    /// field is a 400 there. serde ignored it here, which meant a client could send
    /// `valeu` and get a 200 with nothing written -- the silent wrong answer.
    #[tokio::test]
    async fn put_rejects_an_unknown_field_like_pythons_model() {
        let root = test_root("put-extra");
        let state = test_state(&root, json!({}), sample_schema()).await;
        let response = settings::api_settings_put(
            axum::extract::State(state),
            None,
            axum::extract::Path("feature.enabled".to_string()),
            Json(json!({"value": true, "valeu": false})),
        )
        .await;
        ...
        assert!(
            body["error"]
                .as_str()
                .unwrap_or("")
                .contains("valeu: Extra inputs are not permitted"),
            "body: {body}"
        );
        // This route drops pydantic's `detail`; sending one would be a difference of
        // its own.
        assert!(body.get("detail").is_none(), "body: {body}");
    }

    /// `value: Any` carries no default, so an absent `value` is
    /// "value: Field required" with code `validation_error` -- not the
    /// `bad_request` the deeper write path used to report.
    #[tokio::test]
    async fn put_reports_a_missing_value_as_pydantic_does() {
        let root = test_root("put-novalue");
        let state = test_state(&root, json!({}), sample_schema()).await;
        let response = settings::api_settings_put(
            axum::extract::State(state),
            None,
        ...
        let body = json_body(response).await;
        assert_eq!(body["code"], "validation_error", "body: {body}");
        assert_eq!(body["error"], "value: Field required");
    }

    /// Python looks the key up first: a malformed body against a key the schema does
    /// not know is a 404, not a 400.
    #[tokio::test]
    async fn put_reports_an_unknown_key_before_the_body_errors() {
        let root = test_root("put-unknownkey");
        let state = test_state(&root, json!({}), sample_schema()).await;
        let response = settings::api_settings_put(
            axum::extract::State(state),
            None,
            axum::extract::Path("no.such.key".to_string()),
            Json(json!({"typo": 1})),
        )
        .await;
    ...
    async fn put_encrypts_secret_before_persisting() {
        ...
        let response = settings::api_settings_put(
            ...
            Json(json!({"value": json!("super-secret-pin")})),
        )
        .await;
        let value = json_body(response).await;
        let config: Value =
    ...
    async fn put_op_uri_updates_mapping_without_plaintext_secret() {
        ...
        let response = settings::api_settings_put(
            ...
            None,
            axum::extract::Path("server.pin".to_string()),
            Json(json!({"value": json!(""), "op_uri": "op://Personal/Yu/pin".to_string()})),
        )
        .await;
        let value = json_body(response).await;
        let config: Value =
    ...
    #[test]
    fn executable_on_path_accepts_explicit_paths_and_rejects_dirs() {
        let manifest = std::path::Path::new(env!("CARGO_MANIFEST_DIR"));
        let file = manifest.join("Cargo.toml");
        assert!(super::executable_on_path(file.to_str().unwrap()));
        assert!(!super::executable_on_path(manifest.to_str().unwrap()));
        assert!(!super::executable_on_path(
            manifest.join("no-such-binary").to_str().unwrap()
        ));
    }
    ...
    async fn delete_op_mapping_returns_not_found_for_missing_key() {
        ...
        assert_eq!(
            value,
            json!({"ok": false, "error": "Key not in op_secrets mapping", "code": "not_found"})
        );
    }
