...
/// Python's `api_result` always emits `error` and `data` beside `ok`; this omitted
/// both, so a client testing `"error" in body` saw a different contract per route.
fn api_ok(payload: Value) -> Json<Value> {
    super::api_envelope::api_ok(payload)
}
