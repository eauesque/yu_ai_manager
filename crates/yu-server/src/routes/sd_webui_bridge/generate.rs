...
async fn generate_standard(state: SharedState, body: Value) -> Response {
    ...
    // Python saves when EITHER flag is on (`auto_save_images`: `not auto_save and
    // not force_save` -> skip, where force_save is bridge_managed_save). Reading
    // only bridge_managed_save here made a ticked "auto save" box do nothing.
        ...
        .unwrap_or(false)
    {
        for key in ["prompt", "negative_prompt"] {
            let rewritten = body
                ...
                .map(crate::sd_nai::convert_artist_to_at);
            if let (Some(object), Some(text)) = (body.as_object_mut(), rewritten) {
    ...
    match sd_result {
        ...
        Ok(resp) => {
            ...
            let (saved_paths, save_errors): (Vec<String>, Vec<String>) =
                if crate::routes::bridge_save::should_save_to_disk(
                    auto_save,
                    bridge_managed_save,
                    &save_folder,
                ) {
