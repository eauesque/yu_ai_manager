...
use super::{
    clip_search::{call_clip_image, ImageBackend},
...
pub async fn start_handler(
    ...
    tokio::spawn(async move {
        run_worker(state_for_task, request.batch_size, request.backend).await;
    });
...
fn parse_start_request(
    ...
    let backend = match object.get("backend") {
        None => ImageBackend::Auto,
        Some(Value::String(value)) if value == "auto" => ImageBackend::Auto,
        Some(Value::String(value)) if matches!(value.as_str(), "hailo" | "hailo-10h") => {
            ImageBackend::Hailo
        }
        Some(Value::String(value)) if value == "onnx" => ImageBackend::Onnx,
        Some(Value::String(_)) => {
            return Err(bad_request("backend must be auto, hailo, or onnx"));
        }
...
async fn run_worker(state: SharedState, batch_size: u16, backend: ImageBackend) {
    ...
    loop {
        ...
        for file_id in file_ids {
            ...
            match call_clip_image(&state, std::path::Path::new(path), backend).await {
                Ok(vector) => {
                    ids.push(file_id);
                    vectors.push(vector);
                }
...
mod tests {
    ...
    fn start_body_validation_is_strict() {
        ...
        assert!(parse_start_request(Ok(Json(json!({"backend": "onnx"})))).is_ok());
    }

    #[test]
    fn backend_selection_honors_explicit_requests() {
        use super::super::clip_search::select_image_backend;

        assert_eq!(
            select_image_backend(ImageBackend::Auto, true, true),
            Some(ImageBackend::Hailo)
        );
        assert_eq!(
            select_image_backend(ImageBackend::Auto, false, true),
            Some(ImageBackend::Onnx)
        );
        assert_eq!(select_image_backend(ImageBackend::Hailo, false, true), None);
        assert_eq!(
            select_image_backend(ImageBackend::Onnx, true, true),
            Some(ImageBackend::Onnx)
        );
    }
    #[test]
