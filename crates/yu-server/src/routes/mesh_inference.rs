...
fn python_action_repr(action: Option<&str>) -> String {
    let Some(action) = action else {
        return "None".to_string();
    };
    // ponytail: matches Python repr for the ASCII action protocol; expand to
    // Python's full Unicode-printability rules if arbitrary action text is supported.
    let quote = if action.contains('\'') && !action.contains('"') {
        '"'
    } else {
        '\''
    };
    ...
    for character in action.chars() {
        match character {
            ...
            character if character.is_ascii_control() => {
                use std::fmt::Write;
                let _ = write!(repr, "\\x{:02x}", character as u32);
            }
...
/// Python's `api_result`: the payload is spread at the TOP level, not nested.
///
/// This nested it under `data`, so a client reading `json.peers` got nothing from Rust
/// while Python answered normally. Measured against Python at 200/200 in v4.734.24 --
/// the differing keys were exactly `data` and `peers`.
fn result(data: Value) -> Response {
    super::api_envelope::api_ok(data).into_response()
}
...
pub async fn mesh_bulk(
    ...
    if !matches!(
        action,
        Some("disable_all_remote" | "enable_all" | "local_only")
    ) {
        let message = format!("unknown action: {}", python_action_repr(action));
        return error(StatusCode::BAD_REQUEST, &message, "unknown_action");
    }
    if debounced(format!("{}:{}", action.unwrap_or(""), kind.unwrap_or(""))) {
        return error(
    ...
    if matches!(action, Some("disable_all_remote" | "enable_all"))
...
mod tests {
    ...
    async fn state_sorts_disabled_types_and_degrades_malformed_advertisement() {
        ...
        // Python puts the payload at the top level; `data` stays null.
    ...
    #[tokio::test]
    async fn bulk_unknown_action_preserves_python_repr() {
        let state = test_state().await;
        let response = mesh_bulk(State(state), None, Ok(Json(json!({"action": "foo"})))).await;
    ...
    async fn bulk_actions_preserve_offline_disablement() {
        ...
        assert_eq!(
            local_only,
            json!({"ok": false, "error": "local peer has no effective inference types", "code": "local_peer_has_no_effective_types"})
        );
    }
