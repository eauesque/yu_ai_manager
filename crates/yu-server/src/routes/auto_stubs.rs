...
///
/// The `killed` half was an invented `false`: the Kill Switch is real in Rust
/// (`routes::agent` writes and removes `data/agent_kill.flag`, the same file
/// Python's `kill_switch.is_killed()` polls), and this handler simply never
/// read it. So an operator who had killed the agent -- from this server, from
/// the Python MCP subprocess, or from a previous run -- was told by the nav
/// button and the journal banner that nothing was stopped. That is the one
/// direction a safety control must never be wrong in.
///
/// The circuit breaker and the budget have no Rust implementation at all
/// (`routes::agent_audit::circuit_breaker` and `::budget` are stubs of the
/// same kind). They report `unknown` rather than `closed`/`{}`: "the breaker
/// is closed" and "no budget is being consumed" are claims about state nobody
/// measured, and both consumers (`src/ts/nav/agent-kill.ts`,
/// `src/ts/apps/agent-journal-data.ts`) already treat any state other than
/// `open`/`half_open` as "nothing to show".
pub async fn agent_status(
    ...
    let kill_switch = crate::routes::agent::kill_switch_status(&state.config.project_root);
    let killed = kill_switch
        .get("killed")
        .and_then(serde_json::Value::as_bool)
        .unwrap_or(false);
    // Python derives both from kill_switch (routes/agent_api_core.py:111-112) and
    // exposes them at the top level for backward compatibility. Rust had the map all
    // along and simply did not publish them.
    let reason = kill_switch
        .get("reason")
        .and_then(serde_json::Value::as_str)
        .unwrap_or("")
        .to_string();
    let killed_at = kill_switch
        .get("killed_at")
        .and_then(serde_json::Value::as_str)
        .unwrap_or("")
        .to_string();
    Json(json!({
        // Python wraps with api_result, so the envelope sits beside the payload.
        "ok": true,
        "error": null,
        "data": null,
        "kill_switch": kill_switch,
        // ponytail: no breaker/budget in Rust; report unknown, not "closed"/"{}".
        // Upgrade path is a real port of agent_audit's two stubs, not a wider
        // default here. Python answers `{}` for both; matching that literal would
        // assert "no budget is being consumed", which nothing here measured.
        "circuit_breaker": {"enabled": null, "state": "unknown"},
        "budget": null,
        "processes": null,
        "killed": killed,
        "reason": reason,
...
pub async fn apikeys_list(
    ...
    Json(json!({
        ...
        "error": null,
        "data": null,
        "keys": list_apikeys_from_config(&config),
    }))
    .into_response()
}
...
// `GET /api/ocr/npu` used to return a 501 from here. It is implemented in
// `routes::ocr_npu` now: the Rust server reports the Hailo device node it can
// actually observe, rather than mirroring Python's check for an importable
// Python package. See that module for why the two answers legitimately differ.

// The backup subsystem's `create`, `restore` and `delete` answer 503 here --
// they need sqlite's online backup API, retention and a pre-restore snapshot,
// all of which live in Python (`extensions/builtin_backup`).
//
// `backup_list` and `backup_status` used to sit here too, answering 200 with
// invented state: the listing said there were no backups at all, and the
// status reported `enabled: false` and `backup_on_scan_complete: false` where
// Python's defaults are BOTH `True` (and a 60 minute cooldown where Python's
// default is 5). They are now real, in `routes::backup_read`, reading the
// backup directory and the config.

/// Read `gateway.backends` as Python's public backend responses.
///
/// This deliberately allowlists public fields: gateway config also stores
/// credentials for internal integrations.
fn python_url_component(value: &str) -> &str {
    value
        .find(['/', '?', '#'])
...
fn python_url_parts(url: &str) -> (Option<&str>, &str) {
    let (_scheme, rest) = url
        .find(':')
        .and_then(|i| Some((url.get(..i)?, url.get(i.checked_add(1)?..)?)))
        .filter(|(scheme, _)| {
            !scheme.is_empty()
    ...
    if let Some(rest) = rest.strip_prefix("//") {
        (Some(python_url_component(rest)), "")
    } else {
        (None, python_url_component(rest))
    }
}

fn python_port(url: &str) -> Option<u16> {
    let (authority, _) = python_url_parts(url);
...
fn backends_from_section(section: &serde_json::Value) -> Vec<serde_json::Value> {
    section
        ...
        .filter_map(|(id, value)| {
            ...
            let raw_base_url = entry.get("base_url")?.as_str()?;
            let backend_type = entry.get("type")?.as_str()?;
            if raw_base_url.is_empty() || backend_type.is_empty() {
                return None;
            }
            let base_url = raw_base_url.trim_end_matches('/');
            let (netloc, path) = python_url_parts(base_url);
            let name = entry
                .get("name")
                .and_then(serde_json::Value::as_str)
                .filter(|name| !name.is_empty())
                .map(str::to_owned)
                .unwrap_or_else(|| format!("{backend_type}:{}", netloc.unwrap_or(path)));
            let color = entry
                .get("color")
                .and_then(serde_json::Value::as_str)
                .filter(|color| !color.is_empty())
                .unwrap_or(match backend_type {
                    "comfyui" => "#4a9eff",
                    "sd_webui" => "#ff7a4a",
                    "gradio" => "#f97316",
                    _ => "#888888",
...
pub async fn gateway_local_status(
    ...
    let backends: Vec<serde_json::Value> = configured_backends(&state)
        ...
        .into_iter()
        .filter_map(|entry| {
            let backend_type = entry.get("type")?.clone();
            Some(json!({
...
///
/// The body is Python's, measured by probing `register_error_handlers`: for an
/// /api/ path `handle_http_error` answers
/// `api_error(e.description, status=405, code="http_405")`, and Werkzeug's
/// description is the sentence below. Rust's own wording, with no `code`, was a
/// body difference the moment the comparison was turned on.
...
fn profiles_dir(state: &SharedState) -> std::path::PathBuf {
    // One resolver: the profile CRUD routes here and the startup/read-side
    // merge in `ext_config` must agree on where profiles live, or a profile
    // created through this API is one the server never finds.
    crate::ext_config::profiles_dir(&state.config.project_root)
}
...
// ---------------------------------------------------------------------------
// Profiles: the response envelope, the error codes and the request contract all
// mirror `core/profiles_api/`. See this module's parity notes -- the harness
// suppressed body comparison here on the false premise that Python had no
// implementation, so these two sets of handlers drifted apart unobserved.
// ---------------------------------------------------------------------------

/// Mirror of `core/infra_core/api_errors.py::api_success`: the payload plus the
/// `ok`/`error`/`data` keys the frontend reads. A bare `{"ok": true}` is not the
/// same body, which is why every profile response used to differ from Python's.
fn profile_ok(payload: serde_json::Value, status: StatusCode) -> Response {
    // Every remaining Value variant is named rather than caught by `_`: a
    // catch-all would also swallow a variant serde_json might add later, and the
    // non-object case has a real behaviour (it becomes `data`).
...
/// Mirror of `core/infra_core/api_errors.py::api_error`: `code` is part of the
/// contract, and the message is the human-readable text, not the code again.
fn profile_err(message: &str, status: StatusCode, code: &str) -> Response {
    (
        status,
...
/// Mirror of `core/infra_core/api_request.py::validate_json_model`'s error shape
/// for an ApiModel (`extra="forbid"`), down to the `detail` count.
fn profile_validation_err(messages: &[String]) -> Response {
    (
...
/// `extra="forbid"`: pydantic reports every unexpected key, not just the first.
fn profile_extra_keys(value: &serde_json::Value, known: &[&str]) -> Vec<String> {
    let Some(map) = value.as_object() else {
        return Vec::new();
    };
    map.keys()
        .filter(|k| !known.contains(&k.as_str()))
        .map(|k| format!("{k}: Extra inputs are not permitted"))
        .collect()
}

/// `StrictStr` field: absent is "Field required", a non-string is a type error.
fn profile_str_field(
    ...
    match value.get(field) {
        ...
        Some(_) => {
            errors.push(format!("{field}: Input should be a valid string"));
            None
        }
    }
}
...
pub struct CreateProfileBody {
    name: String,
    /// Optional in Python: `label = (data.label or name).strip()`.
    label: Option<String>,
    description: String,
    /// Python's field name. Rust called this `config`, so every `base_config`
    /// a client sent was dropped without a word.
    base_config: Option<serde_json::Value>,
}

pub struct UpdateProfileBody {
    /// Python keys off `model_fields_set`, so "absent" and "explicitly null"
    /// are different requests. `present_*` preserves that distinction.
    label: Option<String>,
...
pub struct DuplicateProfileBody {
    new_name: String,
    /// Optional in Python: `new_label = (data.new_label or new_name).strip()`.
...
/// `ProfileCreateRequest` in `core/profiles_api/request_models.py`.
fn parse_create_body(raw: &serde_json::Value) -> Result<CreateProfileBody, Response> {
    let mut errors = profile_extra_keys(raw, &["name", "label", "description", "base_config"]);
    let name = profile_str_field(raw, "name", true, &mut errors);
    let label = profile_str_field(raw, "label", false, &mut errors);
    let description = profile_str_field(raw, "description", false, &mut errors);
    // `Field(min_length=1)` runs before the field_validator that strips.
    if let Some(n) = name.as_deref() {
        if n.is_empty() {
            errors.push("name: String should have at least 1 character".to_string());
        } else if n.trim().is_empty() {
            errors.push("name: Value error, name must not be empty".to_string());
        }
    }
    let base_config = match raw.get("base_config") {
        None | Some(serde_json::Value::Null) => None,
        Some(v @ serde_json::Value::Object(_)) => Some(v.clone()),
        Some(_) => {
            errors.push("base_config: Input should be a valid dictionary".to_string());
            None
        }
    };
    if !errors.is_empty() {
        return Err(profile_validation_err(&errors));
    }
    Ok(CreateProfileBody {
...
/// `ProfileUpdateRequest`. Python's `favorite` validator rejects an explicit
/// null, so `{"favorite": null}` is a 400 there and must be one here too.
fn parse_update_body(raw: &serde_json::Value) -> Result<UpdateProfileBody, Response> {
    let mut errors = profile_extra_keys(raw, &["label", "description", "favorite"]);
    let label = profile_str_field(raw, "label", false, &mut errors);
    let description = profile_str_field(raw, "description", false, &mut errors);
    let favorite = match raw.get("favorite") {
        None => None,
        Some(serde_json::Value::Null) => {
            errors.push("favorite: Value error, favorite must be a boolean".to_string());
            None
        }
        Some(serde_json::Value::Bool(b)) => Some(*b),
        Some(_) => {
            errors.push("favorite: Input should be a valid boolean".to_string());
            None
        }
    };
    if !errors.is_empty() {
    ...
    for key in ["label", "description", "favorite"] {
        if raw.get(key).is_some() {
            present.push(key);
        }
    }
    Ok(UpdateProfileBody {
        label,
        description,
        favorite,
        present,
...
fn parse_duplicate_body(raw: &serde_json::Value) -> Result<DuplicateProfileBody, Response> {
    let mut errors = profile_extra_keys(raw, &["new_name", "new_label"]);
    let new_name = profile_str_field(raw, "new_name", true, &mut errors);
    let new_label = profile_str_field(raw, "new_label", false, &mut errors);
    if let Some(n) = new_name.as_deref() {
        if n.is_empty() {
            errors.push("new_name: String should have at least 1 character".to_string());
        } else if n.trim().is_empty() {
            errors.push("new_name: Value error, new_name must not be empty".to_string());
        }
    }
    if !errors.is_empty() {
        return Err(profile_validation_err(&errors));
    }
    Ok(DuplicateProfileBody {
        new_name: new_name.unwrap_or_default().trim().to_string(),
        new_label,
    })
}

/// `ProfileRenameRequest`.
fn parse_rename_body(raw: &serde_json::Value) -> Result<RenameProfileBody, Response> {
    let mut errors = profile_extra_keys(raw, &["new_name"]);
    let new_name = profile_str_field(raw, "new_name", true, &mut errors);
    if let Some(n) = new_name.as_deref() {
        if n.is_empty() {
            errors.push("new_name: String should have at least 1 character".to_string());
    ...
    Ok(RenameProfileBody {
        new_name: new_name.unwrap_or_default().trim().to_string(),
    })
}

/// `core/configuration/profiles.py::validate_profile_name` returns the message,
/// and the route turns it into `code: "invalid_profile_name"`.
fn profile_name_error(name: &str) -> Option<&'static str> {
    if name.is_empty() {
        return Some("Profile name must not be empty");
    }
    if !valid_profile_name(name) {
        return Some("Profile name must be 1-64 chars of [a-zA-Z0-9_-]");
    }
    None
}

fn invalid_profile_name(name: &str) -> Option<Response> {
    profile_name_error(name)
        .map(|m| profile_err(m, StatusCode::BAD_REQUEST, "invalid_profile_name"))
}

fn profile_not_found(name: &str) -> Response {
    profile_err(
...
/// GET /api/profiles
pub async fn profiles_list(State(s): State<SharedState>) -> Response {
    let active = s.config.active_profile.clone();
        ...
        for entry in entries.flatten() {
            ...
            if let Ok(text) = std::fs::read_to_string(&p) {
                if let Ok(v) = serde_json::from_str::<serde_json::Value>(&text) {
                    ...
                    profiles.push(json!({
                        "name": profile_name,
                        "is_active": Some(profile_name.as_str()) == active.as_deref(),
    ...
    profile_ok(json!({"profiles": profiles}), StatusCode::OK)
}

/// GET /api/profiles/{name}
pub async fn profiles_get(State(s): State<SharedState>, Path(name): Path<String>) -> Response {
    if let Some(resp) = invalid_profile_name(&name) {
        return resp;
    }
    match read_profile_file(&profiles_dir(&s), &name) {
        Some(mut v) => {
            // Python returns the profile under `profile` and marks `is_active`;
            // Rust used to return the bare stored object.
            let is_active = Some(name.as_str()) == s.config.active_profile.as_deref();
            if let Some(obj) = v.as_object_mut() {
                obj.insert("is_active".to_string(), json!(is_active));
            }
            profile_ok(json!({"profile": v}), StatusCode::OK)
        }
        None => profile_not_found(&name),
    }
}
...
pub async fn profiles_create(
    ...
    let body = match parse_create_body(&raw) {
        Ok(b) => b,
        Err(resp) => return resp,
    };
    if let Some(resp) = invalid_profile_name(&body.name) {
        return resp;
    }
    // `label = (data.label or name).strip()` -- absent or empty falls back to
    // the name, and only whitespace-only text is rejected.
    let label = match body.label.as_deref() {
        None | Some("") => body.name.clone(),
    ...
    if path.exists() {
        return profile_err(
            "Profile already exists or could not be created",
            StatusCode::CONFLICT,
            "profile_exists",
        );
    }
    ...
    if let (Some(obj), Some(cfg)) = (
        data.as_object_mut(),
        body.base_config.as_ref().and_then(|c| c.as_object()),
    ) {
        for (k, v) in cfg {
    ...
    match write_profile_atomic(&path, &data) {
        // Python answers 201 Created.
        Ok(()) => profile_ok(json!({"profile": data}), StatusCode::CREATED),
        Err(e) => {
...
pub async fn profiles_update(
    ...
    let body = match parse_update_body(&raw) {
        Ok(b) => b,
        Err(resp) => return resp,
    };
    // Python checks the request before it looks for the profile: an empty body
    // is 400 `empty_update`, never 404. Rust used to look first and answer 404,
    // so the same request got two different codes.
    if body.present.is_empty() {
        return profile_err(
            "No fields to update",
            StatusCode::BAD_REQUEST,
            "empty_update",
    ...
    let Some(mut data) = read_profile_file(&dir, &name) else {
        return profile_not_found(&name);
    };
    // Python writes `(value or "").strip()` for the two text fields.
    if body.present.contains(&"label") {
        data["label"] = json!(body.label.as_deref().unwrap_or("").trim());
    }
    if body.present.contains(&"description") {
        data["description"] = json!(body.description.as_deref().unwrap_or("").trim());
    }
    ...
    match write_profile_atomic(&path, &data) {
        Ok(()) => profile_ok(json!({"profile": data}), StatusCode::OK),
        Err(e) => {
...
pub async fn profiles_delete(State(s): State<SharedState>, Path(name): Path<String>) -> Response {
    ...
    // Python refuses to delete the profile currently in use.
    if Some(name.as_str()) == s.config.active_profile.as_deref() {
        return profile_err(
    ...
    match std::fs::remove_file(&path) {
        Ok(()) => profile_ok(json!({"deleted": name}), StatusCode::OK),
        Err(e) => {
...
pub async fn profiles_duplicate(
    ...
    let body = match parse_duplicate_body(&raw) {
        Ok(b) => b,
        Err(resp) => return resp,
    };
    if let Some(resp) = invalid_profile_name(&name).or_else(|| invalid_profile_name(&body.new_name))
    {
        return resp;
    }
    let dir = profiles_dir(&s);
    ...
    if new_path.exists() {
        return profile_err(
            &format!("Profile '{}' already exists", body.new_name),
            StatusCode::CONFLICT,
            "duplicate_failed",
        );
    }
    // `new_label = (data.new_label or new_name).strip()`.
    let new_label = match body.new_label.as_deref() {
        None | Some("") => body.new_name.clone(),
        Some(text) => text.trim().to_string(),
    };
    data["name"] = json!(body.new_name);
    data["label"] = json!(new_label);
...
pub async fn profiles_rename(
    ...
    let body = match parse_rename_body(&raw) {
        Ok(b) => b,
        Err(resp) => return resp,
    };
    if let Some(resp) = invalid_profile_name(&name).or_else(|| invalid_profile_name(&body.new_name))
    ...
    match write_profile_atomic(&new_path, &data) {
        Ok(()) => {
            ...
            profile_ok(json!({"profile": data}), StatusCode::OK)
        }
        Err(e) => {
...
pub async fn profiles_favorite(State(s): State<SharedState>, Path(name): Path<String>) -> Response {
    ...
    let path = dir.join(format!("{name}.json"));
    match write_profile_atomic(&path, &data) {
        // Python returns the whole profile, not just the new flag.
        Ok(()) => profile_ok(json!({"profile": data}), StatusCode::OK),
        Err(e) => {
...
pub async fn profiles_export(State(s): State<SharedState>, Path(name): Path<String>) -> Response {
    ...
    let Some(data) = read_profile_file(&profiles_dir(&s), &name) else {
        // `core/profiles_api/routes_qr.py` says "Profile not found" here -- a
        // shorter message than the CRUD routes use.
        return profile_err(
            "Profile not found",
            StatusCode::NOT_FOUND,
            "profile_not_found",
    ...
    let clean: serde_json::Map<String, serde_json::Value> = data
        ...
        .unwrap_or_default();
    // `routes_qr.py`: api_success({"qr_data": json.dumps({"schema": ..., "profile": ...})}).
    // The payload travels as a JSON *string*, not as a nested object.
    let qr_payload = json!({
        "schema": "yu://profile/1",
        "profile": serde_json::Value::Object(clean),
    });
    match serde_json::to_string(&qr_payload) {
        Ok(encoded) => profile_ok(json!({"qr_data": encoded}), StatusCode::OK),
        Err(e) => {
            tracing::error!("profiles_export: {e}");
            profile_err(
                "Profile not found",
                StatusCode::INTERNAL_SERVER_ERROR,
                "profile_not_found",
...
// The chatlog import/reprocess half has no Rust implementation. The reads
// (`routes::chatlog`: search, stats, conversations) are real, so the page is
// live and an operator reaches these five for real.
//
// They used to answer 200. The two writes said `{"ok": true}` -- one of them
// `"imported": 0` -- for work that never started, so the page reported a
// successful import of nothing; the two status reads said `running: false`,
// which the poller reads as "the import finished". A write that reports
// success it did not perform is the worst shape in this file: the operator has
// no way left to find out. All five now answer 503 like their siblings, which
// the chatlog page already renders (`data.error` -> "Error: unavailable", and
// the poller stops and re-enables the button).
...
// `fleet_peers`, `fleet_peer_allowlist_status`, `fleet_peer_grant` and
// `fleet_peer_revoke` used to sit here, answering `{"peers": []}` and
// `{"enabled": false}` -- "this node is paired with nobody" and "the allowlist
// is off", neither of which anything had looked up. Nothing mounted them: the
// live routes are `routes::lan_cowork_fleet`, which answer 503 before flag-day
// rather than inventing a fleet. Deleted rather than corrected, so a future
// `.route(...)` cannot reach for the invented pair by name.
//
// The `/// GET /api/tools/archive-cleanup/list-models` doc line that used to
// stand here went with them: it documented no function even before the
// deletion (it sat above `fleet_peers`, which is a different route entirely),
// and there is no list-models handler in this file to re-attach it to.
...
///
/// Was a bare `[]`. An empty list is not the neutral answer here -- it is the
/// stronger claim that the dataset has no checkpoints, which is exactly the
/// reading `routes::gateway_status::node_services` warns about. Rust indexes no
/// checkpoints at all, so say so.
...
mod standalone_tests {
    ...
    async fn fwd_post_returns_503_in_standalone_mode() {
        // The body, not just the status. Its GET twin above pins
        // `code == "python_unavailable"`; this one pinned the status alone, so
        // the POST envelope could drift away from Python's while staying green
        // -- and the envelope is the whole contract of a forwarding stub.
        let state = make_standalone_state();
...
mod hailo_genai_vlm_tests {
    ...
    fn model_name_to_hef_path_resolves_known_registry_name_to_real_filename() {
        ...
        // PathBuf equality (not string equality): PathBuf::join always
        // inserts the platform's native separator, so on Windows this
        // literal Unix-styled HOME yields a mixed `/`-then-`\` string that
        // is a different *string* from, but the same *path* as, the
        // expected literal below -- Windows treats `/` and `\` as
        // equivalent separators when parsing path components.
        assert_eq!(
            std::path::PathBuf::from(model_name_to_hef_path("qwen2-vl-2b-instruct")),
            std::path::PathBuf::from("/home/pi/hailo_models/Qwen2-VL-2B-Instruct.hef")
        );
        assert_eq!(
            std::path::PathBuf::from(model_name_to_hef_path("qwen3-1.7b-instruct")),
            std::path::PathBuf::from("/home/pi/hailo_models/Qwen3-1.7B-Instruct.hef")
        );
    }
    ...
    fn model_name_to_hef_path_falls_back_to_naive_convention_for_unknown_name() {
        ...
        std::env::remove_var("HAILO_HEF_DIR");
        assert_eq!(
            std::path::PathBuf::from(model_name_to_hef_path("Llama3.2-1B-Instruct")),
            std::path::PathBuf::from("/home/pi/hailo_models/Llama3.2-1B-Instruct.hef")
        );
    }
    ...
    fn model_name_to_hef_path_honors_hailo_hef_dir_override() {
        ...
        assert_eq!(
            ...
            std::path::PathBuf::from("/tmp/custom_hailo_hefs/Llama3.2-1B-Instruct.hef")
        );
    }
...
mod profiles_tests {
    ...
    /// Python's /api/ error handler answers a 405 with Werkzeug's own description
    /// and `code: "http_405"` (measured by probing `register_error_handlers`).
    /// Rust's own wording, with no `code`, was a body difference the moment the
    /// comparison was turned on -- v4.732.24 measured it as `code, error`.
    #[tokio::test]
    async fn gateway_backends_patch_405_matches_pythons_body() {
        let body = body_json(gateway_backends_patch().await.into_response()).await;
        assert_eq!(body["ok"], false);
        assert_eq!(body["code"], "http_405");
        assert_eq!(
            body["error"], "The method is not allowed for the requested URL.",
            "the wording is Python's, not ours: {body}"
        );
    }
    ...
    fn create_body(name: &str, label: &str) -> serde_json::Value {
    ...
    async fn profiles_crud_roundtrip() {
        ...
        // Python answers 201 Created here.
        ...
        assert_eq!(body_json(res).await["profile"]["label"], "Alpha");

        let res = profiles_update(
            State(state.clone()),
            Path("alpha".to_string()),
            Json(json!({"label": "Renamed label"})),
        )
        .await;
        assert_eq!(res.status(), StatusCode::OK);

        let res = profiles_favorite(State(state.clone()), Path("alpha".to_string())).await;
        assert_eq!(body_json(res).await["profile"]["favorite"], true);
        let res = profiles_favorite(State(state.clone()), Path("alpha".to_string())).await;
        assert_eq!(body_json(res).await["profile"]["favorite"], false);
        ...
        let res = profiles_duplicate(
            State(state.clone()),
            Path("beta".to_string()),
            Json(json!({"new_name": "gamma", "new_label": "Gamma"})),
        )
        .await;
        assert_eq!(res.status(), StatusCode::CREATED);
    ...
    /// Every profile response must carry Python's `api_success` envelope. A bare
    /// `{"ok": true, ...}` is a different body, and `profiles_list` used to omit
    /// `ok` entirely -- neither was ever noticed because the harness suppressed
    /// body comparison behind a false "Rust native" note.
    #[tokio::test]
    async fn profiles_responses_carry_the_python_success_envelope() {
        let root = TempDir::new().unwrap();
        let state = test_state(&root).await;

        let created = body_json(
            profiles_create(State(state.clone()), Json(create_body("alpha", "Alpha"))).await,
        )
        .await;
        for key in ["ok", "error", "data"] {
            assert!(
                created.get(key).is_some(),
                "create response is missing {key}: {created}"
            );
        }
        assert_eq!(created["ok"], true);
        assert_eq!(created["error"], serde_json::Value::Null);

        let listed = body_json(profiles_list(State(state.clone())).await).await;
        for key in ["ok", "error", "data"] {
            assert!(
                listed.get(key).is_some(),
                "list response is missing {key}: {listed}"
            );
        }

        let got =
            body_json(profiles_get(State(state.clone()), Path("alpha".to_string())).await).await;
        assert_eq!(got["ok"], true);
        assert_eq!(got["profile"]["name"], "alpha");
    }

    /// `base_config` is Python's field name. Rust called it `config`, so a client
    /// that sent `base_config` had it dropped without an error, and a client that
    /// sent `config` got a 400 from Python.
    #[tokio::test]
    async fn profiles_create_reads_base_config_and_rejects_the_old_name() {
        let root = TempDir::new().unwrap();
        let state = test_state(&root).await;

        let res = profiles_create(
            State(state.clone()),
            Json(json!({"name": "withcfg", "label": "L", "base_config": {"harmless": "keep"}})),
        )
        .await;
        assert_eq!(res.status(), StatusCode::CREATED);
        assert_eq!(body_json(res).await["profile"]["harmless"], "keep");

        // `config` is now an unexpected key, exactly as it is for Python.
        let res = profiles_create(
            State(state.clone()),
            Json(json!({"name": "oldname", "label": "L", "config": {"harmless": "keep"}})),
        )
        .await;
        ...
        assert!(
            body["error"]
                .as_str()
                .unwrap()
                .contains("config: Extra inputs are not permitted"),
            "unexpected error text: {body}"
        );
        assert_eq!(body["detail"], "1 validation error(s)");
    }

    /// Python's `label` is optional and falls back to the profile name. Rust made
    /// it required, so a request that Python accepts was rejected outright by
    /// serde before any handler ran.
    #[tokio::test]
    async fn profiles_create_label_is_optional_and_defaults_to_the_name() {
        let root = TempDir::new().unwrap();
        let state = test_state(&root).await;

        let res = profiles_create(State(state.clone()), Json(json!({"name": "nolabel"}))).await;
        assert_eq!(res.status(), StatusCode::CREATED);
        assert_eq!(body_json(res).await["profile"]["label"], "nolabel");

        // Whitespace-only is the one case Python rejects.
        let res = profiles_create(
            State(state.clone()),
            Json(json!({"name": "blanklabel", "label": "   "})),
        )
        .await;
        assert_eq!(res.status(), StatusCode::BAD_REQUEST);
        assert_eq!(body_json(res).await["code"], "invalid_label");
    }

    /// Python checks the request before it looks for the profile, so an empty
    /// update body is 400 `empty_update` whether or not the profile exists. Rust
    /// looked first and answered 404, which gave the same request two codes.
    #[tokio::test]
    async fn profiles_update_rejects_an_empty_body_before_looking_for_the_profile() {
        let root = TempDir::new().unwrap();
        let state = test_state(&root).await;

        for name in ["absent", "present"] {
            if name == "present" {
                profiles_create(State(state.clone()), Json(create_body(name, "L"))).await;
            }
            let res = profiles_update(
                State(state.clone()),
                Path(name.to_string()),
            ...
            assert_eq!(
                res.status(),
                StatusCode::BAD_REQUEST,
                "empty update on {name} should be 400 empty_update"
            );
            assert_eq!(body_json(res).await["code"], "empty_update");
        }
    }

    /// `extra="forbid"` on every profile model, and Python's `favorite` validator
    /// rejects an explicit null rather than treating it as "not set".
    #[tokio::test]
    async fn profiles_reject_unknown_fields_and_explicit_null_favorite() {
        let root = TempDir::new().unwrap();
        let state = test_state(&root).await;
        profiles_create(State(state.clone()), Json(create_body("alpha", "Alpha"))).await;
        ...
        let res = profiles_update(
            State(state.clone()),
            Path("alpha".to_string()),
            Json(json!({"label": "x", "typo_field": 1})),
        )
        .await;
        assert_eq!(res.status(), StatusCode::BAD_REQUEST);
        ...
        let res = profiles_rename(
            State(state.clone()),
            Path("alpha".to_string()),
            Json(json!({"new_name": "beta", "extra": true})),
        )
        .await;
        assert_eq!(res.status(), StatusCode::BAD_REQUEST);
        assert!(
            !root.path().join("profiles/beta.json").exists(),
            "a rejected rename must not have written anything"
        );
    }

    /// Error bodies are part of the contract: Python sends a human-readable
    /// message plus a `code`, where Rust used to send the code as the message and
    /// no `code` key at all.
    #[tokio::test]
    async fn profiles_errors_carry_pythons_message_and_code() {
        let root = TempDir::new().unwrap();
        let state = test_state(&root).await;

        let body =
            body_json(profiles_get(State(state.clone()), Path("missing".to_string())).await).await;
        assert_eq!(body["code"], "profile_not_found");
        assert_eq!(body["error"], "Profile 'missing' not found");

        let body =
            body_json(profiles_get(State(state.clone()), Path("bad/name".to_string())).await).await;
        assert_eq!(body["code"], "invalid_profile_name");
        assert_eq!(
            body["error"],
    ...
    async fn profiles_reject_names_that_escape_the_directory() {
        ...
        // The message as well as the status. A path-escape refusal is read by
        // whoever hit it, and the test above (`profiles_errors_carry_pythons_
        // message_and_code`) pins that wording for other errors -- leaving this
        // one on the status alone let the refusal every bad name produces drift
        // silently, across all five of them at once.
        // Per name, not one string for all five: pinning the wording surfaced
        // that an empty name is refused with its own sentence. A single
        // expectation would have had to be loosened to pass, which is how a
        // message assertion turns back into the status-only one it replaced.
        let charset = "Profile name must be 1-64 chars of [a-zA-Z0-9_-]";
        let expected_for = |name: &str| {
            if name.is_empty() {
        ...
        for bad in ["..", "../evil", "a/b", "", long.as_str()] {
            let expected_message = expected_for(bad);
            ...
            let body: serde_json::Value = serde_json::from_slice(&b).unwrap();
            assert_eq!(body["error"], expected_message, "get, name {bad:?}");
    ...
    async fn profiles_export_strips_sensitive_fields() {
        ...
        // Python's field name is `base_config`; Rust used to call it `config`
        // and therefore dropped it in silence.
        let body = json!({
            "name": "secretish",
            "label": "Secretish",
            "base_config": {
        ...
        let envelope =
            body_json(profiles_export(State(state.clone()), Path("secretish".to_string())).await)
                .await;
        // Python hands back the payload as a JSON string under `qr_data`.
        assert_eq!(envelope["ok"], true);
        let qr: serde_json::Value =
            serde_json::from_str(envelope["qr_data"].as_str().expect("qr_data is a string"))
                .expect("qr_data holds JSON");
        assert_eq!(qr["schema"], "yu://profile/1");
        let exported = qr["profile"].clone();
...
///
/// The two backup readers have since been implemented for real and moved to
/// `routes::backup_read`; only the write side still refuses here.
#[cfg(test)]
mod fabricated_state_tests {
    ...
    fn configured_backends_are_listed_with_their_id_injected() {
        let listed = backends_from_section(&json!({
            "backends": {
                ...
                "beta": {"type": "comfyui", "base_url": "http://127.0.0.1:8188"},
            }
        }));
        ...
        assert_eq!(alpha["type"], "ollama");
        assert_eq!(alpha["name"], "ollama:127.0.0.1:11434");
        assert_eq!(alpha["color"], "#888888");
        assert_eq!(alpha["port"], 11434);
        assert_eq!(alpha["status"], "unknown");
    }
    ...
    #[test]
    fn backend_listing_filters_invalid_entries_and_allowlists_secrets() {
        let listed = backends_from_section(&json!({"backends": {
            "public": {"type": "ollama", "base_url": "http://localhost:11434", "secret_enc": "must-not-leak", "internal": {"token": "must-not-leak"}},
            "agentmemory": {"base_url": "http://127.0.0.1:8080", "secret_enc": "secret"},
            "missing_url": {"type": "ollama"},
            "not_an_object": "ollama"
        }}));
        assert_eq!(listed.len(), 1);
        let public = &listed[0];
        assert_eq!(public["id"], "public");
        assert!(public.get("secret_enc").is_none());
        assert!(public.get("internal").is_none());
        assert!(listed.iter().all(|entry| entry["id"] != "agentmemory"));
    }

    #[test]
    fn backend_listing_applies_python_defaults_and_ports() {
        // Measured with Python 3.13 urllib.parse: `localhost:11434` has
        // scheme `localhost`, path `11434`, and port None.
        let listed = backends_from_section(&json!({"backends": {
            "comfy": {"type": "comfyui", "base_url": "http://comfy/"},
            "sd": {"type": "sd_webui", "base_url": "http://sd"},
            "gradio": {"type": "gradio", "base_url": "http://gradio"},
            "ollama": {"type": "ollama", "base_url": "localhost:11434"}
        }}));
        for (id, name, color) in [
            ("comfy", "comfyui:comfy", "#4a9eff"),
            ("sd", "sd_webui:sd", "#ff7a4a"),
            ("gradio", "gradio:gradio", "#f97316"),
            ("ollama", "ollama:11434", "#888888"),
        ] {
            let entry = listed.iter().find(|entry| entry["id"] == id).unwrap();
            assert_eq!(entry["name"], name);
            assert_eq!(entry["color"], color);
            assert_eq!(entry["status"], "unknown");
            assert!(entry.get("port").is_some());
            assert!(entry["port"].is_null());
        }
        assert_eq!(
            listed.iter().find(|entry| entry["id"] == "comfy").unwrap()["base_url"],
            "http://comfy"
        );
    }

    #[test]
    fn backend_listing_preserves_explicit_name_color_and_valid_port() {
        let listed = backends_from_section(&json!({"backends": {
            "custom": {"type": "comfyui", "base_url": "https://host:4321", "name": "Configured name", "color": "#abc123"},
            "non_numeric": {"type": "ollama", "base_url": "http://host:bad"},
            "out_of_range": {"type": "ollama", "base_url": "http://host:65536"},
            "zero": {"type": "ollama", "base_url": "http://host:0"}
        }}));
        let entry = listed.iter().find(|entry| entry["id"] == "custom").unwrap();
        assert_eq!(entry["name"], "Configured name");
        assert_eq!(entry["color"], "#abc123");
        assert_eq!(entry["port"], 4321);
        assert!(listed
            .iter()
            .find(|entry| entry["id"] == "non_numeric")
            .unwrap()["port"]
            .is_null());
        assert!(listed
            .iter()
            ...
            .is_null());
        assert_eq!(
            listed.iter().find(|entry| entry["id"] == "zero").unwrap()["port"],
            0
        );
    }

    #[test]
    fn backend_listing_keeps_a_truthy_slash_only_base_url() {
        // Measured with Python 3.13: raw `/` passes the truthiness filter,
        // then rstrip("/") yields an empty base_url and `ollama:` name.
        let listed = backends_from_section(&json!({"backends": {
            "slash": {"type": "ollama", "base_url": "/"}
        }}));
        assert_eq!(
            listed,
            vec![json!({
    ...
    #[test]
    fn backend_listing_keeps_a_unicode_authority() {
        // Measured with Python 3.13 urllib.parse: `http://münich:123/` has
        // netloc `münich:123`, port 123, and strips to this response URL.
        let listed = backends_from_section(&json!({"backends": {
            "unicode": {"type": "ollama", "base_url": "http://münich:123/"}
        }}));
        assert_eq!(
            listed,
            vec![json!({
                "type": "ollama",
                "base_url": "http://münich:123",
                "name": "ollama:münich:123",
                "color": "#888888",
                "id": "unicode",
                "status": "unknown",
                "port": 123,
    ...
    #[tokio::test]
    async fn write_side_backup_endpoints_still_refuse() {
        // `list` and `status` now answer for real (`routes::backup_read`), but
        // creating, restoring and deleting a backup remain unimplemented here
        // and must keep saying so rather than reporting a success they did not
        // perform.
        let response = stub_unavailable().await.into_response();
        assert_eq!(response.status(), StatusCode::SERVICE_UNAVAILABLE);
        let body = body_json(response).await;
        assert_eq!(body["ok"], json!(false));
        assert!(
            body.get("success").is_none() && body.get("filename").is_none(),
            "the refusal must not look like a completed backup: {body}"
        );
    }
}
