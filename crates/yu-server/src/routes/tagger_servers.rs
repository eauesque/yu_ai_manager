...
pub(crate) fn has_local_tagger_capability(state: &SharedState) -> bool {
    // Scan only the single startup-resolved root (M4): the sidecar binds to
    // exactly one directory, so scanning both primary and legacy here would
    // over-report capability to LAN peers when only one of them is complete
    // (mesh_inference.rs's documented "under-reporting is the honest
    // fallback" invariant requires matching what is actually reachable).
    let Ok(entries) = std::fs::read_dir(&state.config.wd_tagger_root) else {
        return false;
    };
...
// ── tagger-servers batch handlers (no admin scope — mirrors Python) ───────────
//
// `batch_tag` and `batch_cancel` are native: `batch_tag` spawns
// `run_tagger_cluster`, and `batch_cancel` cancels that job through the Rust
// `JobManager` (jobs/mod.rs).
//
// KEEP THEM TOGETHER. They were ported in one step on purpose, and splitting
// them again reintroduces a silent failure: Rust's `JobManager` cancels through
// an in-process `cancel_token` in its own `jobs` map, so a native cancel aimed
// at a batch that Python's `run_tagger_batch` started finds nothing, returns
// false, and answers 404 — reporting "no such job" for a job that is really
// running and that Python would have stopped. The same applies in reverse.
...
mod tests {
    ...
    async fn test_state_with_local_tagger() -> (SharedState, String) {
        ...
        let wd_tagger_root = cache_dir.join("wd_tagger");
