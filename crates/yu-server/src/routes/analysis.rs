...
pub(crate) fn is_hailo_device_available() -> bool {
    hailo_device_paths()
        .iter()
...
pub async fn analyze_file(
    ...
    if !ai.is_object() || ai.as_object().is_some_and(serde_json::Map::is_empty) {
        // 400, not 200: this is a refusal, and returning it with a success status told
        // every client checking `response.ok` that the analysis had been accepted.
        // Python answers 400 for the same condition (routes/analysis_job_routes.py:43
        // via engine_resolver.py:88-97); its message differs because it is chosen at
        // the end of an engine-resolution chain Rust does not have -- recorded as a
        // measured `expected_body_diff`, not hidden.
        return (
            StatusCode::BAD_REQUEST,
            Json(json!({"ok": false, "error": "no_legacy_ai_analysis_config"})),
        )
            .into_response();
    }
...
pub async fn analysis_trends_history_delete(
    ...
    let result = sqlx::query("DELETE FROM prompt_trend_history WHERE id = ?")
        ...
        .execute(&state.db)
        .await;
    // Python uses the `ok` envelope here, not a `success` flag:
    // `api_result({"deleted": True})` on success and `api_error("Not found", 404)`
    // when nothing matched (routes/analysis_server_routes.py). A client reading
    // `ok` or `deleted` got nothing from the Rust shape.
    match result {
        ...
        Ok(_) => (
            ...
            .into_response(),
        Err(e) => {
            tracing::error!("analysis_trends_history_delete: {e}");
            (
                StatusCode::INTERNAL_SERVER_ERROR,
                Json(json!({"ok": false, "error": "Failed to delete trend history"})),
            )
                .into_response()
        }
    }
...
pub async fn analysis_ollama_test(
        ...
        .filter(|s| !s.is_empty())
    {
        ...
        None => {
            return (
                StatusCode::BAD_REQUEST,
                // Python's `api_error` puts `ok: false` beside the message; omitting it
                // was the whole measured difference on this route.
...
pub async fn analysis_servers_discovered_register(
    ...
    // Python validates with a Pydantic model over `ApiModel` and reports EVERY error at
    // once; `{"success": false, "error": "… is required"}` was a different contract for
    // the same condition, and `success` is a key Python does not have (measured 400/400,
    // v4.734.5). The business errors below are untouched -- unmeasured.
    if let Err(errors) = crate::routes::analysis_discovery_models::validate_register(&data) {
        return crate::routes::api_validation::api_validation_errors(&errors);
    }
    let provider = data
        .get("provider")
        .and_then(Value::as_str)
...
pub async fn analysis_servers_discovered_match_post(
    ...
    // the same condition, and `success` is a key Python does not have (measured 400/400,
    // v4.734.5). The business errors below are untouched -- unmeasured.
    if let Err(errors) = crate::routes::analysis_discovery_models::validate_match(&data) {
        return crate::routes::api_validation::api_validation_errors(&errors);
    }
    let base_url =
...
pub async fn analysis_servers_discovered_match_delete(
    ...
    // `unwrap_or(json!({}))` treated an ABSENT body as an empty object, so a
    // request with no JSON at all got a validation error where Python answers
    // `invalid_content_type` (measured, v4.734.5).
    let data: Value = match crate::routes::api_validation::require_json_body(&headers, &body) {
        Ok(v) => v,
    ...
    // the same condition, and `success` is a key Python does not have (measured 400/400,
    // v4.734.5). The business errors below are untouched -- unmeasured.
    if let Err(errors) = crate::routes::analysis_discovery_models::validate_ignore(&data) {
        return crate::routes::api_validation::api_validation_errors(&errors);
    }
    let base_url =
        normalize_base_url_simple(data.get("base_url").and_then(Value::as_str).unwrap_or(""));
...
pub async fn analysis_config_get(
    ...
    if has_servers {
        let active = config
            ...
            .cloned()
            .unwrap_or(json!(null));
        // Python does not hand `ai_servers` out raw: `get_servers_with_status`
        // (core/analysis_api/server_crud_read.py:33-47) marks the active one, sets a
        // passive `status`, and masks `config.api_key`. Passing the array through left
        // `is_active`/`status` missing AND the key unmasked.
        let servers: Vec<Value> = config
            .get("ai_servers")
            .and_then(Value::as_array)
            .map(|rows| {
                rows.iter()
                    .map(|row| server_with_status(row, &active))
                    .collect()
            })
            .unwrap_or_default();
        ai["servers"] = json!(servers);
        ai["active_server"] = active;
    }

    // Python answers through `api_result`, so `ok`, `error` and `data` are all present.
    // Hand-adding `ok` alone left two envelope keys missing -- the sixth site of this.
    super::api_envelope::api_ok(ai).into_response()
}

/// One server entry shaped as Python's `get_servers_with_status` shapes it.
///
/// Mirrors `core/analysis_api/server_crud_read.py:33-47`: `is_active` against the
/// active id, a passive `status` of "unknown" (no probe is performed), and `api_key`
/// masked to first-four + "..." + last-two when longer than six characters.
fn server_with_status(row: &Value, active: &Value) -> Value {
    let mut out = row.clone();
    // Written through `as_object_mut` rather than `out[..] = ..`: indexing a
    // `serde_json::Value` panics when it is not the kind indexed (a server row without
    // a `config` object, say), and the clippy indexing_slicing ratchet gates on it.
    let Some(obj) = out.as_object_mut() else {
        return out;
    };
    let id = row.get("id").cloned().unwrap_or(json!(null));
    obj.insert("is_active".to_string(), json!(&id == active));
    ...
    let masked = obj
        ...
        .and_then(|key| {
            ...
            // Taken with iterators, so the bound travels with the take/skip and no
            // separate length check can fall out of step.
            (count > 6).then(|| {
                let head: String = key.chars().take(4).collect();
                let tail: String = key.chars().skip(count - 2).collect();
                format!("{head}...{tail}")
            })
        });
    if let Some(masked) = masked {
        if let Some(cfg) = obj.get_mut("config").and_then(Value::as_object_mut) {
            cfg.insert("api_key".to_string(), json!(masked));
        }
    }
    out
}
...
mod tests {
    ...
    /// Python uses the `ok` envelope here (`api_result({"deleted": True})` on
    /// success, `api_error("Not found", 404)` when nothing matched), not a
    /// `success` flag. Rust answered `{"success": ...}`, so a client reading `ok`
    /// or `deleted` got nothing on either path -- v4.732.24 measured it as
    /// `ok, success`.
    #[tokio::test]
    async fn trends_history_delete_uses_pythons_ok_envelope() {
        // `test_state` already seeds prompt_trend_history with ids 1 and 2; creating
        // the table here hit `engine NOT NULL` on a hand-rolled insert. Read the
        // fixture rather than build a second one.
        let state = test_state(serde_json::json!({})).await;

        // Absent row: 404 with Python's wording, under `ok`.
        let response =
            analysis_trends_history_delete(State(state.clone()), axum::extract::Path(999_999))
                .await;
        ...
        assert!(
            ...
            "`success` is not part of Python's shape: {body}"
        );

        // Present row: the api_result envelope with `deleted`. id 1 is seeded by
        // `test_state`.
        let response = analysis_trends_history_delete(State(state), axum::extract::Path(1)).await;
        let body = json_body(response).await;
