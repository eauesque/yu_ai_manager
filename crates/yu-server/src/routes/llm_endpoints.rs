...
pub async fn delete_endpoint(
    ...
    if let Err(e) = crate::config_io::write(&config_path, &config) {
        ...
        return Json(json!({"ok": false, "error": "write failed"})).into_response();
    }
...
pub async fn update_llm_endpoints(
    ...
    if category.is_empty() {
        return (
            ...
            Json(json!({"ok": false, "error": "category is required"})),
        )
            .into_response();
    }
    if base_url.is_empty() || model.is_empty() {
        return (
            StatusCode::BAD_REQUEST,
            Json(json!({"ok": false, "error": "base_url and model are required"})),
        )
            .into_response();
    }
    let raw_key = body
