...
pub async fn start(
    ...
    if state.job_manager.is_running(JOB_ID) {
        return (
            StatusCode::CONFLICT,
            Json(json!({"ok": false, "error": "hash backfill already running", "code": "backfill_already_running"})),
        )
            .into_response();
    }
    ...
    Json(json!({"ok": true, "error": null, "data": null, "status": "started"})).into_response()
}
...
pub async fn cancel(
    ...
    if state.job_manager.cancel_job(JOB_ID) {
        Json(json!({"ok": true, "error": null, "data": null, "status": "cancelling", "message": "hash backfill 停止を要求しました"})).into_response()
    } else {
        (
            StatusCode::NOT_FOUND,
            Json(json!({"ok": false, "error": "no running hash backfill", "code": "backfill_not_running"})),
        )
            .into_response()
    }
}
...
mod tests {
    ...
    async fn test_state() -> (SharedState, tempfile::TempDir) {
        ...
        .await;
        sqlx::query(
            "CREATE TABLE files (id INTEGER PRIMARY KEY, path TEXT NOT NULL, is_deleted INTEGER NOT NULL, hash TEXT)",
        )
        .execute(&state.db)
        .await
        .unwrap();
    ...
    #[tokio::test]
    async fn start_returns_python_envelope() {
        let (state, _root) = test_state().await;
        let response = start(State(state), None).await;
        assert_eq!(
            json_body(response).await,
            json!({"ok": true, "error": null, "data": null, "status": "started"})
        );
    }

    #[tokio::test]
    async fn start_running_returns_python_code() {
        let (state, _root) = test_state().await;
        state.job_manager.start(JOB_ID, "Hash Backfill");
        let response = start(State(state), None).await;
        assert_eq!(response.status(), StatusCode::CONFLICT);
        assert_eq!(
            json_body(response).await,
            json!({"ok": false, "error": "hash backfill already running", "code": "backfill_already_running"})
        );
    }

    #[tokio::test]
    async fn cancel_running_returns_python_message_and_envelope() {
        let (state, _root) = test_state().await;
        state.job_manager.start(JOB_ID, "Hash Backfill");
        let response = cancel(State(state), None).await;
        assert_eq!(
            json_body(response).await,
            json!({"ok": true, "error": null, "data": null, "status": "cancelling", "message": "hash backfill 停止を要求しました"})
        );
    }

    #[tokio::test]
    async fn cancel_absent_returns_python_code() {
        let (state, _root) = test_state().await;
        let response = cancel(State(state), None).await;
        assert_eq!(response.status(), StatusCode::NOT_FOUND);
        assert_eq!(
            json_body(response).await,
            json!({"ok": false, "error": "no running hash backfill", "code": "backfill_not_running"})
        );
    }
}
