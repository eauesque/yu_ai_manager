...
pub(crate) async fn run_detect_worker(
    ...
    for (index, file_id) in targets.into_iter().enumerate() {
            ...
            .await
        {
            ...
            Err(error) => {
                // A cancelled job whose in-flight request then fails is still a
                // cancellation, not a failure: the request was abandoned
                // because the operator asked to stop. Without this check the
                // verdict depended on whether the cancel landed before or
                // after the client gave up on the request -- a race that made
                // `run_detect_worker_stops_at_file_boundary_when_cancelled`
                // report "error" instead of "cancelled" on roughly two runs in
                // five, and that would equally mislead a real operator who
                // cancelled a long detection run.
                if cancel.is_cancelled() {
                    state.job_manager.finish_cancelled(
                        &job_id,
