...
async fn build_scan_errors(
    ...
    if !table_exists(pool, "scan_errors").await? {
        return Ok(json!({"errors": [], "total": 0, "unresolved_count": 0}));
    }
...
mod tests {
    ...
    #[tokio::test]
    async fn list_without_table_returns_python_empty_payload() {
        let state = test_state("").await;
        sqlx::query("DROP TABLE scan_errors")
            .execute(&state.db_read)
            .await
            .unwrap();
        let response = list(State(state), None, Query(HashMap::new())).await;
        assert_eq!(
    ...
    #[tokio::test]
    async fn resolve_database_failure_uses_python_error_shape() {
        let state = test_state("").await;
        sqlx::query("DROP TABLE scan_errors")
            .execute(&state.db)
            .await
