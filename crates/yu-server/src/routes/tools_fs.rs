...
///
/// Parameters:
/// - `q` / `query`: path filter, folded through `nfkc_lower` on both sides
...
pub async fn file_search(
    ...
    if !q.is_empty() {
        // `nfkc_lower`, not plain LIKE: Python compares the folded path against a
        // folded pattern (`core/tools/services_file_search.py:17`), which matches
        // rows that raw LIKE misses (full-width, dash variants, wave dash). The
        // function is registered on every serving pool by tagdb-core.
        conditions.push("nfkc_lower(f.path) LIKE ?".to_string());
    }
    match meta.as_str() {
        "has_meta" => {
    ...
    // Folded like the column, and NOT escaped: Python binds `%{nfkc_lower(q)}%`
    // with no ESCAPE clause, so `%` and `_` from the caller are wildcards there.
    // Escaping them here made the same query answer with different rows.
    let like_pat = format!("%{}%", tagdb_core::db::custom_functions::nfkc_lower(&q));

    // No `is_deleted` filter: `core/tools/services_file_search.py` returns deleted
    // rows too and hands the caller an `is_deleted` field to judge with. This is the
    // admin file-search view, and hiding them is a different answer -- the v4.733.4
    // run measured the difference as `results[len]`.
    //
    // `meta_source` comes from `files`, not `templates`: Python reads
    // `f.meta_source` and coalesces it to "unknown".
    let rows_sql = if has_templates {
        format!(
            "SELECT f.id, f.path, f.meta_source, f.mtime, f.size, f.is_deleted, \
             ...
             WHERE {where_clause} ORDER BY f.path LIMIT ?"
        )
    } else {
        format!(
            "SELECT f.id, f.path, f.meta_source, f.mtime, f.size, f.is_deleted, \
                    f.hash AS content_hash, f.parser_version, \
                    NULL AS raw_prompt, NULL AS format \
             FROM files f \
             WHERE {where_clause} ORDER BY f.path LIMIT ?"
        )
    };
    ...
    let results: Vec<serde_json::Value> = rows
        ...
        .map(|r| {
            json!({
                ...
                // Python: `r["meta_source"] or "unknown"`.
                "meta_source": r
                    .try_get::<Option<String>, _>("meta_source")
                    .unwrap_or(None)
                    .filter(|source| !source.is_empty())
                    .unwrap_or_else(|| "unknown".to_string()),
                // `mtime INTEGER NOT NULL` and `parser_version INTEGER NOT NULL`
                // (genesis_v89.sql). Reading them as f64/String made mtime a float
                // and parser_version null -- what the v4.733.4 run measured.
                "mtime": r.try_get::<i64, _>("mtime").unwrap_or(0),
                "size": r.try_get::<i64, _>("size").unwrap_or(0),
                "is_deleted": r.try_get::<i64, _>("is_deleted").unwrap_or(0) != 0,
                "content_hash": r.try_get::<Option<String>, _>("content_hash").unwrap_or(None),
                "parser_version": r.try_get::<Option<i64>, _>("parser_version").unwrap_or(None),
                // Python: `bool(r["raw_prompt"])` -- presence, not the text.
                "has_prompt": r
                    .try_get::<Option<String>, _>("raw_prompt")
                    .unwrap_or(None)
                    .is_some_and(|prompt| !prompt.is_empty()),
                "format": r.try_get::<Option<String>, _>("format").unwrap_or(None),
            })
        })
        .collect();

    // Python hands this to `api_result`: ok/error/data plus the payload at the
    // top level.
    Json(json!({
        "ok": true,
        "error": null,
