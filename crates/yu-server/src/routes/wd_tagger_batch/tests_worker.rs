...
#[tokio::test]
async fn backend_error_skips_the_file_instead_of_aborting_the_batch() {
    // A yu-infer 500 on one image (a corrupt frame, a decode failure) used to
    // kill the whole job; the remaining targets must still be tagged.
    let state = build_test_state().await;
    let cancel = state.job_manager.start(WD_TAGGER_JOB_ID, "WD-Tagger batch");
    ...
    run_batch_worker_with_tagger(
        ...
        false,
        cancel,
        |_state, file_id, _force| {
            Box::pin(async move {
                if file_id == 902 {
                    TagOutcome::Fatal(FatalReason::BackendError("inference failed".to_string()))
                } else {
    ...
    let job = state.job_manager.get_job(WD_TAGGER_JOB_ID).unwrap();
    assert!(job.error.is_none(), "batch must not abort: {:?}", job.error);
    let result_value = job.result.expect("finished job must carry a result");
    assert_eq!(result_value["total"], json!(3));
    assert_eq!(result_value["processed"], json!(2));
    assert_eq!(result_value["errors"], json!(1));
}

#[tokio::test]
async fn consecutive_backend_errors_still_abort_the_batch() {
    // The flip side: a backend that is actually down fails every file, so the
    // batch must give up instead of grinding through the whole library.
    let state = build_test_state().await;
    let cancel = state.job_manager.start(WD_TAGGER_JOB_ID, "WD-Tagger batch");
    ...
    let job = state.job_manager.get_job(WD_TAGGER_JOB_ID).unwrap();
    assert!(job.error.as_deref().unwrap().contains("BackendError"));
    assert_eq!(
        attempted.load(std::sync::atomic::Ordering::SeqCst),
        MAX_CONSECUTIVE_BACKEND_ERRORS as usize
    );
}
