...
use crate::{
    ...
    paths::{home_dir, safe_model_dir_name},
    state::SharedState,
};
...
pub async fn model_status(
    ...
    let (repo, cache_dir, file_names, required) = if let Some(profile) = profile {
        let repo = profile
            ...
            .unwrap_or(&key)
            .to_string();
        let cache_dir = match profile_model_dir(&state, profile) {
            Ok(dir) => dir,
            Err(error) => {
                return api_error_code(
                    &format!("Failed to resolve model directory: {error}"),
                    StatusCode::INTERNAL_SERVER_ERROR,
                    "model_dir_resolution_failed",
                )
            }
        };
        let files = profile
        ...
        let primary = state
            ...
            .cache_dir
            .join("wd_tagger")
            .join(safe_model_dir_name(&key));
        let legacy = home_dir().map(|home| {
            home.join(".cache/yu_ai_manager/wd_tagger")
                .join(safe_model_dir_name(&key))
        });
        let cache_dir = pick_dir_by_file_count(primary, legacy, &names);
...
fn profile_model_dir_from_base(base: &Path, profile: &Value) -> Result<PathBuf, HfError> {
    let model_id = profile
        .get("model_id")
        .and_then(Value::as_str)
        .ok_or_else(|| HfError::Request("profile missing model_id".into()))?;
    let mut base = base.join(safe_model_dir_name(model_id));
...
fn primary_profile_model_dir(state: &SharedState, profile: &Value) -> Result<PathBuf, HfError> {
    profile_model_dir_from_base(&state.config.cache_dir.join("wd_tagger"), profile)
}

/// Names of a profile's `required: true` files. A file entry missing the
/// `required` key is treated as not required, matching this module's prior
/// `profile_has_required_file` behavior.
fn required_file_names(profile: &Value) -> Vec<String> {
    profile
        .get("files")
        .and_then(Value::as_array)
        .into_iter()
        .flatten()
        .filter(|file| file.get("required") == Some(&Value::Bool(true)))
        .filter_map(|file| file.get("name").and_then(Value::as_str).map(str::to_string))
        .collect()
}

/// Count how many of `names` exist as files directly under `base`.
fn count_files_present(base: &Path, names: &[String]) -> usize {
    names
        .iter()
...
/// Pick between a primary and an optional legacy directory by how many of
/// `names` each one holds. Legacy wins only with a strictly higher count;
/// ties (including both empty) go to primary.
fn pick_dir_by_file_count(primary: PathBuf, legacy: Option<PathBuf>, names: &[String]) -> PathBuf {
    match legacy {
        Some(legacy)
            if count_files_present(&legacy, names) > count_files_present(&primary, names) =>
        {
            legacy
        }
        _ => primary,
...
fn profile_model_dir(state: &SharedState, profile: &Value) -> Result<PathBuf, HfError> {
    let primary = primary_profile_model_dir(state, profile)?;
    let legacy = home_dir()
        .map(|home| {
            profile_model_dir_from_base(&home.join(".cache/yu_ai_manager/wd_tagger"), profile)
        })
        .transpose()?;
    let names = required_file_names(profile);
    Ok(pick_dir_by_file_count(primary, legacy, &names))
}
...
/// Resolve the WD-Tagger cache root passed to the yu-infer sidecar.
///
/// The sidecar (and the upstream `yu_infer_core::resolve_model_dir`) builds
/// `<root>/<model_id>[/<hf_subdir>]` itself from a single root, so unlike
/// `profile_model_dir` this cannot resolve per-profile -- it picks one root
/// for every profile. Primary wins whenever it holds anything; legacy is
/// used only when primary is absent/empty and legacy actually has entries,
/// so a fresh install with an empty primary directory does not accidentally
/// route inference at an empty legacy directory instead.
pub(crate) fn wd_tagger_root(cache_dir: &Path) -> PathBuf {
    let primary = cache_dir.join("wd_tagger");
    if dir_has_entries(&primary) {
        return primary;
    }
    if let Some(home) = home_dir() {
        let legacy = home.join(".cache/yu_ai_manager/wd_tagger");
...
async fn download_hf_file_with_request(
    ...
    let base = primary_profile_model_dir(state, profile)?;
...
pub(crate) mod tests {
    ...
    async fn hf_download_uses_cached_file_without_http() {
        let dirs = test_dirs();
        // profile_model_dir resolves under state.config.cache_dir (not
        // project_root), so the fixture cache dir below must match it -- a
        // plain test_state() defaults cache_dir to "." and would silently
        // miss the fixture, sending a real HTTP request instead.
        let state = test_state_ex(&dirs, json!({}), None, true, dirs.root.join("cache")).await;
    ...
    fn hf_url_keeps_slashes_and_path_guard_rejects_escape() {
        ...
        assert!(matches!(
            profile_model_dir_from_base(
                root.path(),
                &json!({"model_id":"org/model", "hf_subdir":"../../etc"})
            ),
            Err(HfError::Path(_))
        ));
    }
    ...
    #[tokio::test]
    async fn profile_model_directory_resolves_primary_legacy_and_download_destination() {
        ...
        let cache_env = std::env::var_os("TAGDB_CACHE_DIR");
        ...
        unsafe {
            ...
            std::env::set_var("TAGDB_CACHE_DIR", cache_dir.path());
        }

        let dirs = test_dirs();
        let state =
            test_state_ex(&dirs, json!({}), None, true, cache_dir.path().to_path_buf()).await;
        let profile = full_profile("model-a", "Model A", "org/model");
        let primary = primary_profile_model_dir(&state, &profile).unwrap();
        let legacy = home_dir
            .path()
            .join(".cache/yu_ai_manager/wd_tagger/org_model");

        fs::create_dir_all(&primary).unwrap();
        fs::write(primary.join("model.onnx"), []).unwrap();
        assert_eq!(profile_model_dir(&state, &profile).unwrap(), primary);

        fs::remove_file(primary.join("model.onnx")).unwrap();
        fs::create_dir_all(&legacy).unwrap();
        fs::write(legacy.join("model.onnx"), []).unwrap();
        assert_eq!(profile_model_dir(&state, &profile).unwrap(), legacy);

        fs::remove_file(legacy.join("model.onnx")).unwrap();
        assert_eq!(profile_model_dir(&state, &profile).unwrap(), primary);
        assert_eq!(
            primary_profile_model_dir(&state, &profile).unwrap(),
            primary
        );
        ...
        unsafe {
            ...
            match cache_env {
                Some(value) => std::env::set_var("TAGDB_CACHE_DIR", value),
                None => std::env::remove_var("TAGDB_CACHE_DIR"),
            }
        }
    }

    #[tokio::test]
    async fn profile_model_directory_prefers_more_complete_directory_by_file_count() {
        let _guard = crate::ENV_MUTATION_TEST_LOCK
            .lock()
            .unwrap_or_else(std::sync::PoisonError::into_inner);
        let home = std::env::var_os("HOME");
        ...
        // Primary has only one of two required files (an interrupted
        // download); legacy has both. The more complete directory wins.
        fs::create_dir_all(&primary).unwrap();
        fs::write(primary.join("model.onnx"), []).unwrap();
        fs::create_dir_all(&legacy).unwrap();
        fs::write(legacy.join("model.onnx"), []).unwrap();
        fs::write(legacy.join("selected_tags.csv"), []).unwrap();
        assert_eq!(profile_model_dir(&state, &profile).unwrap(), legacy);

        // Both directories are equally complete: tie goes to primary.
        fs::write(primary.join("selected_tags.csv"), []).unwrap();
        assert_eq!(profile_model_dir(&state, &profile).unwrap(), primary);
    ...
    #[test]
    fn wd_tagger_root_resolves_primary_then_legacy() {
        let _guard = crate::ENV_MUTATION_TEST_LOCK
            .lock()
            .unwrap_or_else(std::sync::PoisonError::into_inner);
        let home = std::env::var_os("HOME");
        ...
        let primary = cache_dir.path().join("wd_tagger");
        let legacy = home_dir.path().join(".cache/yu_ai_manager/wd_tagger");

        // Both absent: falls back to primary.
        assert_eq!(wd_tagger_root(cache_dir.path()), primary);

        // Legacy has entries, primary still absent: legacy wins.
        fs::create_dir_all(legacy.join("org_model")).unwrap();
        assert_eq!(wd_tagger_root(cache_dir.path()), legacy);

        // Primary now has entries too: primary wins regardless of legacy.
        fs::create_dir_all(primary.join("org_model")).unwrap();
        assert_eq!(wd_tagger_root(cache_dir.path()), primary);
    ...
    pub(crate) async fn test_state_ex(
        ...
        let wd_tagger_root_val = wd_tagger_root(&cache_dir);
        let clip_model_dir_val = crate::routes::clip_model::model_dir(&cache_dir);
        Arc::new(
            AppState::new_with_infer(
                Config {
                    ...
                    wd_tagger_root: wd_tagger_root_val,
                    clip_model_dir: clip_model_dir_val,
                },
                pool.clone(),
                pool,
    ...
    async fn model_status_reports_legacy_cache_files_and_known_models() {
        // model_status's non-profile branch also probes the legacy HOME-based
        // directory (pick_dir_by_file_count) to decide which one wins. Pin
        // HOME to an empty tempdir under the shared lock so a concurrently
        // running test that temporarily repoints HOME elsewhere cannot make
        // this test observe stray files and flip `ready` to true.
        let _guard = crate::ENV_MUTATION_TEST_LOCK
            .lock()
            .unwrap_or_else(std::sync::PoisonError::into_inner);
        ...
        // model_status resolves under state.config.cache_dir (not
        // project_root) so the fixture above must match it.
        let state = test_state_ex(
            &dirs,
    ...
    #[tokio::test]
    async fn model_status_reports_ready_when_only_legacy_has_the_model() {
        let _guard = crate::ENV_MUTATION_TEST_LOCK
            .lock()
            .unwrap_or_else(std::sync::PoisonError::into_inner);
        let home = std::env::var_os("HOME");
        ...
        let dirs = test_dirs();
        // cache_dir stands in for a non-default TAGDB_CACHE_DIR; primary
        // under it stays empty, so only the legacy directory has the model.
        let cache_dir = dirs.root.join("cache");
        let legacy = home_dir
            .path()
            .join(".cache/yu_ai_manager/wd_tagger/SmilingWolf_wd-swinv2-tagger-v3");
        fs::create_dir_all(&legacy).unwrap();
        fs::write(legacy.join("model.onnx"), b"x").unwrap();
        fs::write(legacy.join("selected_tags.csv"), b"name,category\na,0\n").unwrap();
        let state = test_state_ex(
            &dirs,
            json!({"wd_tagger": {"model": "SmilingWolf/wd-swinv2-tagger-v3"}}),
            None,
        ...
        assert_eq!(value["ready"], true);
        assert_eq!(value["files"]["model.onnx"]["exists"], true);
        assert_eq!(value["files"]["selected_tags.csv"]["exists"], true);
    ...
    async fn profile_delete_rejects_active_profile() {
        ...
        assert_eq!(
            value,
            json!({"ok": false, "error": "profile is active model", "code": "in_use", "active_model_id": "model-a"})
        );
    }
    ...
    async fn tag_file_native_wrapper_unchanged_behavior_for_fallback() {
        if !crate::routes::video_analysis::check_ffmpeg() {
            // Without ffmpeg, tag_video_core short-circuits to keyframe_error
            // (400) before ever reaching the infer-sidecar-unavailable path
            // this test exercises.
            return;
        }
    ...
    /// A sidecar that refuses this one path (403 outside its scan roots, or 400
    /// because the file moved since indexing) must stay a per-file skip. Before
    /// this, both surfaced as `FatalReason::BackendError`, and the batch worker
    /// aborted the whole job once 20 of them piled up in a row.
    #[tokio::test]
    async fn sidecar_path_rejection_is_a_per_file_skip_not_a_backend_error() {
        for (status, error) in [
            (StatusCode::FORBIDDEN, "path outside allowed roots"),
            ...
            let dirs = test_dirs();
            let image_path = dirs.root.join("rejected.png");
            image::RgbImage::new(1, 1).save(&image_path).unwrap();
            let root = fs::canonicalize(&dirs.root).unwrap();
            let state = test_state_ex(
                &dirs,
            ...
            let outcome = tag_file_native_core(&state, 107, false).await;
            assert!(
                matches!(outcome, TagOutcome::Rejected(_)),
                "{status} should be a per-file skip, got {outcome:?}"
            );
            server.abort();
        }
    }
    ...
    async fn tag_file_native_core_returns_fatal_model_not_downloaded_when_model_absent() {
        ...
        assert_eq!(
            body,
            json!({"error": "Model not downloaded", "code": "model_not_available"})
        );
    }
    ...
    async fn tag_video_core_no_longer_forwards_when_strategy_is_scene() {
        ...
        let response = tag_file_native(&state, 401, false).await;
        assert_eq!(response.status(), StatusCode::BAD_REQUEST);
        assert_eq!(
    ...
    async fn tag_video_core_no_longer_forwards_when_store_per_keyframe_is_true() {
        ...
        let response = tag_file_native(&state, 402, false).await;
        assert_eq!(response.status(), StatusCode::BAD_REQUEST);
        assert_eq!(
            json_body(response).await,
            json!({"error": "Failed to extract keyframes", "code": "keyframe_error"})
