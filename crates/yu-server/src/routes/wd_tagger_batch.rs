...
/// `TagOutcome::Fatal` aborts the whole job immediately -- except
/// `FatalReason::BackendError`, see `MAX_CONSECUTIVE_BACKEND_ERRORS`.
...
/// Consecutive `FatalReason::BackendError` files tolerated before the batch
/// gives up: past this many in a row the backend, not the files, is broken.
const MAX_CONSECUTIVE_BACKEND_ERRORS: u64 = 20;
    ...
    F: for<'a> Fn(&'a SharedState, i64, bool) -> BoxFuture<'a, TagOutcome>,
{
    ...
    let mut consecutive_backend_errors: u64 = 0;
    ...
    for (idx, file_id) in targets.into_iter().enumerate() {
        ...
        match tag_file(&state, file_id, force).await {
            ...
            // A backend error is usually specific to one file -- a corrupt
            // image, a decode failure, a sidecar 500 on that frame -- so it
            // skips the file instead of killing a batch of thousands. A
            // genuinely broken backend fails every file in a row, which the
            // consecutive counter catches; the other FatalReasons are
            // sidecar-wide from the first occurrence and still abort at once.
            TagOutcome::Fatal(FatalReason::BackendError(msg))
                if consecutive_backend_errors + 1 < MAX_CONSECUTIVE_BACKEND_ERRORS =>
            {
                errors += 1;
                ...
                tracing::warn!(
                    ...
                    "wd-tagger batch: backend error on this file, skipping"
                );
            }
