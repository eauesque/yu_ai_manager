...
fn ext_dir(state: &AppState) -> PathBuf {
...
pub(crate) fn find_extension_manifest(state: &AppState, name: &str) -> Option<(PathBuf, Value)> {
...
///
/// `definitions` is a copy of Python's static `HOOK_DEFINITIONS`
/// (`core/extensions_core/extensions_defs_constants.py:7`) -- the set of hook
/// names and their dispatch modes, which does not depend on what is running.
/// Returning `{}` for it was a fabrication: it claimed the concept did not
/// exist rather than that nothing implements it here.
///
/// `hooks` stays empty, and honestly so: that field lists the *registered*
/// callbacks, and this server runs no Python extension code, so none are.
pub async fn hooks(
    State(state): State<SharedState>,
    auth: Option<Extension<AuthContext>>,
) -> Response {
    ...
    let definitions: serde_json::Map<String, Value> = HOOK_DEFINITIONS
        .iter()
        .map(|(name, mode)| ((*name).to_string(), json!({"mode": mode})))
        .collect();
    (
        StatusCode::OK,
        Json(json!({"ok": true, "hooks": [], "definitions": definitions})),
    )
        .into_response()
}

/// The hook names and dispatch modes Python defines. Pinned to
/// `core/extensions_core/extensions_defs_constants.py::HOOK_DEFINITIONS`;
/// a hook added there without being added here makes this endpoint lie.
...
fn review_manifest(trust_level: &str, manifest: &Value) -> (bool, Vec<Value>) {
    ...
    // The Rust server runs no hooks at all. Saying so here is the whole point:
    // before this, a third-party extension appeared in the installed list with
    // its `hooks` field displayed, and its hooks silently never ran -- no
    // error, no warning, and a file only it could parse became an "unknown
    // format" row with nothing to explain why.
    //
    // The audit view is where this belongs rather than a startup log line: a
    // user reads the extension list, not the server's stdout.
    //
    // Builtins are exempt because their hook bodies are already ported into
    // Rust directly (`meta-extract` / `fallback_chain` for `on_scan_file`,
    // `routes/file_detail_sections.rs` for `on_build_sections`,
    // `routes/lora_dataset.rs` for `on_db_migrate`). Warning about those would
    // be false: the feature is present, it simply does not travel via a hook.
    let is_builtin = manifest
        .get("name")
        .and_then(Value::as_str)
        .is_some_and(|name| name.starts_with("builtin-"));
    if has_hooks && !is_builtin {
        issues.push(warn(
            "この拡張は hooks を宣言していますが、Rust standalone サーバーは \
             hooks を一切実行しません（黙って無視されます）。hooks を動かすには \
             Python サーバーを明示的に起動してください。詳細は \
             docs/development/development_docs/EXTENSION_HOOKS_SPEC.md 冒頭の警告を参照。"
                .to_string(),
        ));
    }
...
mod tests {
    ...
    /// A third-party extension declaring hooks must be told they will not run.
    ///
    /// Before this warning existed the extension appeared in the installed
    /// list with its `hooks` field on display, and the hooks silently never
    /// ran -- no error anywhere, and a file only that extension could parse
    /// became an "unknown format" row with nothing to explain it.
    ///
    /// Builtins must NOT warn: their hook bodies are ported into Rust directly
    /// (meta-extract, file_detail_sections.rs, lora_dataset.rs), so the
    /// feature is present and the warning would be false. Measured 2026-09-09:
    /// the only extensions implementing any hook are those five builtins.
    #[test]
    fn review_warns_a_third_party_that_its_hooks_will_not_run() {
        let hook_warning = "Rust standalone サーバーは";
        // `permissions` is mandatory for non-builtins: without it review blocks
        // at once and never reaches the hook check. `event_bus` is what a hook
        // user is supposed to declare, so declaring it keeps this test focused
        // on the new warning rather than tripping the older one beside it.
        let perms = json!({"required": ["event_bus"]});
        ...
        assert!(
            approved,
            "the warning must not block installation: {issues:?}"
        );
        assert!(
            warns.iter().any(|m| m.contains(hook_warning)),
            "a third-party extension with hooks must be warned: {warns:?}"
        );

        // Builtin, reviewed on the same untrusted path so the check is actually
        // reached -- `trust_level == "trusted"` returns before any of this, so
        // passing "trusted" here would prove nothing about the name exemption.
        let (_, builtin_issues) = review_manifest(
            "untrusted",
            &json!({
                "name": "builtin-a1111",
                "hooks": ["on_scan_file"],
        ...
        assert!(
            !builtin_issues.iter().any(|i| i["message"]
                .as_str()
                .unwrap_or_default()
                .contains(hook_warning)),
            "builtins are ported directly; warning about them would be false: {builtin_issues:?}"
        );

        // An empty hooks list is not a declaration, matching Python's truthiness.
        let (_, empty_issues) = review_manifest(
            "untrusted",
            &json!({"name": "no-hooks", "hooks": [], "permissions": perms}),
        );
        assert!(
            !empty_issues.iter().any(|i| i["message"]
                .as_str()
                .unwrap_or_default()
                .contains(hook_warning)),
            "an empty hooks list must not warn: {empty_issues:?}"
        );
    }
