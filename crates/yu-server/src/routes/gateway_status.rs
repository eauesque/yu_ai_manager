...
fn gateway_gate(
    ...
    // NOT `state.gateway_loopback_bypass`. Python's `check_request` defaults
    // `allow_loopback_bypass` to False and each call site opts in: the five proxy
    // routes do (gateway_ollama, gateway_sd, gateway_comfy, gateway_gradio,
    // headroom_api), and `routes/gateway_status.py` -- these two routes -- does not.
    // Passing the flag here served a keyless loopback caller what Python answers 401
    // to, and a widened `accept_statuses` hid it (measured rust=200 py=401, v4.734.8).
    // `gateway_proxy.rs` keeps the bypass, because its Python counterpart opts in.
    let Some(auth) = gateway::check_request(&state.gateway_keys, false, headers, &client_ip) else {
        return Some(err_401());
    };
...
mod tests {
    ...
    /// These two routes do NOT honour the loopback bypass, even when it is enabled
    /// for the gateway as a whole.
    ///
    /// Python's `check_request` defaults `allow_loopback_bypass` to False and each
    /// call site opts in; `routes/gateway_status.py` -- these two routes -- does not,
    /// while the five proxy routes do. Rust used to pass the flag here, so a keyless
    /// loopback caller was served what Python answers 401 to. Measured rust=200 py=401
    /// (v4.734.8), hidden until then by a widened `accept_statuses`.
    #[tokio::test]
    async fn the_loopback_bypass_does_not_reach_these_two_routes() {
        let temp = TempDir::new().unwrap();
        // bypass enabled for the gateway; these routes must still refuse.
    ...
    async fn capabilities_reports_the_declared_surface_and_no_models() {
        let temp = TempDir::new().unwrap();
        let keys = scoped_keys(temp.path(), &["llm:models", "node:status"]);
        let state = test_state(temp.path().to_path_buf(), "{}", keys, false).await;
        let body = json_body(
            router_capabilities(State(Arc::clone(&state)), remote(), bearer("secret")).await,
        )
        .await;
    ...
    async fn node_services_lists_configured_backends_as_unknown() {
        ...
        .await;
        let body =
            json_body(node_services(State(Arc::clone(&state)), remote(), bearer("secret")).await)
                .await;
