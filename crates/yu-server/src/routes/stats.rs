...
async fn build_stats_all(state: &SharedState) -> Result<Value, sqlx::Error> {
    ...
    // A TTL hit is not evidence that the cached aggregate still agrees with the
    // DB, and it is not evidence that it disagrees either. Treating a plain hit
    // as `_stale` (which is what this did) lit the stats page's "these numbers
    // are behind" notice for the whole 60s window on a DB that had not changed
    // at all -- a standing false alarm, and the only body difference left
    // against Python once the comparison was un-suppressed (v4.732.8).
    //
    // Decide it by the DB signature instead, the way Python does
    // (`core/stats_api/stats_cache.py` keys its disk cache on the signature):
    // when the signature moves, drop the cached blocks. A hit therefore always
    // agrees with the DB, so this snapshot is honestly not stale. That also
    // fixes the substantive bug behind the flag -- these routes used to serve
    // up-to-60s-old aggregates after a write and merely flag them.
    {
        let mut last_sig = state.stats_db_sig.lock().await;
        if last_sig.as_ref() != Some(&db_sig) {
            state.stats_basic_cache.invalidate().await;
            state.stats_timeline_cache.invalidate().await;
            state.stats_models_cache.invalidate().await;
            state.stats_resolutions_cache.invalidate().await;
            *last_sig = Some(db_sig.clone());
        }
    }
    let (basic, _) = build_basic_stats(pool, &state.stats_basic_cache).await?;
    let (timeline, _) = build_timeline_stats(pool, "month", &state.stats_timeline_cache).await?;
    let (models, _) = build_model_stats(pool, &state.stats_models_cache).await?;
    let (resolutions, _) = build_resolution_stats(pool, &state.stats_resolutions_cache).await?;
    Ok(json!({
        "_stale": false,
...
mod tests {
    ...
    #[tokio::test]
    async fn stats_all_reflects_a_write_instead_of_serving_the_ttl_cache() {
        // The TTL caches live 60s. Before the db_sig check, a second call within
        // that window returned the first call's aggregates and merely set
        // `_stale: true` -- i.e. it served numbers it knew were behind, and lit
        // the stats page's "behind" notice even when nothing had changed.
        let state = test_state().await;

        let first = stats_all(State(state.clone()), None).await;
        let body = to_bytes(first.into_body(), usize::MAX).await.unwrap();
        let before: serde_json::Value = serde_json::from_slice(&body).unwrap();
        assert_eq!(before["basic"]["total_files"], 3);
        assert_eq!(before["_stale"], false);

        sqlx::query(
            "INSERT INTO files(id, path, mtime, is_deleted, meta_source) VALUES
               (7, '/added.png', 1704078000, 0, 'novelai_png')",
        )
        .execute(&state.db_read)
        .await
        .unwrap();

        let second = stats_all(State(state.clone()), None).await;
        let body = to_bytes(second.into_body(), usize::MAX).await.unwrap();
        let after: serde_json::Value = serde_json::from_slice(&body).unwrap();
        assert_eq!(
            after["basic"]["total_files"], 4,
            "the write must be visible immediately, not after the 60s TTL"
        );
        assert_eq!(
            after["_stale"], false,
            "a cache dropped on a signature change is never stale"
        );

        // An unchanged DB must keep the cache (no signature movement), and must
        // still not claim staleness -- that claim was the standing false alarm.
        let third = stats_all(State(state), None).await;
        let body = to_bytes(third.into_body(), usize::MAX).await.unwrap();
        let again: serde_json::Value = serde_json::from_slice(&body).unwrap();
        assert_eq!(again["basic"]["total_files"], 4);
        assert_eq!(again["_stale"], false);
    }
