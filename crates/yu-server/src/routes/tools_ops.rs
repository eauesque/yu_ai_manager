...
/// Mirror of `core/infra_core/api_errors.py::api_success`. Handlers here built
/// success payloads bare; Python's carry `ok`/`error`/`data`, so every one of
/// them was a body difference once the parity comparison was turned on.
fn tools_ok(payload: serde_json::Value) -> Response {
...
pub async fn clear_cache(
    ...
    // Python reports how many files it removed (`{"cleared": n}` in
    // core/tools_api/ops.py::clear_cache); Rust returned no `cleared` key at
    // all. Count before the directory goes away.
...
pub async fn rebuild_groups(
    ...
    match s.groups_index_cache.get(&s.db_read).await {
        // Python wraps with `api_result`, so `ok`/`error`/`data` sit beside the
        // payload. Measured once Python's own ImportError was fixed and the two
        // statuses finally agreed -- until then the bodies were never compared.
        Ok(idx) => Json(json!({
            "ok": true,
            "error": null,
            "data": null,
...
pub async fn dnd_inbox(
    ...
    // Mirror of `core/tools/services_drop_upload.py::_enabled_scan_roots`: only
    // object entries count, `enabled` defaults to true, and a blank path is
    // skipped. Rust used to take the first entry that happened to be a
    // directory, enabled or not.
    let enabled_roots: Vec<std::path::PathBuf> = cfg["scan_roots"]
        .as_array()
        .map(|roots| {
            roots
                .iter()
    ...
    // `resolve_drop_inbox_dir` returns (path, error); Rust reported a single
    // hardcoded message for every failure, so the caller could not tell "no scan
    // roots" from "your drop_inbox_dir is outside them".
    let (inbox, reason): (Option<std::path::PathBuf>, Option<String>) = if enabled_roots.is_empty()
    {
        (None, Some("no scan roots configured".to_string()))
    } else if let Some(ref dir) = explicit_dir {
        let candidate = std::path::PathBuf::from(dir);
        let resolved = candidate.canonicalize().unwrap_or(candidate);
        if enabled_roots.iter().any(|root| {
            ...
            (
                None,
                Some(format!(
                    "drop_inbox_dir '{dir}' is not inside any configured scan root"
                )),
            )
        }
    } else {
        (enabled_roots.first().cloned(), None)
    };

    match inbox {
        Some(p) => tools_ok(json!({
        ...
        None => Json(json!({
            "ok": false,
            "code": "no_inbox",
            "error": reason.unwrap_or_else(|| "inbox unresolved".to_string()),
        }))
        .into_response(),
    }
}
...
pub async fn register_path(
    ...
    if !matches!(
        ...
        return (
            ...
            Json(json!({
                ...
                "filename": canonical.file_name().and_then(|name| name.to_str()).unwrap_or("")
            })),
        )
            .into_response();
    }
    ...
    match result {
        ...
        Ok(r) => tools_ok(json!({"id": r.last_insert_rowid(), "path": canonical})),
        Err(e) => (
            StatusCode::INTERNAL_SERVER_ERROR,
            Json(json!({"error": e.to_string()})),
        )
...
pub async fn delete_duplicates(
    ...
    // Python echoes `mode` and attaches `errors` only when there are some,
    // capped at ten (core/tools/duplicates_delete_ops.py). Rust omitted `mode`
    // and always sent `errors`, empty list included.
    let mut payload = json!({"deleted": deleted, "mode": mode});
    if !errors.is_empty() {
...
mod tests {
    ...
    /// Python's api_success puts `ok`/`error`/`data` on every response
    /// (core/infra_core/api_errors.py). These handlers built the payload bare, so
    /// each was a body difference the moment the parity comparison was turned on
    /// -- v4.732.16 measured three of them in this file alone.
    /// `resolve_drop_inbox_dir` in Python names the failure; Rust answered a
    /// single hardcoded "inbox unresolved" whatever went wrong, so a caller could
    /// not tell "you have no scan roots" from "your drop_inbox_dir is outside
    /// them". Python also honours `enabled` on each scan root.
    #[tokio::test]
    async fn dnd_inbox_names_the_reason_it_could_not_resolve() {
        let dir = tempfile::TempDir::new().unwrap();

        // No scan roots at all.
        let cfg_a = dir.path().join("a.json");
        let state = test_state_with_config(&cfg_a, json!({"scan_roots": []})).await;
        let body = json_body(
            dnd_inbox(
                State(state),
                None,
                Some(Extension(ConnectInfo("127.0.0.1:9".parse().unwrap()))),
            )
            .await,
        )
        .await;
        assert_eq!(body["code"], "no_inbox");
        assert_eq!(body["error"], "no scan roots configured", "body: {body}");

        // A scan root exists but is disabled -- Python skips it, so this is still
        // "no scan roots configured".
        let root = dir.path().join("root");
        std::fs::create_dir_all(&root).unwrap();
        let cfg_b = dir.path().join("b.json");
        let state = test_state_with_config(
            &cfg_b,
            json!({"scan_roots": [{"path": root.to_string_lossy(), "enabled": false}]}),
        )
        .await;
        let body = json_body(
            dnd_inbox(
        ...
        .await;
        assert_eq!(
            body["error"], "no scan roots configured",
            "a disabled root must not count: {body}"
        );

        // An explicit drop_inbox_dir outside every scan root names itself.
        let outside = dir.path().join("outside");
        std::fs::create_dir_all(&outside).unwrap();
        let cfg_c = dir.path().join("c.json");
        let state = test_state_with_config(
            &cfg_c,
            json!({
                "scan_roots": [{"path": root.to_string_lossy(), "enabled": true}],
                "drop_inbox_dir": outside.to_string_lossy(),
            }),
        )
        .await;
        let body = json_body(
        ...
        assert!(
            body["error"]
                .as_str()
                .unwrap_or("")
                .contains("is not inside any configured scan root"),
            "body: {body}"
        );

        // An enabled root with no explicit dir resolves to that root.
        let cfg_d = dir.path().join("d.json");
        let state = test_state_with_config(
            &cfg_d,
            json!({"scan_roots": [{"path": root.to_string_lossy(), "enabled": true}]}),
        )
        .await;
        let body = json_body(
            dnd_inbox(
    ...
    /// Python echoes `mode` and attaches `errors` only when there are some
    /// (core/tools/duplicates_delete_ops.py). Rust omitted `mode` and always sent
    /// `errors`, so an ordinary success carried an empty list Python never sends.
    #[tokio::test]
    async fn delete_duplicates_echoes_mode_and_omits_empty_errors() {
        let dir = tempfile::TempDir::new().unwrap();
        let config_path = dir.path().join("config.json");
        let state = test_state_with_config(&config_path, json!({})).await;

        let body = json_body(
            delete_duplicates(State(state.clone()), None, Json(json!({"groups": []}))).await,
        )
        .await;
        assert_eq!(body["mode"], "soft", "default mode must be echoed: {body}");
        assert!(
            body.get("errors").is_none(),
            "an empty errors list must not be sent: {body}"
        );
        ...
        let body = json_body(
            delete_duplicates(
                State(state),
                None,
                Json(json!({"groups": [], "mode": "hard"})),
            )
            .await,
        )
        .await;
        assert_eq!(body["mode"], "hard", "body: {body}");
    }

    #[tokio::test]
    async fn tools_success_bodies_carry_the_python_envelope() {
        let dir = tempfile::TempDir::new().unwrap();
        let config_path = dir.path().join("config.json");
        let state = test_state_with_config(&config_path, json!({})).await;

        let body = json_body(clear_cache(State(state.clone()), None).await).await;
        for key in ["ok", "error", "data"] {
            assert!(
                body.get(key).is_some(),
                "clear-cache response is missing {key}: {body}"
            );
        }
        assert_eq!(body["ok"], true);
        assert_eq!(body["error"], Value::Null);

        // `groups` is required; an empty list is the valid "nothing to do" call,
        // which still has to answer with the success envelope.
        let body = json_body(
            delete_duplicates(State(state.clone()), None, Json(json!({"groups": []}))).await,
        )
        .await;
        for key in ["ok", "error", "data"] {
            assert!(
                body.get(key).is_some(),
                "delete-duplicates response is missing {key}: {body}"
            );
        }
    }

    /// Python reports the number of files it removed (`{"cleared": n}` in
    /// core/tools_api/ops.py::clear_cache). Rust used to answer `{"ok": true}`
    /// with no `cleared` key at all, so a caller could not tell an empty cache
    /// from a cleared one.
    #[tokio::test]
    async fn clear_cache_reports_how_many_files_it_removed() {
        let dir = tempfile::TempDir::new().unwrap();
        let config_path = dir.path().join("config.json");
        let state = test_state_with_config(&config_path, json!({})).await;

        // resolve_thumbnail_cache: <config dir>/thumbnails unless cache_dir is set.
        let cache = dir.path().join("thumbnails");
        std::fs::create_dir_all(&cache).unwrap();
        for name in ["a.webp", "b.webp", "c.webp"] {
            std::fs::write(cache.join(name), b"x").unwrap();
        }
        // A subdirectory must not be counted -- Python counts files only.
        std::fs::create_dir_all(cache.join("nested")).unwrap();

        let body = json_body(clear_cache(State(state.clone()), None).await).await;
        assert_eq!(body["cleared"], 3, "wrong cleared count: {body}");
        assert!(cache.exists(), "the cache directory must be recreated");

        // Second call: nothing left to clear.
        let body = json_body(clear_cache(State(state), None).await).await;
        assert_eq!(body["cleared"], 0, "second clear should report 0: {body}");
    }
    ...
    async fn register_path_rejects_files_outside_scan_roots() {
        ...
        assert_eq!(
            body,
            json!({"ok": false, "code": "outside_scan_root", "error": "path is not inside any configured scan root"})
        );
    }
    ...
    async fn register_path_rejects_unsupported_extensions() {
        ...
        for (name, error) in [
            ("file.txt", "unsupported extension: .txt"),
            ("file", "unsupported extension: (none)"),
            ("file.", "unsupported extension: (none)"),
        ] {
            ...
            assert_eq!(
                body,
                json!({"ok": false, "code": "unsupported_type", "error": error, "path": file.to_string_lossy(), "filename": name})
            );
        }
    }
