...
/// A shell handler that says WHICH feature is missing and why.
///
/// This replaced a bare `unavailable()` that answered every un-ported handler
/// in this file with the single word `unavailable`. The diagnostics page prints
/// the `error` field verbatim (`src/ts/diagnostics-page/index.ts:63`), so a user
/// who pressed "Verify update" on a standalone build was told nothing -- not
/// which feature was missing, and not that retrying could never help.
///
/// Status and envelope are unchanged (503 + `ok:false`), so callers that branch
/// on either still work. `code: "not_ported"` is the stable value for a UI to
/// match on, and is deliberately distinct from a runtime outage: the feature is
...
fn sns_config_value(config: &serde_json::Value) -> serde_json::Value {
    let mut value = json!({
        ...
        // Python's default (credential_store.py:11). Rust's "{title}\n{url}"
        // named placeholders post_builder.py:102 does not substitute, so an
        // unsaved template would have been posted with both braces intact.
        "post_template": "{positive_short}\n\nModel: {model}\n#AIart",
    });
...
#[derive(Deserialize)]
pub(crate) struct SnsPreviewQuery {
    file_id: Option<String>,
    template: Option<String>,
}
#[derive(Deserialize)]
pub(crate) struct SnsIntentQuery {
    file_id: Option<String>,
}

fn sns_file_id(file_id: Option<&str>) -> Option<i64> {
    file_id?.parse().ok().filter(|id: &i64| *id > 0)
}
fn char_short(text: &str, limit: usize) -> String {
...
fn truncate_graphemes(text: &str, limit: usize) -> String {
    ...
    for (index, ch) in text.char_indices() {
        ...
        if count > limit {
            return format!("{}...", text[..index].trim_end());
        }
    }
    text.to_string()
}
fn meta_text(value: &Value) -> String {
    value
        .as_str()
...
async fn sns_file_meta(
    s: &SharedState,
    file_id: i64,
) -> Result<Option<serde_json::Map<String, Value>>, sqlx::Error> {
    let row = sqlx::query("SELECT f.path, t.raw_prompt, t.raw_negative, t.raw_meta_json FROM files f LEFT JOIN templates t ON t.file_id = f.id WHERE f.id=? AND f.is_deleted=0")
        .bind(file_id).fetch_optional(&s.db_read).await?;
    let Some(row) = row else {
        return Ok(None);
    };
    let path: String = row.try_get::<Option<String>, _>(0)?.unwrap_or_default();
    let positive: String = row.try_get::<Option<String>, _>(1)?.unwrap_or_default();
    let negative: String = row.try_get::<Option<String>, _>(2)?.unwrap_or_default();
    let params = row
        .try_get::<Option<String>, _>(3)?
        .and_then(|raw| serde_json::from_str::<Value>(&raw).ok())
        .and_then(|value| value.as_object().cloned())
        .unwrap_or_default();
    let tags = sqlx::query("SELECT t.tag FROM file_tags ft JOIN tags t ON ft.tag_id=t.id WHERE ft.file_id=? ORDER BY (SELECT COUNT(*) FROM file_tags ft2 WHERE ft2.tag_id=t.id) DESC LIMIT 5")
        .bind(file_id).fetch_all(&s.db_read).await?.into_iter()
        .filter_map(|row| row.try_get::<Option<String>, _>(0).ok().flatten()).collect::<Vec<_>>().join(", ");
    let param = |lower, upper| {
        params
            .get(lower)
            .or_else(|| params.get(upper))
            .cloned()
            .unwrap_or_else(|| json!(""))
    };
    ...
    Ok(Some(serde_json::Map::from_iter([
        ("positive".into(), json!(positive)),
        ("positive_short".into(), json!(char_short(&positive, 100))),
        ("negative_short".into(), json!(char_short(&negative, 50))),
        ("model".into(), param("model", "Model")),
        ("seed".into(), json!(meta_text(&param("seed", "Seed")))),
        ("steps".into(), json!(meta_text(&param("steps", "Steps")))),
        (
            "cfg".into(),
            json!(meta_text(&param("cfg_scale", "CFG scale"))),
        ),
        ("sampler".into(), param("sampler", "Sampler")),
        ("size".into(), param("size", "Size")),
        ("tags".into(), json!(tags)),
        ("filename".into(), json!(filename)),
    ])))
}
fn sns_template(s: &SharedState) -> String {
    load_cfg(&s.config.config_path)
        .get("sns")
        .and_then(|sns| sns.get("post_template"))
        .and_then(serde_json::Value::as_str)
        .unwrap_or("{positive_short}")
        .to_string()
}
fn expand_sns_template(mut template: String, meta: &serde_json::Map<String, Value>) -> String {
    for (key, value) in meta {
        template = template.replace(&format!("{{{key}}}"), &meta_text(value));
    }
    template
}
...
mod sns_tests {
    ...
    const SNS_SCHEMA: &str = "
        CREATE TABLE files(id INTEGER PRIMARY KEY, path TEXT, is_deleted INTEGER DEFAULT 0);
        CREATE TABLE templates(file_id INTEGER PRIMARY KEY, raw_prompt TEXT, raw_negative TEXT, raw_meta_json TEXT);
    ...
    async fn sns_state(config: &str) -> (tempfile::TempDir, SharedState) {
        ...
        let pool = SqlitePoolOptions::new()
            ...
            .await
            .unwrap();
        sqlx::raw_sql(SNS_SCHEMA).execute(&pool).await.unwrap();
        let state = Arc::new(
            AppState::new(
                Config {
    ...
    /// `StatusCode::BAD_GATEWAY.into_response()` sends a 502 with no body and no
    /// Content-Type, so a client that parses JSON gets nothing to parse. Python's
    /// SD forwarder answers `openai_error("SD WebUI unavailable",
    /// "backend_unavailable", 502, "server_error")` as application/json
    /// (core/gateway/errors.py). The v4.732.21 parity run caught this as a
    /// content-type mismatch, not a body difference -- the body was empty.
    #[tokio::test]
    async fn sd_forwarder_502_is_json_shaped_like_pythons() {
        let response = sd_backend_unavailable();
        ...
        assert!(
            ...
            "a 502 with no content type cannot be parsed: {content_type:?}"
        );
        let body = response_json(response).await;
        assert_eq!(body["error"]["message"], "SD WebUI unavailable");
        assert_eq!(body["error"]["type"], "server_error");
        assert_eq!(body["error"]["code"], "backend_unavailable");
    }

    async fn seed_sns_file(state: &SharedState, positive: &str) {
        sqlx::query("INSERT INTO files(id, path, is_deleted) VALUES (1, '/library/pic.png', 0)")
            .execute(&state.db)
            .await
            .unwrap();
        sqlx::query("INSERT INTO templates(file_id, raw_prompt, raw_negative, raw_meta_json) VALUES (1, ?, 'bad', ?)")
            .bind(positive).bind(r#"{"model":"model-a","seed":7}"#).execute(&state.db).await.unwrap();
        sqlx::query("INSERT INTO tags(id, tag) VALUES (1, 'tag-a')")
            .execute(&state.db)
            .await
            .unwrap();
    ...
    #[tokio::test]
    async fn sns_handlers_reject_missing_file_id() {
        let (_root, state) = sns_state("{}").await;
        let preview = sns_preview(
            State(state.clone()),
            None,
            Query(SnsPreviewQuery {
                file_id: None,
                template: None,
            }),
        )
        .await;
        let intent =
            sns_x_intent(State(state), None, Query(SnsIntentQuery { file_id: None })).await;
        assert_eq!(preview.status(), StatusCode::BAD_REQUEST);
        assert_eq!(intent.status(), StatusCode::BAD_REQUEST);
    }

    #[tokio::test]
    async fn sns_preview_returns_not_found_for_unknown_file() {
        let (_root, state) = sns_state("{}").await;
        let response = sns_preview(
            State(state),
            None,
            Query(SnsPreviewQuery {
                file_id: Some("9".into()),
        ...
        assert_eq!(response_json(response).await["error"], "file_not_found");
    }

    #[tokio::test]
    async fn sns_preview_expands_template_for_seeded_file() {
        let (_root, state) = sns_state("{}").await;
        seed_sns_file(&state, "sunset").await;
        let response = sns_preview(
            State(state),
            None,
            Query(SnsPreviewQuery {
                file_id: Some("1".into()),
                template: Some("{positive_short} #{model} {seed} {tags} {filename}".into()),
            }),
        )
        .await;
        assert_eq!(response.status(), StatusCode::OK);
        let body = response_json(response).await;
        assert_eq!(body["text"], "sunset #model-a 7 tag-a pic.png");
    }

    #[test]
    fn sns_grapheme_counter_matches_python_mark_rule() {
        assert_eq!(count_graphemes("e\u{301}がか"), 3);
        assert_eq!(count_graphemes("a\u{0903}"), 1);
        assert_eq!(count_graphemes("a\u{20dd}"), 1);
    }

    #[tokio::test]
    async fn sns_preview_truncates_multibyte_prompt_at_character_boundary() {
        let (_root, state) = sns_state("{}").await;
        seed_sns_file(&state, &"あ".repeat(101)).await;
        let response = sns_preview(
            State(state),
            None,
            Query(SnsPreviewQuery {
                file_id: Some("1".into()),
                template: Some("{positive_short}".into()),
            }),
        )
        .await;
        let text = response_json(response).await["text"]
            .as_str()
            .unwrap()
            .to_string();
        assert_eq!(text, format!("{}...", "あ".repeat(100)));
    }

    #[tokio::test]
    async fn sns_intent_uses_form_encoding_and_truncates() {
        let (_root, state) = sns_state(r#"{"sns":{"post_template":"{positive}"}}"#).await;
        seed_sns_file(&state, &format!("hello world {}", "x".repeat(201))).await;
        let response = sns_x_intent(
            State(state),
            None,
            Query(SnsIntentQuery {
                file_id: Some("1".into()),
            }),
        )
        .await;
        ...
        let url = response_json(response).await["url"]
            ...
            .unwrap()
            .to_string();
        assert!(url.starts_with("https://x.com/intent/tweet?text=hello+world+"));
...
pub async fn search_union(
    ...
    not_ported("Cross-collection union search")
}
...
pub async fn debug_query(
    ...
    not_ported("The SQL debug console")
}
...
pub async fn workflow_gen_params(
    ...
    not_ported("Workflow generation parameters")
}
...
pub async fn sns_preview(
    State(s): State<SharedState>,
    auth: Option<Extension<AuthContext>>,
    Query(query): Query<SnsPreviewQuery>,
) -> Response {
    if let Some(r) = gate(&s, auth.as_ref()) {
        return r;
    }
    let Some(file_id) = sns_file_id(query.file_id.as_deref()) else {
        return (
            StatusCode::BAD_REQUEST,
            Json(json!({"error": "file_id is required"})),
        )
            .into_response();
    };
    let meta = match sns_file_meta(&s, file_id).await {
        Ok(Some(meta)) => meta,
        Ok(None) => {
            return (
        ...
        Err(error) => {
            tracing::error!(?error, "failed to read SNS post metadata");
            return (
                StatusCode::INTERNAL_SERVER_ERROR,
    ...
    let text = expand_sns_template(query.template.unwrap_or_else(|| sns_template(&s)), &meta);
    Json(json!({"text": text, "graphemes": count_graphemes(&text), "meta": meta})).into_response()
}
...
pub async fn sns_x_intent(
    State(s): State<SharedState>,
    auth: Option<Extension<AuthContext>>,
    Query(query): Query<SnsIntentQuery>,
) -> Response {
    if let Some(r) = gate(&s, auth.as_ref()) {
        return r;
    }
    ...
    let text = truncate_graphemes(&expand_sns_template(sns_template(&s), &meta), 200);
    let query = url::form_urlencoded::Serializer::new(String::new())
        .append_pair("text", &text)
        .finish();
    Json(json!({"url": format!("https://x.com/intent/tweet?{query}")})).into_response()
}
...
pub async fn sns_bluesky_test(
    ...
    not_ported("Testing the Bluesky connection")
}
...
pub async fn sns_bsky_queue(
    ...
    not_ported("The Bluesky post queue")
}
...
pub async fn sns_bsky_queue_pending(
    ...
    not_ported("Listing pending Bluesky posts")
}
...
pub async fn sns_config_get(
    ...
    // Python's `api_result` (routes/sns_share.py:116) spreads the payload at the TOP
    // level and leaves `data` null. Nesting it under `data` meant a client reading
    // `json.<field>` got nothing from Rust -- the same dead read mesh-inference/state
    // had. Seventh site of this envelope; `api_envelope` owns the shape since v4.736.1.
    super::api_envelope::api_ok(sns_config_value(&s.config.app_config)).into_response()
}
...
pub async fn sns_config_post(
    ...
    not_ported("Saving SNS settings")
}
...
pub async fn collections_export(
    ...
    let Some(cname) = name else {
        return (
            StatusCode::NOT_FOUND,
            // Python answers "collection not found" with a space
            // (routes/collections.py:110); measured as the only differing key at
            // 404/404 in v4.737.9.
            Json(json!({"ok": false, "error": "collection not found"})),
        )
            .into_response();
    };
...
mod llm_agent_tests {
    ...
    /// A shell handler must say WHICH feature is missing, not just "unavailable".
    ///
    /// The diagnostics page prints the `error` field verbatim
    /// (`src/ts/diagnostics-page/index.ts:63`), so before v4.711.0 a user who
    /// pressed "Verify update" on a standalone build saw the single word
    /// `unavailable` -- no hint that the feature is absent from this runtime
    /// rather than temporarily down, and nothing to act on.
    ///
    /// This pins the parts a caller can depend on: the 503, the `ok:false`
    /// envelope (both unchanged, so existing branches still work), the stable
    /// `code`, and that the message actually names the feature.
    #[tokio::test]
    async fn update_shells_name_the_missing_feature() {
        let state = test_state(0).await;
        let cases: Vec<(&str, Response)> = vec![
            (
                "system update",
                super::system_update_apply(State(state.clone()), None).await,
            ),
            (
                "unified update",
                super::system_update_unified_apply(State(state.clone()), None).await,
            ),
            (
                "verify",
                super::update_verify(State(state.clone()), None).await,
            ),
            (
                "apply",
                super::update_apply(State(state.clone()), None).await,
            ),
            (
                "rollback",
                super::update_rollback(State(state.clone()), None).await,
            ),
        ];
        for (label, response) in cases {
            assert_eq!(
                response.status(),
                StatusCode::SERVICE_UNAVAILABLE,
                "{label}: status must stay 503 so existing callers are unaffected"
            );
            ...
            assert_eq!(
                body["code"], "not_ported",
                "{label}: the UI needs a stable code to match on"
            );
            let message = body["error"].as_str().unwrap_or_default();
            assert!(
                message.len() > "unavailable".len(),
                "{label}: the message must explain, not just say 'unavailable': {message:?}"
            );
            assert!(
                message.contains("Rust server"),
                "{label}: the message must say which runtime lacks it: {message:?}"
            );
        }
    }
}
...
/// Mirror of `core/gateway/errors.py::openai_error` for the SD forwarder's
/// unreachable-backend case. `StatusCode::BAD_GATEWAY.into_response()` sends a
/// 502 with no body and no Content-Type at all, so a client that parses JSON gets
/// nothing to parse; Python sends the OpenAI-shaped error as application/json.
fn sd_backend_unavailable() -> Response {
    (
        StatusCode::BAD_GATEWAY,
        Json(json!({"error": {
            "message": "SD WebUI unavailable",
            "type": "server_error",
...
async fn fwd_get_sd(state: &SharedState, path: &str) -> Response {
    ...
    match state.python_client.get(&url).send().await {
        Ok(r) => {
            ...
            r.bytes()
                .await
                .map_or_else(|_| sd_backend_unavailable(), |b| (s, b).into_response())
        }
        Err(_) => sd_backend_unavailable(),
    }
}
...
pub async fn gateway_keys(
    ...
    let auth_cfg = crate::auth::gateway::auth_config(&cfg);
    let keys = crate::auth::gateway::list_keys(&auth_cfg, &s.config.project_root);
...
pub async fn router_capabilities_target(
    ...
    not_ported("LLM router capability targeting")
}
...
pub async fn system_update_apply(
    ...
    not_ported("Applying a system update")
}
...
pub async fn system_update_unified_apply(
    ...
    not_ported("Applying a unified update")
}
...
pub async fn update_verify(
    ...
    not_ported("Verifying a signed update package")
}
...
pub async fn update_apply(
    ...
    not_ported("Applying a signed update package")
}
...
pub async fn update_rollback(
    ...
    not_ported("Rolling back to the previous update")
}
...
fn scan_to_json(
    ...
    use meta_extract::{parse_metadata, read_exif_tags, read_png_text_chunks};
    use tagdb_core::import::fallback_chain::exif_chunks_from_tags;
    ...
    let (chunks, raw_metadata) = if ext == ".png" {
        ...
        // `raw_metadata` keeps the tags as read, for the panel that shows them.
        let raw: serde_json::Map<String, serde_json::Value> = exif
            .iter()
            .map(|(k, v)| (k.clone(), json!(v.chars().take(500).collect::<String>())))
            .collect();
        // The parsers look for `Comment`, so feed them the same mapping the real
        // scanner uses. Prefixing every key with `exif:` -- what this did before
        // -- meant no parser ever matched and every NAI WebP came back as
        // `format=unknown`.
        (exif_chunks_from_tags(exif), serde_json::Value::Object(raw))
    };
