...
async fn run_doctor_job(state: SharedState, job_id: String) {
    ...
    let report_dir = doctor_report_dir(&state.config.project_root);
...
/// Doctor report output directory.
///
/// `YU_DOCTOR_REPORT_DIR` overrides it so a caller that must not write into
/// the checkout (the parity harness runs with cwd=repo root) can redirect the
/// side effect instead of skipping the route entirely. Python honours the same
/// variable in `routes/diagnostics.py::_run_doctor_job`; the two must stay in
/// step or the harness silently compares one server that wrote and one that
/// did not.
fn doctor_report_dir(project_root: &Path) -> PathBuf {
    match std::env::var_os("YU_DOCTOR_REPORT_DIR") {
        Some(dir) if !dir.is_empty() => PathBuf::from(dir),
        _ => project_root.join("reports"),
    }
}
...
/// Shared with `mcp::diagnostics::check_update_pending`, so the doctor's
/// reading of a residual's `created_at` cannot drift from the cleanup route's.
