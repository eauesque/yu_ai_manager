...
fn invalid_value(message: String) -> Response {
    (
        ...
        Json(json!({"ok": false, "error": message, "code": "invalid_value"})),
    )
        .into_response()
}
...
pub async fn tag_file(
    ...
    if let Some(err) = result.get("error") {
        ...
        // `tag_one` classifies its failures ("disabled", "not_configured",
        // "file_not_found", "db_error", ...) and this used to throw that away,
        // answering with a bare message and a `data: null` Python never sends.
        // Clients branch on `code` -- routes/hailo_tagger.py passes it through
        // as `code=result.get("code", "tag_error")` -- so against Rust every
        // failure looked alike and no caller could tell "Hailo is switched off"
        // from "that file is gone". Found by un-suppressing the body
        // comparison on POST /api/hailo-tagger/tag/{file_id} (v4.732.12).
        let code = result
            .get("code")
            .and_then(Value::as_str)
            .unwrap_or("tag_error");
        return (
            status,
            Json(json!({"ok": false, "error": err, "code": code})),
        )
            .into_response();
    }
    api_result(result)
...
pub async fn batch(
    ...
    let file_ids: Option<Vec<i64>> = match data.get("file_ids") {
        ...
        Some(Value::Array(arr)) => {
            ...
            match ids {
                ...
                None => {
                    return (
                        StatusCode::BAD_REQUEST,
                        Json(json!({"ok":false,"error":"file_ids must be a list of integers"})),
                    )
                        .into_response()
                }
            }
        ...
        _ => {
            return (
                StatusCode::BAD_REQUEST,
                Json(json!({"ok":false,"error":"file_ids must be a list"})),
            )
                .into_response()
        }
    };
    ...
    if let Some(ref ids) = file_ids {
        if ids.len() > 500 {
            return (
                StatusCode::BAD_REQUEST,
                Json(json!({"ok":false,"error":"file_ids max 500","code":"batch_too_large"})),
            )
                .into_response();
        }
    }
    ...
    if state.job_manager.is_running("hailo_tagger") {
        return (
            StatusCode::CONFLICT,
            Json(json!({"ok":false,"error":"Hailo Tagger batch is already running","code":"job_running"})),
        )
            .into_response();
    }
