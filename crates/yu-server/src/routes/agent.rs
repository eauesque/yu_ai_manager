...
/// Current Kill Switch state, read from the flag file.
///
/// Mirrors Python `AgentKillSwitch.status()`: the flag file's existence *is*
/// the state, and its contents are the reason. There is no in-memory copy to
/// consult -- a kill issued by the Python MCP subprocess, or by a previous run
/// of this server, is visible only here. `killed_at` is the file's mtime, which
/// is the only timestamp either process records.
///
/// Returned as a `Value` in the same shape `write_kill_flag`/`remove_kill_flag`
/// return, so every caller sees one shape regardless of who last changed it.
///
/// Takes the resolved path (not `project_root`), like its write/remove
/// siblings, so the tests never depend on `TAGDB_DATA_DIR` resolution.
fn read_kill_flag(flag_path: &Path) -> Value {
    let Ok(metadata) = std::fs::metadata(flag_path) else {
        return json!({"killed": false, "reason": "", "killed_at": ""});
    };
    let reason = std::fs::read_to_string(flag_path).unwrap_or_default();
    let killed_at = metadata
        .modified()
        .ok()
        .map(|t| chrono::DateTime::<Utc>::from(t).to_rfc3339())
        .unwrap_or_default();
    json!({"killed": true, "reason": reason, "killed_at": killed_at})
}

pub(crate) fn kill_switch_status(project_root: &Path) -> Value {
    read_kill_flag(&kill_flag_path(project_root))
}
...
mod tests {
    ...
    #[test]
    fn status_reads_a_kill_this_process_did_not_issue() {
        // The case the /api/agent/status stub got wrong: the flag was written by
        // someone else (the Python MCP subprocess, or an earlier run), so there
        // is no in-memory state to consult. Reporting `killed: false` here tells
        // the operator the agent is running while it is stopped.
        let dir = temp_dir();
        let flag = dir.join("agent_kill.flag");
        std::fs::write(&flag, "killed by the MCP subprocess").unwrap();

        let status = read_kill_flag(&flag);

        assert_eq!(status["killed"], true);
        assert_eq!(status["reason"], "killed by the MCP subprocess");
        assert!(!status["killed_at"].as_str().unwrap().is_empty());
        let _ = std::fs::remove_dir_all(&dir);
    }

    #[test]
    fn status_reports_not_killed_when_no_flag_exists() {
        let dir = temp_dir();
        let status = read_kill_flag(&dir.join("agent_kill.flag"));
        assert_eq!(status["killed"], false);
        assert_eq!(status["reason"], "");
        assert_eq!(status["killed_at"], "");
        let _ = std::fs::remove_dir_all(&dir);
    ...
    #[test]
    fn status_matches_what_kill_and_resume_just_did() {
        // One shape for all three: a caller must not have to know which of the
        // three produced the value it is reading.
        let dir = temp_dir();
        let flag = dir.join("agent_kill.flag");

        write_kill_flag(&flag, "boom");
        let after_kill = read_kill_flag(&flag);
        assert_eq!(after_kill["killed"], true);
        assert_eq!(after_kill["reason"], "boom");

        remove_kill_flag(&flag);
        assert_eq!(read_kill_flag(&flag), remove_kill_flag(&flag));
