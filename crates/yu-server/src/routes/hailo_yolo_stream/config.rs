...
fn restrict_owner_only(path: &Path) -> io::Result<()> {
    ...
    let result = (|| {
        ...
        #[allow(
            ...
            reason = "acl_bytes is ACL+ACE header sizes plus one SID length, always far below u32::MAX"
        )]
    ...
    fn real_app_state_is_inactive_for_all_startup_corpora() {
        ...
        run_bounded_test(TEST_TIMEOUT, async {
            ...
            for (name, content, mode, expected_sources) in cases {
                ...
                // Startup rewrites a plaintext config in the background. A flat
                // 20ms sleep here raced that write: on a loaded machine the
                // rewrite had not landed yet and the assertion below blamed the
                // migration ("plaintext should be migrated") for a write that
                // was merely late. Wait for the outcome instead of a duration.
                //
                // The budget was 2s and still went red once under a concurrent
                // `pre_push` (measured 8/9 in v4.698.11: the only failure was
                // the second pre_push run, i.e. the machine under load). The
                // migration is one file write, so a longer wait costs nothing
                // when it is not needed; two waiting corpora at 5s stay inside
                // the 20s TEST_TIMEOUT even if both time out.
                //
                // The corpora that must NOT be rewritten cannot be waited for --
                // absence never arrives -- so those keep a fixed settle window,
                // which is what makes a stray write observable at all.
                let mut timed_out = false;
                match &before {
                    Some((bytes, _)) if expected_sources > 0 => {
                        let budget = Duration::from_secs(5);
                        let deadline = std::time::Instant::now() + budget;
                        while fs::read(&path).ok().as_ref() == Some(bytes) {
                ...
                match before {
                    ...
                    Some((bytes, modified)) if expected_sources > 0 => {
                        ...
                        // Say WHICH failure this is. "plaintext should be
                        // migrated" reads as "the migration is broken", but the
                        // same assertion fires when the rewrite was merely still
                        // in flight -- and that is the one that actually happened
                        // (twice, both under load). A reader who cannot tell the
                        // two apart re-debugs the migration every time.
                        assert!(
                            !timed_out,
                            "{name}: the config was still byte-identical after \
                             waiting 5s for the startup migration. The secret \
                             store probe above passed, so the key is fine -- this \
                             is a late write, not a broken migration. If this \
                             recurs on an idle machine, suspect the migration; \
                             under load, suspect the budget."
                        );
