...
pub(super) mod tests {
        ...
        for repository in std::fs::read_dir(checkouts).map_err(|error| error.to_string())? {
                ...
                std::fs::read_dir(repository.path()).map_err(|error| error.to_string())?
            {
                ...
                let revision = Command::new("git")
                    // `GIT_DIR` and friends outrank `-C` when git picks a
                    // repository, and a git hook exports them: run under
                    // `.githooks/pre-push` this asked the checkout for its HEAD
                    // and got yu_ai_manager's instead, which never equals the
                    // pin, so every checkout looked wrong and the lookup
                    // reported "not found". Passes standalone, failed every
                    // time under the hook -- deterministic, not a flake.
                    .env_remove("GIT_DIR")
    ...
    /// Where cargo keeps its git checkouts.
    ///
    /// Reads the live environment. Callers must hold
    /// `crate::ENV_MUTATION_TEST_LOCK` first: several tests in this same binary
    /// hijack `HOME` process-wide (`routes::auto_stubs` and `mcp::diagnostics`
    /// set it to `/home/pi` and `/home/testuser`, `routes::hailo_model_registry`
    /// to a tmpdir), and every one of those writers takes that lock. A reader
    /// that skips it can resolve `~/.cargo` to `/home/pi/.cargo` mid-run.
    fn cargo_checkouts() -> PathBuf {
    ...
    /// The lookup must survive a git hook's environment.
    ///
    /// `.githooks/pre-push` runs the suite with `GIT_DIR` exported, and that
    /// variable outranks `-C`, so the child `git` answered for yu_ai_manager
    /// instead of the checkout. Deterministic under the hook and invisible
    /// outside it, which is why three clean manual runs said nothing.
    #[test]
    fn pinned_lookup_ignores_an_inherited_git_dir() {
        // This test is itself an env writer (it sets GIT_DIR), so it takes the
        // same seam lock the other writers take.
        ...
        let checkouts = cargo_checkouts();

        // Point GIT_DIR at this repository, exactly as the hook does.
        let repo_git_dir = crates.parent().unwrap().join(".git");
        let restore = std::env::var_os("GIT_DIR");
        std::env::set_var("GIT_DIR", &repo_git_dir);
        let found = pinned_router(&cargo_toml, &checkouts);
        match restore {
        ...
        assert!(
            found.is_ok(),
            "an inherited GIT_DIR redirected the checkout lookup: {found:?}"
        );
    }

    #[test]
    fn pinned_yu_infer_checkout_exposes_metadata_frame_size_contract() {
        // Resolves `~/.cargo`, so it must not run while another module holds
        // HOME hijacked. The guard is held for the whole bounded test.
        let _guard = crate::ENV_MUTATION_TEST_LOCK
            .lock()
            .unwrap_or_else(|e| e.into_inner());
        let checkouts = cargo_checkouts();
