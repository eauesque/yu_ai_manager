...
pub async fn open_folder(
    ...
    // Python: api_result(open_folder_for_file(...)) whose payload is
    // {"success": True, "path": ...} -- no `data` key, so api_success spreads it at
    // the top level and leaves `data` null.
    Json(json!({"ok":true,"error":null,"data":null,"success":true,"path":dir_str})).into_response()
}
...
mod tests {
    ...
    async fn extract_from_zip_rejects_dotdot_traversal() {
        ...
        assert_eq!(
            value,
            json!({"ok": false, "error": "Path traversal blocked", "code": "zip_path_traversal"})
        );
    }
    ...
    async fn extract_from_zip_rejects_corrupted_zip() {
        ...
        assert_eq!(
            value,
            json!({"ok": false, "error": "ZIPファイルが破損しているため解凍できません", "code": "bad_zip_file"})
        );
    }
    ...
    async fn extract_from_zip_rejects_nonexistent_entry() {
        ...
        assert_eq!(
            value,
            json!({"ok": false, "error": "ZIP内に対象ファイルが見つかりません", "code": "zip_entry_not_found", "hint": "再スキャンでZIP内エントリを更新してください"})
        );
    }
    ...
    async fn extract_from_zip_rejects_symlink_escape_via_post_extraction_check() {
        ...
        {
            if let Err(error) =
                std::os::windows::fs::symlink_dir(outside.path(), extract_dir.join("link"))
            {
                // Creating a symlink on Windows requires Administrator
                // privilege or Developer Mode; without either this is not a
                // code bug but an environment limitation.
                assert_eq!(
                    error.raw_os_error(),
                    Some(1314),
    ...
    async fn extract_from_zip_rejects_path_without_bang() {
        ...
        assert_eq!(
            value,
            json!({"ok": false, "error": "アーカイブ内ファイルではありません", "code": "not_zip_member"})
        );
    }
    ...
    async fn extract_from_zip_rejects_missing_file_id() {
        ...
        assert_eq!(
            value,
            json!({"ok": false, "error": "file_id is required", "code": "missing_file_id"})
        );
    }
}
