...
fn list_rar_entries(archive: &FsPath) -> Option<Vec<RarEntry>> {
    unrar::Archive::new(archive)
        ...
        .map(|entry| {
            let entry = entry.ok()?;
            Some(RarEntry {
                // unrar returns `filename` through a native PathBuf, so on
                // Windows internal RAR directory separators come back as
                // `\` -- unlike every other archive path in this codebase
                // (zip/7z members, `virtual_path`), which is always `/`.
                // Normalize here, at the single point paths leave the
                // unrar crate, rather than at each of this function's
                // callers.
...
async fn cached_rar_member(
    ...
    let bytes = tokio::task::spawn_blocking(move || {
        ...
        while let Some(header) = reader.read_header().ok()? {
            ...
            // Must match list_rar_entries' normalization: `target` came from
            // a `RarEntry.path` (already `/`-normalized), but this raw
            // re-read of the archive gets unrar's native-separator filename
            // again.
            if !entry.is_directory()
                && entry.filename.to_string_lossy().replace('\\', "/") == target
            {
...
mod tests {
    ...
    async fn thumbnails_warmup_populates_preview_cache() {
        ...
        tokio::time::timeout(std::time::Duration::from_secs(10), async {
            // Sleep rather than `yield_now`: the latter spins this task at
            // full speed for the whole wait, and on a loaded machine that
            // steals the very CPU the encode it is waiting for needs -- the
            // test then times out on work it starved itself.
