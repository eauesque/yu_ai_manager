...
pub async fn approval_history(
    ...
    // Python answers `api_result({"history": ...})` and nothing else -- no `count`.
    // Measured 200/200 in v4.734.24: with the envelope repaired, `count` was the only
    // remaining difference, and no caller in the tree reads it.
    super::api_envelope::api_ok(json!({"history": history})).into_response()
}
