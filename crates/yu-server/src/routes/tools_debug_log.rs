...
///
/// The two branches differ in Python and the difference is observable: an
/// explicit `TAGDB_DEBUG_LOG` is used verbatim (`Path(raw)`), while the log
/// *directory* -- env-provided or the `cwd/logs` default -- goes through
/// `init_app_paths`'s `.resolve()` and always comes out absolute. `log_path`
/// is echoed in this route's body, so leaving ours relative is a visible
/// divergence, not just an internal one.
fn debug_log_path() -> PathBuf {
    ...
    let dir = if log_dir.trim().is_empty() {
        PathBuf::from("logs")
    } else {
        PathBuf::from(log_dir.trim())
    };
    std::path::absolute(&dir).unwrap_or(dir).join("debug.log")
}
...
mod tests {
    ...
    fn the_log_path_follows_pythons_precedence() {
        ...
        temp_env(
            ...
            || {
                // Not a full-path comparison: `std::path::absolute` makes a
                // Unix-rooted-but-driveless input like "/tmp/dir" absolute by
                // prefixing the current drive on Windows (e.g. "O:\tmp\dir"),
                // so there is no platform-independent literal to assert here.
                // The property under test -- explicit dir wins and comes out
                // absolute -- still holds.
                let path = debug_log_path();
                assert!(path.is_absolute(), "{path:?} should be absolute");
                assert!(path.ends_with("dir/debug.log"), "{path:?}");
            },
        );
        temp_env(
            &[("TAGDB_DEBUG_LOG", None), ("TAGDB_LOG_DIR", None)],
            || {
                // Python's log dir goes through `.resolve()`, so its
                // `log_path` in the response body is absolute. A relative
                // "logs/debug.log" here would be a body difference.
                let path = debug_log_path();
                assert!(path.is_absolute(), "{path:?} should be absolute");
                assert_eq!(
                    path,
        ...
        temp_env(
            &[
                ("TAGDB_DEBUG_LOG", None),
                ("TAGDB_LOG_DIR", Some("rel_logs")),
            ],
            || {
                assert!(
                    debug_log_path().is_absolute(),
                    "a relative TAGDB_LOG_DIR must still come out absolute"
                );
            },
        );
    }
