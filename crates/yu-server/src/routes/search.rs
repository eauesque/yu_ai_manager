...
pub struct SearchParams {
    ...
    pub wd_model: String,
    pub also_path: bool,
    /// Whether `files_path_fts` exists. Gates the MATCH branch only: the
    /// short-term LIKE fallback reads `files` and needs no FTS table, and Python
    /// takes that branch whether or not one is present.
    pub path_fts_available: bool,
    pub fav_only: bool,
    ...
    fn from(r: SearchQueryRaw) -> Self {
        ...
        SearchParams {
            ...
            wd_model: first_alias(&[r.wd_model.as_deref(), r.wd_tagger_model.as_deref()])
                ...
                .trim()
                .to_string(),
            // Assumed present; the `search` handler proves it per request. Left true
            // for the handlers that never checked, so their behaviour is unchanged.
            path_fts_available: true,
...
/// Would Python serve this search from its in-memory cache?
///
/// Port of `can_use_cache` (core/search_api/file_meta_cache_rules.py:181-221). It is
/// mirrored because the two Python payload builders disagree on shape: the cache path
/// omits `count_pending`, the SQL path includes it. Rust has no such cache, so this
/// predicate exists only to answer with the same KEY SET Python would.
fn python_would_use_cache(p: &SearchParams) -> bool {
    let text_filters = [
        p.tag_query.as_str(),
        p.artist.as_str(),
        p.in_prompt.as_str(),
        p.in_negative.as_str(),
        p.in_char_negative.as_str(),
        p.in_char_positive.as_str(),
        p.checkpoint_filter.as_str(),
        p.or_tags.as_str(),
        // wd_model has no representation in Python's cache records either.
        p.wd_model.as_str(),
    ];
    if text_filters.iter().any(|v| !v.trim().is_empty()) {
        return false;
    }
    if p.fav_only
        || p.collection_id != 0
    ...
    matches!(
        p.sort_by.as_str(),
        "date" | "date_new" | "date_old" | "path" | "random"
    )
}

/// The prompt pair for one row, exactly as Python resolves it.
///
/// `core/search_api/search_rows.py:29-36` and `_normalize_prompt`
/// (search_response_helpers.py:51-66) apply the same rule, and so do both callers
/// here: parse when the A1111 markers are present, and take the PARSED negative only
/// when the stored one is empty -- never in place of it.
fn resolve_prompt_pair(raw_prompt: Option<&str>, raw_negative: Option<&str>) -> (String, String) {
    let Some(rp) = raw_prompt else {
        return (String::new(), String::new());
    };
    let stored_negative = raw_negative.unwrap_or_default().to_string();
    if rp.contains("Steps:") || rp.contains("Negative prompt:") {
        let (parsed_positive, parsed_negative) = parse_a1111_prompt(rp);
        let negative = if stored_negative.is_empty() && !parsed_negative.is_empty() {
            parsed_negative
        } else {
            stored_negative
        };
        (parsed_positive, negative)
    } else {
        (rp.to_string(), stored_negative)
    }
}
...
enum WdModelScope {
    /// No wd_model override — WD tags are not matched at all (neither the
    /// tag_query OR clause nor a standalone filter). Mirrors Python's
    /// `apply_wd_model_filter`, which is a no-op when `wd_model` is empty
    /// regardless of any "active" WD model setting -- that setting only
    /// scopes the separate `ai_analyzed` filter, not a plain/default search.
    Unset,
    /// wd_model="all" — match tags/files from any model.
    Any,
    /// wd_model=<name> resolved to this db id.
    Model(i64),
...
async fn resolve_wd_tag_search_model(db: &sqlx::SqlitePool, wd_model_param: &str) -> WdModelScope {
    ...
    if !wd_model_param.is_empty() {
        return match resolve_wd_model_db_id(db, wd_model_param).await {
            ...
            // A name that is not in `wd_model_dict` matches nothing -- that is
            // the caller's filter, honoured exactly.
            Ok(None) => WdModelScope::NoMatch,
            // A lookup FAILURE is not the same thing, even though it lands in
            // the same scope. Spelling the arms apart keeps the two from being
            // conflated by the next edit, and gives the failure a voice: a db
            // error here silently narrows the user's search to zero rows, and
            // without this log there is nothing to tell them why.
            Err(error) => {
                tracing::warn!(%error, model = wd_model_param, "wd_model lookup failed; the search will match nothing");
                WdModelScope::NoMatch
            }
        };
    }
    // No wd_model param given: stay Unset. Do NOT fall back to the "active"
    // WD model here -- that silently turned every unconditioned search
    // (has_conditions() == false) into "only files WD-tagged by the active
    // model", hiding any file the tagger backlog hadn't reached yet. Python
    // never does this outside the opt-in ai_analyzed path (core/query/
    // filters_tags_apply_helpers.py::apply_wd_model_filter is a no-op on an
    // empty wd_model). The active-model lookup for ai_analyzed already
    // happens separately via active_wd_model_id() in search()/search_count().
    WdModelScope::Unset
}
...
fn push_where_filters(
    ...
    if !p.tag_query.is_empty() && !p.tag_query_regex {
        ...
        if let Some(phrase) = fts_phrase.filter(|_| p.path_fts_available) {
            qb.push(" OR f.id IN (SELECT rowid FROM files_path_fts WHERE path MATCH ")
                .push_bind(phrase)
                .push(")");
        } else if p.also_path {
            // A term under 3 characters cannot be indexed by the trigram tokenizer,
            // so there is no MATCH phrase -- but Python does not drop the path
            // widening for it. `path_search_condition`
            // (core/query/filters_tags_path.py:21) falls back to the folded LIKE
            // pair, reached from `_apply_included_tag` (filters_tags_apply_helpers.py
            // :101-105) with no phrase gating. Dropping it here made a 1-2 character
            // path search answer with nothing where Python answered with the matches.
            //
            // Not escaped, and no ESCAPE clause: Python binds the pattern raw, so `%`
            // and `_` in the term are wildcards there. The second pattern is the same
            // one with backslashes turned into forward slashes, which is how a
            // Windows-shaped term finds a path stored the other way round.
            let folded = tagdb_core::db::custom_functions::nfkc_lower(&p.tag_query);
            let forward = folded.replace('\\', "/");
            qb.push(" OR (nfkc_lower(f.path) LIKE ")
                .push_bind(format!("%{folded}%"))
                .push(" OR nfkc_lower(f.path) LIKE ")
                .push_bind(format!("%{forward}%"))
                .push(")");
        }
        // Widening the tag_query OR-clause with WD-tagger tags falls back to
        // the active model when no explicit wd_model was given -- this
        // mirrors Python's _wd_tag_exists_condition (core/query/
        // filters_tags_apply_helpers.py), which resolves the active model
        // specifically for this per-tag OR match. That fallback is
        // deliberately NOT shared with the standalone wd_model AND filter
        // below (push_wd_model_filter): Python's apply_wd_model_filter is a
        // no-op when wd_model is empty, with no active-model fallback, so an
        // unconditioned search isn't silently narrowed to "tagged by the
        // active model only".
        let or_clause_model = match wd_search_model {
            WdModelScope::Unset => match active_wd {
                Some(id) => WdModelScope::Model(id),
                None => WdModelScope::Unset,
            },
            // Spelled out rather than `other => other`: a new WdModelScope
            // variant must be considered here, not silently carried through.
            // The whole point of this block is that Unset alone gets the
            // active-model fallback, so a variant added later needs a decision.
            WdModelScope::Any => WdModelScope::Any,
            WdModelScope::Model(id) => WdModelScope::Model(id),
            WdModelScope::NoMatch => WdModelScope::NoMatch,
        };
        push_wd_tag_or_clause(qb, &p.tag_query, or_clause_model);
        qb.push(")");
    }
    ...
    if include_templates {
        if !p.in_prompt.is_empty() && !p.in_prompt_regex {
            qb.push(" AND tm.raw_prompt LIKE ")
                .push_bind(like_sub(&p.in_prompt))
                .push(" ESCAPE '\\'");
        }
        if !p.in_negative.is_empty() {
            qb.push(" AND tm.raw_negative LIKE ")
    ...
    if !p.in_path.is_empty() {
        ...
        qb.push(" AND LOWER(REPLACE(f.path, '\\', '/')) LIKE ")
            .push_bind(pat)
            .push(" ESCAPE '\\'");
    }
        ...
        if let Some(phrase) = fts_phrase {
            qb.push(
                " AND (EXISTS(SELECT 1 FROM templates tm2 WHERE tm2.file_id=f.id \
                 ...
                 AND (lower(tm2.model_name) LIKE ",
            )
            .push_bind(like_pat.clone())
            .push(" OR lower(tm2.model_hash) LIKE ")
            .push_bind(like_pat)
            .push(
                ")) OR f.id IN (\
...
pub async fn search(
    ...
    if p.also_path
        && !table_exists(&state.db_read, "files_path_fts")
            ...
            .unwrap_or(false)
    {
        // NOT `also_path = false`: that also killed the short-term LIKE fallback,
        // which needs no FTS table. Only the MATCH branch depends on it.
        p.path_fts_available = false;
    }
    ...
    let mut results: Vec<serde_json::Value> = rows
        ...
        .map(|row| {
            ...
            // Mirror of `core/search_api/search_rows.py:14-46`, via the shared helper.
            let (positive, negative) =
                resolve_prompt_pair(raw_prompt.as_deref(), raw_negative.as_deref());

            json!({
                "id": id,
                "path": path,
                "mtime": mtime,
                // Passed through, NOT defaulted: Python returns the column as it is,
                // so a NULL stays null rather than becoming an empty string.
    ...
    // Port of `backfill_prompts` (core/search_api/search_response_helpers.py:26-48),
    // which Python calls unconditionally at search_response.py:224. Without it a
    // search that filters on no prompt skips the templates JOIN and answers with blank
    // prompts -- measured as `results[0].positive` / `.negative` against Python.
    if !include_templates && !results.is_empty() {
        let missing: Vec<i64> = results
            .iter()
            .filter(|r| r["positive"].as_str().unwrap_or_default().is_empty())
            .filter_map(|r| r["id"].as_i64())
            .collect();
        // Guarded, unlike Python, which would raise on a database with no templates
        // table. Skipping leaves the prompts blank -- the same answer as before.
        if !missing.is_empty()
            && table_exists(&state.db_read, "templates")
                .await
                .unwrap_or(false)
        {
            let mut prompts: std::collections::HashMap<i64, (String, String)> =
                std::collections::HashMap::new();
            // Chunked for the same reason Python chunks: SQLITE_MAX_VARIABLE_NUMBER.
            for chunk in missing.chunks(400) {
                let placeholders = vec!["?"; chunk.len()].join(",");
                let sql = format!(
                    "SELECT file_id, raw_prompt, raw_negative FROM templates \
                ...
                match q.fetch_all(&state.db_read).await {
                    Ok(rows) => {
                        for row in rows {
                            let fid: i64 = row.get(0);
                            let rp: Option<String> = row.get(1);
                            let rn: Option<String> = row.get(2);
                            prompts.insert(fid, resolve_prompt_pair(rp.as_deref(), rn.as_deref()));
                        }
                    }
                    Err(e) => tracing::warn!("prompt backfill query failed: {e}"),
                }
            }
            for result in &mut results {
                ...
                if let Some((positive, negative)) = prompts.get(&id) {
                    result["positive"] = json!(positive);
                    result["negative"] = json!(negative);
                }
            }
        }
    }
    ...
    // Rust never defers a count, so the VALUE is always false -- but whether the key
    // is present at all is part of the contract, and Python's two payload builders
    // disagree about it. Present on the SQL path, absent on the cache path.
    let mut body = json!({
        "ok": true,
        // Python wraps the payload with api_success, so `error` and `data` sit beside
        // `ok` and the payload keys live at the top level. Building the payload alone
        // left both out of every response.
        "error": null,
        "data": null,
        "status": status,
        "total": total,
    ...
    if !python_would_use_cache(&p) {
        if let Some(obj) = body.as_object_mut() {
            obj.insert("count_pending".to_string(), json!(false));
        }
    }
...
pub async fn search_count(
        ...
        .await
    {
        ...
        // Python hands this to `api_result`, which adds ok/error/data and spreads
        // the payload at the top level. Rust sent the payload bare, so `ok` was
...
mod tests {
    ...
    // Regression: LIKE filters mixed a literal "?" with QueryBuilder::push_bind,
    // which emits its own placeholder — the resulting SQL was malformed and every
    // search using in_prompt / in_negative / in_path / checkpoint returned 500.
    #[tokio::test]
    /// Python widens a short path term; Rust used to drop it.
    ///
    /// Under 3 characters there is no trigram phrase, so the FTS branch cannot fire.
    /// Python falls back to `(nfkc_lower(f.path) LIKE ? OR ...)`
    /// (core/query/filters_tags_path.py:21, reached from
    /// filters_tags_apply_helpers.py:101-105 without phrase gating), and this pins
    /// that Rust does the same. Opened through `tagdb_core::connect` on purpose: an
    /// in-memory pool built here would not have `nfkc_lower` registered, so the test
    /// would pass or fail for reasons unrelated to this branch.
    async fn a_short_path_term_still_matches_by_path() {
        let dir = tempfile::tempdir().expect("tempdir");
        let db = dir.path().join("search.db");
        let seed = sqlx::SqlitePool::connect(&format!("sqlite://{}?mode=rwc", db.display()))
            .await
            .expect("seed pool");
        sqlx::raw_sql(
            "CREATE TABLE files (id INTEGER PRIMARY KEY, path TEXT, mtime INTEGER, is_deleted INTEGER DEFAULT 0, meta_source TEXT, width INTEGER, height INTEGER);
             CREATE TABLE templates (id INTEGER PRIMARY KEY, file_id INTEGER, raw_prompt TEXT, raw_negative TEXT, model_name TEXT, model_hash TEXT);
             CREATE TABLE file_tags (file_id INTEGER, tag_id INTEGER);
             CREATE TABLE tags (id INTEGER PRIMARY KEY, tag TEXT);
             -- Full-width `ＡＢ`: only the folded comparison finds it from `ab`.
             INSERT INTO files(id, path, mtime, is_deleted) VALUES (1,'/img/ＡＢ-one.png',1,0), (2,'/img/other.png',2,0);
             INSERT INTO templates(id, file_id, raw_prompt) VALUES (1,1,'x'), (2,2,'y');",
        )
        .execute(&seed)
        .await
        .expect("seed schema");
        ...
        let pool = tagdb_core::connect(db.to_str().expect("utf8"))
            .await
            .expect("connect");
        let params = SearchParams::from(SearchQueryRaw {
            q: Some("ab".to_string()),
            also_path: Some("1".to_string()),
            ..Default::default()
        });
        let mut qb: QueryBuilder<Sqlite> = QueryBuilder::new(
            "SELECT DISTINCT f.id FROM files f JOIN templates tm ON tm.file_id=f.id WHERE 1=1",
        );
        push_where_filters(&mut qb, &params, None, WdModelScope::Unset, true);
        let rows = qb
            .build()
            .fetch_all(&pool)
            .await
            .expect("short-term path query");
        let ids: Vec<i64> = rows.iter().map(|r| r.get::<i64, _>(0)).collect();
        assert_eq!(
            ids,
            vec![1],
            "a 2-character path term must still match by path"
        );
        pool.close().await;
    }

    #[tokio::test]
    async fn like_filters_build_valid_sql() {
        use sqlx::sqlite::{SqliteConnectOptions, SqlitePoolOptions};
        use std::str::FromStr;
        ...
        sqlx::raw_sql(
            "CREATE TABLE files (id INTEGER PRIMARY KEY, path TEXT, mtime INTEGER, is_deleted INTEGER DEFAULT 0, meta_source TEXT, width INTEGER, height INTEGER);
             CREATE TABLE templates (id INTEGER PRIMARY KEY, file_id INTEGER, raw_prompt TEXT, raw_negative TEXT, model_name TEXT, model_hash TEXT);
             CREATE VIRTUAL TABLE templates_fts USING fts5(raw_prompt);
             INSERT INTO files(id, path, mtime, is_deleted) VALUES (1,'/img/one_piece.png',1,0), (2,'/img/other.png',2,0);
             INSERT INTO templates(id, file_id, raw_prompt, raw_negative, model_name, model_hash)
               VALUES (1, 1, 'ワンピース, 1girl', 'bad hands', 'animagine', 'abc123'),
                      (2, 2, 'landscape', 'blurry', 'other', 'def456');",
        )
        .execute(&pool)
        .await
        .unwrap();

        async fn matching_ids(pool: &sqlx::SqlitePool, raw: SearchQueryRaw) -> Vec<i64> {
            let params = SearchParams::from(raw);
            let mut qb: QueryBuilder<Sqlite> = QueryBuilder::new(
                "SELECT DISTINCT f.id FROM files f JOIN templates tm ON tm.file_id=f.id WHERE 1=1",
            );
        ...
        assert_eq!(
            matching_ids(
                &pool,
                SearchQueryRaw {
                    in_prompt: Some("ワンピース".to_string()),
                    ..Default::default()
                }
            )
            .await,
        ...
        assert_eq!(
            matching_ids(
                &pool,
                SearchQueryRaw {
                    in_negative: Some("blurry".to_string()),
                    ..Default::default()
                }
            )
            .await,
        ...
        assert_eq!(
            matching_ids(
                &pool,
                SearchQueryRaw {
                    in_path: Some("one_piece".to_string()),
                    ..Default::default()
                }
            )
            .await,
        ...
        assert_eq!(
            matching_ids(
                &pool,
                SearchQueryRaw {
                    checkpoint: Some("animagine".to_string()),
                    ..Default::default()
                }
            )
            .await,
        ...
        // checkpoint term shorter than 3 chars takes the non-FTS branch
        assert_eq!(
            matching_ids(
                &pool,
                SearchQueryRaw {
                    checkpoint: Some("de".to_string()),
                    ..Default::default()
                }
            )
            .await,
