...
pub async fn list_prompts(
    ...
    // Python calls `api_success(result)` positionally (prompt_library_api.py:65),
    // so the payload is spread and `data` stays None. Passing Some(payload) here
    // duplicated the whole body into `data`.
    api_success(payload, StatusCode::OK, None)
}
...
pub async fn export_library(
    ...
    match export_library_value(&state.db_read, folder_id).await {
        // Python: `api_success(export_library(...))` positionally
        // (prompt_library_api_routes_bulk.py:70) -- `data` stays None.
        Ok(value) => api_success(value, StatusCode::OK, None),
...
pub async fn from_file(
    ...
    // Characters live in the parsed NovelAI v4 detail payload, not in the raw
    // metadata blob, which carries `v4_prompt.caption.char_captions` instead.
    let detail = meta_extract::resolve_detail_fields(
        &meta_source,
        &raw_prompt,
        &raw_negative,
    ...
    let create_data = json!({
        ...
        "characters": extract_novelai_characters(detail.novelai_v4.as_ref()),
    });
    match insert_prompt(
        &state.db,
        &create_data,
...
fn extract_novelai_characters(novelai_v4: Option<&Value>) -> Value {
    let Some(data) = novelai_v4 else {
        return json!([]);
    };
    let prompts = data
...
Steps: 20, Sampler: Euler, CFG scale: 7, Seed: 123, Model: test-model', '', NULL, '');",
    ...
    /// `raw_meta_json` holds NovelAI's own schema, so characters must come from
    /// the parsed detail payload rather than from the raw blob.
    #[test]
    fn novelai_characters_come_from_the_parsed_detail_payload() {
        let raw = json!({
            "v4_prompt": {"caption": {
                "base_caption": "2girls",
        ...
        let detail =
            meta_extract::resolve_detail_fields("novelai_v4_png", "", "", Some(&raw), None);
        let chars = extract_novelai_characters(detail.novelai_v4.as_ref());

        assert_eq!(chars[0]["prompt"], "blonde hair");
        assert_eq!(chars[0]["negative"], "extra limbs");
        assert_eq!(chars[0]["center"], json!({"x": 0.25, "y": 0.5}));
        assert_eq!(extract_novelai_characters(None), json!([]));
    }
}
