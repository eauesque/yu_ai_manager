...
/// Validate the reorder body the way Python does.
///
/// `AnalysisServerReorderRequest` (core/analysis_api/server_api_models.py)
/// declares `server_ids: list[str]` with no default and derives from
/// `ApiModel`, i.e. `extra="forbid"`. This used to be
/// `#[derive(Deserialize)] { #[serde(default)] server_ids: Vec<String> }`, so
/// `PUT /api/analysis/servers/reorder` with `{}` was accepted as "reorder
/// nothing", answered 200, and REWROTE config.json -- while Python answered
/// 400. A missing field and an empty list are not the same request, and only
/// one of them should reach the config writer. Found by un-skipping this entry
/// (v4.732.14): it had never been sent to either server.
fn parse_reorder_body(body: &Value) -> Result<Vec<String>, Vec<String>> {
    let mut errors = Vec::new();
    let ids = match body.get("server_ids") {
        None => {
            errors.push("server_ids: Field required".to_string());
            Vec::new()
        }
        Some(Value::Array(items)) => {
            ...
            for item in items {
                match item.as_str() {
                    ...
                    None => {
                        errors.push("server_ids: Input should be a valid string".to_string());
                        break;
                    }
                }
            }
        ...
        Some(_) => {
            errors.push("server_ids: Input should be a valid list".to_string());
            Vec::new()
        }
    };
...
pub async fn reorder_servers(
    ...
    let server_ids = match parse_reorder_body(&body) {
        Ok(server_ids) => server_ids,
        // Python's shape: api_error(message, 400, extra={code, detail}) yields
        // {ok, error, code, detail} and NO `data` key
        // (core/infra_core/api_errors.py). Adding "data": null here was the one
        // remaining difference after the status matched.
        Err(errors) => {
            return (
    ...
    match do_reorder(&mut config, &server_ids) {
        Ok(result) => match crate::config_io::write(&state.config.config_path, &config) {
            Ok(()) => api_result(result),
            Err(error) => internal_error(error, "failed to write config"),
        },
...
mod tests {
    ...
    /// A missing `server_ids` must be rejected, not read as "reorder nothing".
    ///
    /// Before v4.732.14 the body was `#[serde(default)]`, so `{}` reached the
    /// config writer and answered 200 while Python answered 400. The rewrite was
    /// content-neutral, but the contract was not: one implementation accepted a
    /// request the other refused.
    #[test]
    fn a_missing_server_ids_is_rejected_like_pydantic() {
        let errors = parse_reorder_body(&json!({})).unwrap_err();
        assert_eq!(errors, vec!["server_ids: Field required".to_string()]);
    }

    #[test]
    fn an_empty_server_ids_list_is_still_accepted() {
        // Distinct from the case above: the field is present and empty, which
        // Python's `list[str]` accepts too.
        assert_eq!(
            parse_reorder_body(&json!({"server_ids": []})).unwrap(),
            Vec::<String>::new()
        );
    }

    #[test]
    fn a_well_formed_server_ids_list_parses() {
        assert_eq!(
            parse_reorder_body(&json!({"server_ids": ["b", "a"]})).unwrap(),
            vec!["b".to_string(), "a".to_string()]
        );
    }

    #[test]
    fn an_unknown_field_is_rejected_like_pydantic() {
        // AnalysisServerReorderRequest derives from ApiModel (extra="forbid").
        let errors = parse_reorder_body(&json!({"server_ids": [], "oops": 1})).unwrap_err();
        assert_eq!(
            errors,
            vec!["oops: Extra inputs are not permitted".to_string()]
        );
    }

    #[test]
    fn a_non_list_server_ids_is_rejected() {
        let errors = parse_reorder_body(&json!({"server_ids": "a"})).unwrap_err();
        assert_eq!(
            errors,
            vec!["server_ids: Input should be a valid list".to_string()]
        );
    }
}
