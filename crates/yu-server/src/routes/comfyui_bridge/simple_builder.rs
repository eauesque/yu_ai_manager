...
pub fn parse_params(
    ...
    // Opt-in: rewrite `artist:NAME` into the anima `@NAME` form for anima checkpoints.
    if body
        .get("artist_at_style")
        .and_then(Value::as_bool)
        .unwrap_or(false)
    {
        prompt = crate::sd_nai::convert_artist_to_at(&prompt);
        negative_prompt = crate::sd_nai::convert_artist_to_at(&negative_prompt);
    }
    let a1111_mode = body
        .get("a1111_mode")
        .and_then(Value::as_bool)
...
mod tests {
    ...
    #[test]
    fn parse_params_rewrites_artist_tags_when_opted_in() {
        let params = parse_params(
            &json!({
                "prompt": "1girl, (artist:foo:1.2)",
                "negative_prompt": "artist:bar",
                "artist_at_style": true,
            }),
            4,
            None,
            "",
        )
        .expect("artist_at_style should be accepted");
        assert_eq!(params.prompt, "1girl, (@foo:1.2)");
        assert_eq!(params.negative_prompt, "@bar");

        // Opt-in only: the default leaves the prompt alone.
        let params = parse_params(&json!({"prompt": "1girl, (artist:foo:1.2)"}), 4, None, "")
            .expect("default should be accepted");
        assert_eq!(params.prompt, "1girl, (artist:foo:1.2)");
    }
}
