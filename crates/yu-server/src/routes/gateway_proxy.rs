...
/// Mirrors Python's `_DASH_ALLOW` (`routes/gateway_agentmemory.py:310-319`).
/// Every pair is `memory:read` scope; the dashboard needs nothing else. A path
/// absent here is refused before any credential is attached, so widening this
/// list is what grants reach — not the upstream's own permissions.
static AGENTMEMORY_DASH_ALLOWED: &[(&str, &str)] = &[
    ("GET", "livez"),
    ("GET", "health"),
    ("GET", "profile"),
    ("GET", "sessions"),
...
/// One flat limit, deliberately, where Python has a table.
///
/// Python's `_BODY_LIMITS` (`core/gateway/agentmemory_proxy.py:20-27`) raises the
/// cap to 20-50 MiB for `vision-search`, `vision-embed`, `replay/import-jsonl`,
/// `import` and `obsidian/export` — and **not one of those is in the allowlist
/// above**. Every dash path therefore falls to Python's `_DEFAULT_LIMIT` of 1 MiB,
/// which is this constant. Porting the table would copy limits for paths this
/// surface refuses outright, and would read as permission the allowlist does not
/// grant. If a large-body path is ever added to the allowlist, add its limit then.
const AGENTMEMORY_DASH_BODY_LIMIT: usize = 1024 * 1024;

/// Mirrors Python's `_DEFAULT_BASE_URL` (`core/gateway/agentmemory_proxy.py:18`).
/// An unconfigured install must behave the same on both runtimes.
const AGENTMEMORY_DEFAULT_BASE_URL: &str = "http://127.0.0.1:3111";
...
fn agentmemory_dash_error(status: StatusCode, message: &str) -> Response {
...
/// Session-authenticated AgentMemory dashboard proxy.
pub async fn agentmemory_dash_handler(
    ...
    let sub = parts
        .uri
        .path()
        .strip_prefix("/api/agentmemory-dash/")
        .unwrap_or("")
        .trim_matches('/');
    if !AGENTMEMORY_DASH_ALLOWED
        .iter()
        .any(|(method, path)| *method == parts.method.as_str() && *path == sub)
    {
        return agentmemory_dash_error(StatusCode::FORBIDDEN, "Not allowed");
    }
    if !proxy_origin_allowed(&parts.headers) {
        return agentmemory_dash_error(StatusCode::FORBIDDEN, "Forbidden");
    }

    let body = match to_bytes(body, AGENTMEMORY_DASH_BODY_LIMIT + 1).await {
        Ok(body) if body.len() <= AGENTMEMORY_DASH_BODY_LIMIT => body,
        _ => return agentmemory_dash_error(StatusCode::PAYLOAD_TOO_LARGE, "Body too large"),
    };
    let cfg = misc_admin::read_config_json(&state).await;
    let backend = &cfg["gateway"]["backends"]["agentmemory"];
    // Default exactly as Python does. `agentmemory_proxy.configure` takes
    // `base_url or _DEFAULT_BASE_URL` (`agentmemory_proxy.py:18,56`), so an
    // unconfigured Python install still dials 127.0.0.1:3111 and answers 502
    // "agentmemory unavailable" when nothing is listening.
    //
    // The two handlers this replaced returned 200 with
    // `{"ok":false,"status":"not_configured"}` instead. That diverged twice
    // over: the parity harness could not compare bodies, and the dashboard
    // checks `res.ok`, so a 200 carrying `ok:false` reads to the page as a
    // successful call. 502 is both what Python says and what the UI can act on.
    let base_url = match backend["base_url"].as_str().map(str::trim) {
        Some(url) if !url.is_empty() => url.trim_end_matches('/'),
        _ => AGENTMEMORY_DEFAULT_BASE_URL,
    };
    let mut upstream_url = match Url::parse(&format!("{base_url}/agentmemory/{sub}")) {
        Ok(url) => url,
        Err(_) => {
            return agentmemory_dash_error(StatusCode::BAD_GATEWAY, "agentmemory unavailable")
        }
    };
    upstream_url.set_query(parts.uri.query());

    let mut headers = parts.headers;
    for name in REQ_STRIP
        .iter()
        .filter_map(|name| name.parse::<HeaderName>().ok())
    {
        headers.remove(name);
    }
    if let Ok(client_ip) = HeaderValue::try_from(addr.ip().to_string()) {
        headers.insert("x-forwarded-for", client_ip);
    }
    if let Some(secret) = backend["secret_enc"]
        .as_str()
        .filter(|value| !value.is_empty())
    {
        let secret = crate::secret_store::decrypt(secret, &state.config.project_root);
    ...
    if let Some(response_headers) = response.headers_mut() {
        for (name, value) in &upstream_headers {
            if !RESP_STRIP
                .iter()
                .any(|strip| name.as_str().eq_ignore_ascii_case(strip))
            {
    ...
    response.body(Body::from(body)).unwrap_or_else(|_| {
        agentmemory_dash_error(StatusCode::BAD_GATEWAY, "agentmemory unavailable")
    })
}
...
mod tests {
    ...
    async fn dash_call(state: SharedState, method: &str, path: &str, body: Body) -> Response {
        agentmemory_dash_handler(
            State(state),
            ConnectInfo(SocketAddr::from(([127, 0, 0, 1], 45678))),
            Request::builder()
                .method(method)
    ...
    async fn dash_upstream() -> (
        ...
        let app = Router::new().fallback(any(move |headers: HeaderMap| {
            ...
            async move {
                ...
                ([("set-cookie", "upstream-only=1")], r#"{"ok":true}"#)
            }
        }));
    ...
    #[tokio::test]
    async fn agentmemory_dash_rejects_unlisted_paths_and_methods() {
        let (root, _key) = temp_root("dash-allowlist");
        let state = test_state(json!({}), root).await;
        for path in [
            "/api/agentmemory-dash/graph/query",
            "/api/agentmemory-dash/../sessions",
        ] {
            let response = dash_call(state.clone(), "GET", path, Body::empty()).await;
            assert_eq!(response.status(), StatusCode::FORBIDDEN, "{path}");
            assert_eq!(body_json(response).await, json!({"error": "Not allowed"}));
        }
    }

    #[tokio::test]
    async fn agentmemory_dash_injects_secret_without_returning_it() {
        let (root, key) = temp_root("dash-secret");
        let (base_url, received, server) = dash_upstream().await;
        std::fs::write(
            root.join("config.json"),
            serde_json::to_vec(&json!({"gateway":{"backends":{"agentmemory":{
                "base_url": base_url,
                "secret_enc": seal("upstream-secret", &key),
            }}}}))
            .unwrap(),
        )
        .unwrap();
        let response = dash_call(
        ...
        assert!(!String::from_utf8_lossy(&response_body).contains("upstream-secret"));
        server.abort();
    }

    #[tokio::test]
    async fn agentmemory_dash_rejects_oversized_graph_query() {
        let (root, _key) = temp_root("dash-body-limit");
        let response = dash_call(
            test_state(json!({}), root).await,
            "POST",
            "/api/agentmemory-dash/graph/query",
            Body::from(vec![0_u8; AGENTMEMORY_DASH_BODY_LIMIT + 1]),
        )
        .await;
    ...
    #[tokio::test]
    async fn agentmemory_dash_reports_an_unreachable_upstream() {
        let (root, _key) = temp_root("dash-unreachable");
        std::fs::write(
            root.join("config.json"),
            r#"{"gateway":{"backends":{"agentmemory":{"base_url":"http://127.0.0.1:9"}}}}"#,
        )
        .unwrap();
        let response = dash_call(
            test_state(json!({}), root).await,
        ...
        assert_eq!(
            body_json(response).await,
            json!({"error": "agentmemory unavailable"})
        );
    }

    /// An unset base_url must behave exactly like Python's, which defaults to
    /// 127.0.0.1:3111 and therefore answers 502 when nothing listens there.
    ///
    /// This pins a divergence that was live until v4.709.0: Rust answered 200
    /// with `{"ok":false,"status":"not_configured"}`. That is worse than a
    /// cosmetic difference in two ways -- the parity harness had to stop
    /// comparing bodies to tolerate it, and the dashboard branches on
    /// `res.ok`, so a 200 carrying `ok:false` reads to the page as success.
    /// The other tests all configure a base_url, so none of them reach here.
    #[tokio::test]
    async fn agentmemory_dash_defaults_the_base_url_like_python() {
        let (root, _key) = temp_root("dash-default-base-url");
        std::fs::write(root.join("config.json"), r#"{"gateway":{"backends":{}}}"#).unwrap();
        let response = dash_call(
            test_state(json!({}), root).await,
            "GET",
            "/api/agentmemory-dash/health",
        ...
        assert_eq!(
            response.status(),
            StatusCode::BAD_GATEWAY,
            "an unset base_url must dial the default and report it unreachable, \
             not answer 200 not_configured"
        );
        assert_eq!(
            body_json(response).await,
            json!({"error": "agentmemory unavailable"})
