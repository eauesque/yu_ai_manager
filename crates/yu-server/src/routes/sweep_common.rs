...
/// DB `files.path` normalization mirroring Python's `normalize_path`
/// (`core/platform/path_normalize.py`): lexical `.`/`..` resolution +
/// separator unification, plus lowercasing on Windows. Without this,
/// the same file saved via a `save_folder` config value with different
/// slash style (e.g. `h:\img` vs `h:/img`) creates duplicate `files` rows
/// (TODO.md v4.708.2).
pub(crate) fn normalize_db_path(path_str: &str) -> String {
    let normalized = crate::routes::cross_search::normpath(path_str);
    if cfg!(windows) {
...
/// `native_import_one`, honouring the per-format scan toggles. The scan and
/// the watcher pass what config.json says; the image-generation bridges keep
/// the default, since a file the user just generated is not a library scan.
pub(crate) async fn native_import_one_with(
    pool: &SqlitePool,
    path_str: &str,
    force: bool,
    toggles: meta_extract::ParserToggles,
) -> Result<i64, String> {
    let db_path = normalize_db_path(path_str);
    ...
    let extracted = fallback_chain::extract_regular_metadata_with(p, toggles);
