...
use crate::{
    ...
    routes::scan_api::api_error_code as api_error,
    state::SharedState,
};
...
fn roots_summary_error(error: impl std::fmt::Debug) -> Response {
    ...
    api_error(
        "Failed to compute roots summary",
        Some("roots_summary_failed"),
        StatusCode::INTERNAL_SERVER_ERROR,
    )
}
...
fn notify_scan_roots_changed(state: &SharedState) {
    crate::routes::watcher::restart_on_roots_changed(state);
...
pub async fn add_scan_root(
    ...
    {
        return api_error(
            &format!("既に登録済みです: {path}"),
            Some("scan_root_already_exists"),
            StatusCode::CONFLICT,
        );
    }
    ...
    let new_root = roots.last().cloned().unwrap_or(Value::Null);
    ...
    api_result(json!({"success": true, "root": new_root}))
}
...
pub async fn remove_scan_root(
    ...
    notify_scan_roots_changed(&state);
    api_result(json!({"success": true, "removed": removed_root}))
}
...
pub async fn toggle_scan_root(
    ...
    let updated_root = roots.get(idx).cloned().unwrap_or(Value::Null);
...
pub async fn batch_toggle_scan_roots(
    ...
    for root in roots.iter_mut() {
        if root.get("enabled").and_then(Value::as_bool).unwrap_or(true) != enable {
            changed += 1;
        }
    ...
    notify_scan_roots_changed(&state);
    api_result(json!({"success": true, "changed": changed, "total": total, "enabled": enable}))
}
...
pub async fn reorder_scan_roots(
    ...
    if let Some(new_roots) = body.roots {
        let valid_roots = new_roots
            .into_iter()
            .filter(|root| {
                root.get("path")
                    .and_then(Value::as_str)
                    .is_some_and(|path| path.trim().len() >= 2)
            })
            .collect::<Vec<_>>();
        if valid_roots.is_empty() {
            return api_error(
                "No valid roots in payload",
                Some("invalid_roots"),
                StatusCode::BAD_REQUEST,
            );
        }
        config["scan_roots"] = json!(valid_roots);
    ...
    notify_scan_roots_changed(&state);
    api_result(json!({"success": true, "roots": roots_snapshot}))
}
...
mod tests {
    ...
    #[tokio::test]
    async fn add_scan_root_returns_python_payload() {
        let root = test_root();
        ...
        assert_eq!(
            value["root"],
            json!({"path": "/new/root", "enabled": true, "recursive": false, "comment": "added"})
        );
    }
    ...
    async fn add_scan_root_rejects_duplicate() {
        ...
        let value = json_body(response).await;
        assert_eq!(value["code"], json!("scan_root_already_exists"));
        assert_eq!(value["error"], json!("既に登録済みです: /dupe"));
    }
    ...
    #[tokio::test]
    async fn get_scan_root_config_failure_uses_python_error_shape() {
        let root = test_root();
        let response =
            get_scan_root(State(test_state(&root, json!({})).await), None, AxumPath(0)).await;
    ...
    async fn toggle_scan_root_flips_enabled() {
        ...
        assert_eq!(value["success"], json!(true));
        assert_eq!(value["root"]["enabled"], json!(false));
    }
    ...
    async fn batch_toggle_disables_all() {
        ...
        assert_eq!(
            value,
            json!({"ok": true, "error": null, "data": null, "success": true, "changed": 2, "total": 2, "enabled": false})
        );
    }
    ...
    async fn reorder_rejects_invalid_order() {
        ...
        assert_eq!(
            json_body(response).await,
            json!({"ok": false, "error": "Invalid order array", "code": "invalid_order_array"})
        );
    }

    #[tokio::test]
    async fn reorder_rejects_empty_roots_with_python_code() {
        let root = test_root();
        let state = test_state_with_config_file(&root, json!({"scan_roots": []})).await;
        let response = reorder_scan_roots(
            State(state),
        ...
        assert_eq!(
            json_body(response).await,
            json!({"ok": false, "error": "No valid roots in payload", "code": "invalid_roots"})
        );
    }
...
mod recovery_tests {
    ...
    fn sanitize_path_unquotes_before_expanding_a_tilde() {
        ...
        //
        // Reading is not enough on its own, though: HOME is read twice below --
        // once for `expected`, once inside `sanitize_path` -- and a writer in
        // another module can land between them. Take the same seam lock every
        // writer takes, so the value cannot move mid-test.
