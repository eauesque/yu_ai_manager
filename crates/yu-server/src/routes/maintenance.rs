...
/// Map a maintenance query failure to a response, keeping the common
/// "database is locked" case distinguishable from a genuine internal error.
fn db_error(error: sqlx::Error, message: &'static str) -> Response {
    // ponytail: match on the message text - SQLite exposes the lock as a plain
    // busy/locked code that sqlx does not surface in a typed way.
    if let sqlx::Error::Database(ref db_error) = error {
        if db_error.message().contains("database is locked") {
            tracing::warn!(?error, "{}", message);
            return (
                StatusCode::SERVICE_UNAVAILABLE,
                Json(json!({"ok": false, "error": "database_locked"})),
            )
                .into_response();
        }
    }
...
pub async fn vacuum(
    ...
    match result {
        Ok(value) => api_result(value),
        Err(error) => db_error(error, "VACUUM failed"),
    }
}
...
pub async fn analyze(
    ...
    match sqlx::query("ANALYZE").execute(&state.db).await {
        // Match Python verbatim (core/services_core/maintenance_service.py's
        // analyze_database returns "ANALYZE 完了"). This is a user-facing
        // string the settings page prints as-is, so "ANALYZE complete" was not
        // a harmless wording choice -- the same action reported itself in a
        // different language depending on which server answered. Found by
        // un-suppressing the body comparison on POST /api/maintenance/analyze
        // (v4.732.13).
        Ok(_) => api_result(json!({"message": "ANALYZE 完了"})),
        Err(error) => db_error(error, "ANALYZE failed"),
    }
}
