...
/// Expand dynamic-prompt / wildcard syntax in every prompt field of a request.
///
/// Character captions get the same treatment as the base prompt: NAI does not
/// interpret `{a|b}` or `__wc__` in `char_captions` either, so an unexpanded
/// character prompt reaches the API as literal text.
///
/// Wildcard lists are commonly authored in SD syntax, so a field that the
/// expansion actually changed is additionally rewritten to NAI syntax
/// (`(tag:1.3)` -> `1.3::tag::`). Fields the expansion left alone are passed
/// through untouched -- hand-written prompts are never rewritten.
fn expand_request_prompts(
    req: &mut GenRequest,
    seed: Option<u64>,
    wildcards: &std::collections::BTreeMap<String, Vec<String>>,
) {
    let expand = |s: &str| {
        let expanded = crate::prompt_sim_core::expand_dynamic_prompt(s, seed, wildcards);
        if expanded == s {
            expanded
        } else {
            crate::sd_nai::convert_sd_to_nai(&expanded, true, true, true)
        }
    };
    req.prompt = expand(&req.prompt);
    req.negative_prompt = expand(&req.negative_prompt);
    for c in req.characters.iter_mut().flatten() {
        if let Some(p) = c.prompt.as_deref() {
            c.prompt = Some(expand(p));
        }
        if let Some(n) = c.negative.as_deref() {
            c.negative = Some(expand(n));
        }
    }
}

/// Mirrors `MAX_CHARACTERS` / `MAX_CHARACTERS_V5` in
/// extensions/builtin_nai_bridge/core_impl/nai_params.py.
///
/// NovelAI Diffusion V5 accepts up to twenty-two character prompts; V4 and
/// V4.5 stop at six. <https://docs.novelai.net/en/image/multiplecharacters/>
const MAX_CHARACTERS: usize = 6;
const MAX_CHARACTERS_V5: usize = 22;

/// How many character prompts `model` accepts.
///
/// An unrecorded generation gets the lower cap: sending more characters than
/// the model accepts is an API error, while sending fewer only drops one.
///
/// Mirrors `max_characters` in
/// extensions/builtin_nai_bridge/core_impl/nai_params.py.
fn max_characters(model: &str) -> usize {
    if model_generation(model) == Some(GEN_V5) {
        MAX_CHARACTERS_V5
    } else {
        MAX_CHARACTERS
    }
...
/// Whether a center object actually carries a usable position.
///
/// `Some({})` is not a position. Treating it as one made the Rust side send
/// `use_coords: true` for a character with no coordinates, while Python sent
/// false -- the same request produced two different images.
fn has_center(center: Option<&Value>) -> bool {
    center.is_some_and(|c| {
        c.get("x").and_then(Value::as_f64).is_some() && c.get("y").and_then(Value::as_f64).is_some()
    })
}

/// Build the char_captions and derive `use_coords` from **the same set**.
///
/// The single rule: a character contributes a coordinate only if it is actually
/// emitted (non-empty prompt) *and* its center has usable x/y. Deriving the flag
/// from the unfiltered list, as Python did, claimed coordinates for characters
/// that were never sent.
///
/// Mirrors `_build_char_captions` in
/// extensions/builtin_nai_bridge/core_impl/nai_params.py.
fn build_char_captions(chars: Option<&[CharEntry]>, model: &str) -> (Vec<Value>, Vec<Value>, bool) {
...
fn build_request_body(req: &GenRequest) -> (Value, u64, String) {
    ...
    let (pos_chars, neg_chars, use_coords) = build_char_captions(req.characters.as_deref(), &model);
...
async fn info() -> Json<Value> {
    // Python answers a BARE `jsonify({"name": ..., "bridge": ...})`
    // (nai_api.py:116-121) -- no envelope, two keys, and `name` is the extension id
    // rather than a display title. Rust answered `api_ok({bridge_id, name, version})`,
    // so all four keys differed: `bridge` existed only here, `bridge_id`/`version` only
    // there, and `name` held two different things. Measured 200/200, v4.739.2.
    Json(json!({
        "name": "builtin-nai-bridge",
        "bridge": "nai-api",
    }))
}
...
async fn models_list() -> Json<Value> {
    // `generation` lets the browser pick the position picker without
    // re-implementing the mapping in JS.
    api_ok(json!({
        "models": MODELS
            .iter()
            .map(|(id, name)| json!({
                "id": id,
                "name": name,
                "generation": model_generation(id),
            }))
            .collect::<Vec<_>>()
    }))
}
...
async fn generate(
    ...
    // so gating on it silently sent literal `__wc__` prompts straight to NAI.
    {
        ...
        expand_request_prompts(&mut req, seed, &wildcards);
    }
...
mod tests {
    ...
    #[test]
    fn expansion_reaches_character_prompts_not_just_the_base_prompt() {
        let mut req: GenRequest = serde_json::from_value(json!({
            "prompt": "{base|base}",
            "negative_prompt": "{nbase|nbase}",
            "characters": [{"prompt": "{alpha|alpha}", "negative": "{nalpha|nalpha}"}],
        }))
        .unwrap();
        expand_request_prompts(&mut req, Some(1), &Default::default());
        assert_eq!(req.prompt, "base");
        assert_eq!(req.negative_prompt, "nbase");
        let c = &req.characters.as_ref().unwrap()[0];
        assert_eq!(c.prompt.as_deref(), Some("alpha"));
        assert_eq!(c.negative.as_deref(), Some("nalpha"));
    }

    #[test]
    fn expanded_wildcards_are_rewritten_from_sd_syntax_to_nai() {
        let mut wildcards = std::collections::BTreeMap::new();
        wildcards.insert("style".to_string(), vec!["(detailed:1.3)".to_string()]);
        let mut req: GenRequest = serde_json::from_value(json!({
            "prompt": "1girl, __style__",
            "negative_prompt": "(worst quality:1.4)",
            "characters": [{"prompt": "__style__"}],
        }))
        .unwrap();
        expand_request_prompts(&mut req, Some(1), &wildcards);
        // The expanded field is rewritten...
        assert_eq!(req.prompt, "1girl, 1.3::detailed::");
        let c = req.characters.iter().flatten().next().unwrap();
        assert_eq!(c.prompt.as_deref(), Some("1.3::detailed::"));
        // ...while a hand-written field that the expansion left alone is untouched.
        assert_eq!(req.negative_prompt, "(worst quality:1.4)");
    }
    ...
    /// Emit this side of the send-parity fixtures.
    ///
    /// `#[ignore]` because it writes a file; run it explicitly:
    ///   cargo test -p yu-server -- --ignored emit_send_parity_fixtures
    ///
    /// The seam is `build_request_body`'s output, not the network. The upstream
    /// base URL stays a `const` on purpose: making it configurable so a test
    /// could point it at a local sink would also let an attacker point a real
    /// token at any host.
    #[test]
    #[ignore = "writes crates/yu-server/tests/fixtures/nai_send_parity/rust.json"]
    fn emit_send_parity_fixtures() {
        // Case names are shared with scripts/verify_nai_send_parity.py so a
        // mismatch names the case rather than an index.
        let cases: Vec<(&str, Value)> = vec![
            (
                "minimal",
                json!({"prompt": "a cat", "negative_prompt": "bad"}),
            ),
            (
        ...
        // The per-generation character cap: V5 keeps 22, V4.5 keeps 6. The
        // captions are numbered so a truncation at the wrong index shows up.
        let crowd: Vec<Value> = (0..24)
            .map(|i| json!({"prompt": format!("char{i}"), "negative": format!("bad{i}")}))
            .collect();
        let mut cases = cases;
        for (name, model) in [
            ("v5_character_cap", "nai-diffusion-5-full"),
            ("v45_character_cap", "nai-diffusion-4-5-full"),
        ] {
            cases.push((
                name,
                json!({
        ...
        fn strip(v: &Value) -> Value {
            match v {
                Value::Object(map) => Value::Object(
                    map.iter()
                        // `seed` is per-call; comparing it would drown the signal.
        ...
        for (name, input) in cases {
            let req: GenRequest =
                serde_json::from_value(input).unwrap_or_else(|e| panic!("{name}: {e}"));
            let (body, _seed, _fmt) = build_request_body(&req);
            out.insert(name.to_owned(), strip(&body));
        }

        let dir =
            std::path::Path::new(env!("CARGO_MANIFEST_DIR")).join("tests/fixtures/nai_send_parity");
        std::fs::create_dir_all(&dir).unwrap();
        let text = serde_json::to_string_pretty(&Value::Object(out)).unwrap();
        std::fs::write(dir.join("rust.json"), text + "\n").unwrap();
    }

    #[test]
    fn character_cap_follows_the_model_generation() {
        // V5 accepts 22 character prompts, V4/V4.5 six, and an unrecorded
        // generation gets the lower cap.
        assert_eq!(max_characters("nai-diffusion-5-full"), 22);
        assert_eq!(max_characters("nai-diffusion-5-curated"), 22);
        assert_eq!(max_characters("nai-diffusion-4-5-full"), 6);
        assert_eq!(max_characters("nai-diffusion-4-full"), 6);
        assert_eq!(max_characters("nai-diffusion-3"), 6);

        let chars: Vec<Value> = (0..30)
            .map(|i| json!({"prompt": format!("char{i}"), "negative": ""}))
            .collect();
        let captions = |model: &str| -> Vec<Value> {
        ...
        let v5 = captions("nai-diffusion-5-full");
        assert_eq!(v5.len(), 22);
        // Truncation takes the first N, not an arbitrary slice.
        assert_eq!(v5[0]["char_caption"], json!("char0"));
        assert_eq!(v5[21]["char_caption"], json!("char21"));
        assert_eq!(captions("nai-diffusion-4-5-full").len(), 6);
    }
    ...
    async fn test_info_returns_200() {
        ...
        // Python answers `{name, bridge}` and nothing else (nai_api.py:116-121);
        // `bridge_id` was Rust-only, so asserting it pinned the divergence.
        assert_eq!(body["name"], json!("builtin-nai-bridge"));
        assert_eq!(body["bridge"], json!("nai-api"));
        assert!(
            body.get("bridge_id").is_none(),
            "bridge_id is not in the contract"
        );
    }
