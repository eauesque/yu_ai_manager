...
use crate::sd_nai::{
    convert_artist_to_at, convert_at_to_artist, convert_nai_to_sd, convert_sd_to_nai,
};
...
pub async fn sd_to_nai(body: Result<Json<Value>, JsonRejection>) -> Response {
    ...
    let artist_at = option_value(&options, "artist_at", false);
    let to_nai = |text: &str| {
        let out = convert_sd_to_nai(
            text,
            py_truthy(&strip_lora),
            py_truthy(&strip_embedding),
            py_truthy(&emphasis),
        );
        if py_truthy(&artist_at) {
            convert_artist_to_at(&out)
        } else {
            out
        }
    };
    ...
    Json(json!({
        "prompt": to_nai(prompt),
        "negative": to_nai(negative),
        "direction": "sd_to_nai",
        "options": {
...
pub async fn nai_to_sd(body: Result<Json<Value>, JsonRejection>) -> Response {
    ...
    let to_sd = |text: &str| {
        ...
        let out = if py_truthy(&artist_at) {
            convert_at_to_artist(text)
        } else {
            text.to_string()
        };
        convert_nai_to_sd(&out, py_truthy(&emphasis))
    };

    Json(json!({
        "prompt": to_sd(prompt),
        "negative": to_sd(negative),
        "direction": "nai_to_sd",
        "options": {
            "convert_emphasis": emphasis,
            "artist_at": artist_at,
...
pub async fn batch(body: Result<Json<Value>, JsonRejection>) -> Response {
    ...
    let artist_at = py_truthy(&option_value(&options, "artist_at", false));
    for item in items {
        ...
        let converted = if direction == "sd_to_nai" {
            ...
            let input = if artist_at {
                convert_at_to_artist(&prompt)
            } else {
                prompt.clone()
            };
            convert_nai_to_sd(
