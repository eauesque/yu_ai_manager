...
pub async fn check(
    ...
    .await
    .unwrap_or(false);
    // Python answers `{"ffmpeg_available": ...}` and nothing else
    // (api_generate_routes.py:30), and the page reads only that key
    // (_script.html:153). `status`, `busy` and `job_id` were Rust-only and unread.
    Json(json!({"ffmpeg_available": ffmpeg_available})).into_response()
}
...
pub async fn status_handler(
    ...
    // Python's idle answer carries seven keys (job_runner.py:83-91) and its running
    // answer carries `phase` and `message`. Rust answered `{"running": false}` when
    // idle and never emitted `phase`/`message` at all -- so the page's progress text,
    // which reads `data.message` (_script_generate.html:151), stayed blank on Rust.
...
pub async fn outputs_list(
    ...
    // Python answers `total` beside `outputs` (api_output_routes.py:33); Rust omitted
    // it, so the key differed in every run whatever the directory held.
    let outputs = list_outputs();
    let total = outputs.len();
    Json(json!({"outputs": outputs, "total": total})).into_response()
}
