...
static DP_CHOICE_RE: LazyLock<Regex> =
    LazyLock::new(|| Regex::new(r"\{[^{}]*\|[^{}]*\}").expect("valid regex"));
// SD writes artist tags as `artist:NAME`; anima / NovelAI v4 write them as `@NAME`.
// Only the prefix is rewritten, so surrounding syntax such as `(artist:foo:1.2)` keeps
// its shape.
static ARTIST_PREFIX_RE: LazyLock<Regex> =
    LazyLock::new(|| Regex::new(r"(?i)\bartist:\s*").expect("valid regex"));
// The `@` must start a token (line start or a separator, including the `::` of NAI
// weight syntax) and be followed by a name character, so e-mail-ish text such as
// `foo@bar` is left alone.
static AT_ARTIST_RE: LazyLock<Regex> =
    LazyLock::new(|| Regex::new(r"(^|[\s,(\[{|:])@([^\s,)\]}|])").expect("valid regex"));

/// Rewrite SD-style `artist:NAME` into the anima/NAI `@NAME` form.
pub fn convert_artist_to_at(text: &str) -> String {
    ARTIST_PREFIX_RE.replace_all(text, "@").into_owned()
}

/// Inverse of [`convert_artist_to_at`].
pub fn convert_at_to_artist(text: &str) -> String {
    AT_ARTIST_RE
        .replace_all(text, "${1}artist:${2}")
        .into_owned()
}
...
mod tests {
    ...
    #[test]
    fn artist_at_style_round_trips() {
        // Prefix only: surrounding SD syntax keeps its shape.
        assert_eq!(
            convert_artist_to_at("(artist:coupe50:1.2)"),
            "(@coupe50:1.2)"
        );
        assert_eq!(
            convert_artist_to_at("1girl, artist:foo, blue eyes"),
            "1girl, @foo, blue eyes"
        );
        assert_eq!(convert_artist_to_at("1girl, ARTIST:foo"), "1girl, @foo");
        // A bare `@` inside a word (e-mail-ish text) is not an artist tag.
        assert_eq!(convert_at_to_artist("mail foo@bar"), "mail foo@bar");
        assert_eq!(convert_at_to_artist("@foo, @bar"), "artist:foo, artist:bar");
        assert_eq!(convert_at_to_artist("(@foo:1.2)"), "(artist:foo:1.2)");
        assert_eq!(
            convert_at_to_artist(&convert_artist_to_at("1girl, artist:foo, (artist:bar:1.2)")),
            "1girl, artist:foo, (artist:bar:1.2)"
        );
    }
