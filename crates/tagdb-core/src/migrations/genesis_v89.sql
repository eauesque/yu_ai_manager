...
-- Schema:    v89 (core/schema_core/schema_constants.py)
--
...
-- The default collection. Migration 7 seeds this row, but a database created
-- from this base schema never replays migrations, so without it a fresh
-- database has no collection 1 at all. Both routes/collections.py and
-- crates/yu-server/src/routes/collections.rs refuse to delete id=1 as "the
-- default collection", so on such a database the FIRST collection a user
-- creates takes id 1 and can never be deleted. favorites.collection_id also
-- defaults to 1 and carries a foreign key to this row.
--
-- Fixed text, no interpolation: the WHERE NOT EXISTS is what makes re-running
-- the base schema idempotent, not query construction.
--
-- created_at is a fixed 0, NOT strftime('%s','now'). Seed data has to be
-- byte-identical in every database created from this schema: the Python and
-- Rust servers each create their own, and the parity harness compares
-- GET /api/collections between them. With a clock expression the two rows are
-- seeded seconds apart and the comparison fails on collections[0].created_at.
-- 0 also sorts the default collection first, which is where it belongs.
INSERT INTO collections (id, name, sort_order, created_at)
  SELECT 1, 'Favorites', 0, 0
