...
export type ApiFetchOpts = RequestInit & {
  ...
  silent?: boolean;
  /**
   * HTTP statuses the caller handles itself (e.g. 409 for "already running").
   * They are neither reported to the error bundle nor thrown — the Response is
   * returned unread so the caller can inspect status and body.
   */
  allowStatuses?: number[];
};

export async function apiFetch(path: string, opts?: ApiFetchOpts): Promise<Response> {
  const { silent, allowStatuses, ...fetchOpts } = opts ?? {};
  ...
  if (!res.ok) {
    // Caller-handled status: hand the untouched Response back, no bug report.
