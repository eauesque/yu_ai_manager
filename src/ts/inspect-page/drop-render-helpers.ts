...
export async function renderInspectCharacterPrompts(
  ...
  if (data.raw_meta_json && isNaiSource(data.meta_source)) {
    try {
...
export async function renderInspectCharGrid(
  ...
  try {
    // Two sources, then one shared parser. `raw_meta_json` may be either the
    // Python wrapper or the bare Comment chunk the Rust scanner writes, so the
    // shape handling lives in parseNovelAIComment rather than here.
    let comment = parseNovelAIComment(data.raw_meta_json);
    if (!comment) {
      const raw = data.raw_metadata || {};
      if (typeof raw.Comment === 'string') comment = parseNovelAIComment(raw.Comment);
    }
    if (!comment) return;
    ...
    // Three-valued: false means the image used character order only, null means
    // it did not say. Folding null into false would claim a V5 image has no
    // coordinates in force.
    const rawMode = (comment.v4_prompt as { use_coords?: unknown } | undefined)?.use_coords;
    const coordsMode = typeof rawMode === 'boolean' ? rawMode : null;
    const doRender = () => {
      requestAnimationFrame(() => {
        if (img.clientWidth && img.clientHeight) {
          renderCharacterGrid(wrapper, img, gridChars, coordsMode);
        } else {
          requestAnimationFrame(() => {
            if (img.clientWidth && img.clientHeight) renderCharacterGrid(wrapper, img, gridChars, coordsMode);
          });
        }
      });
    };
