...
interface WtJobEntry extends Record<string, unknown> {
  job_id: string;
  percent?: number;
  message?: string;
  phase?: string;
...
function _setRunEnabled(enabled: boolean): void {
  const btn = document.getElementById('wtRunBatchBtn') as HTMLButtonElement | null;
  if (btn) btn.disabled = !enabled;
}

export async function wtRunBatch(): Promise<void> {
  // The start request can take a long time on large libraries; without this
  // guard every extra click queues another POST, and all but the first come
  // back 409 once the server answers.
  ...
  try {
    ...
    const res = await apiFetch('/api/wd-tagger/batch', {
      ...
      // A job started elsewhere (another tab, or one still running after a
      // reload) is a normal answer, not a bug worth reporting.
      allowStatuses: [409],
    });
    if (res.status === 409) {
      getNavApi().showToast(_t('api_error.job_running', 'A batch job is already running'), true);
      _showProgress(true);
...
function _startPolling(): void {
  ...
  _pollTimer = setInterval(async () => {
    try {
      ...
      const data = await res.json();
      const jobs: WtJobEntry[] = data.active || [];
      const wtJob = jobs.find((j) => j.job_id === 'wd_tagger');
      if (wtJob) {
        if (!_cancelRequested) {
        ...
        // Check recent
        const recent: WtJobEntry[] = data.recent || [];
        const done = recent.find((j) => j.job_id === 'wd_tagger');
        if (done) {
          if (done.phase === 'cancelled') {
            _updateProgress(done.percent || 0, _t('tools.wt_cancelled', 'Cancelled'));
          } else {
...
export async function wtCancelBatch(): Promise<void> {
  try {
    ...
    const data = await res.json();
    if (data.ok === false) {
      // Without this the button was indistinguishable from a no-op: a 404
      // ("no running job") or a scope rejection was swallowed silently.
      getNavApi().showToast(data.error || _t('tools.wt_cancel_failed', 'Cancel failed'), true);
      return;
    }
    // Cancellation only takes effect at the next file boundary, so the poll
    // would otherwise overwrite this label with live progress and make the
    // request look ignored.
    _cancelRequested = true;
    _setCancelVisible(false);
    _updateProgress(0, _t('tools.wt_cancelling', 'Cancelling...'));
  } catch {
    getNavApi().showToast(_t('tools.wt_cancel_failed', 'Cancel failed'), true);
  }
}
