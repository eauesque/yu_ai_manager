...
/**
 * Open a folder picker and write the chosen path into `targetInputId`.
 *
 * The target is a parameter because the same picker serves the tools page
 * scan field and the settings page scan-root field; the delegated
 * `data-action-arg` supplies it, and omitting it keeps the tools page working.
 */
export async function pickFolderNative(targetInputId = 'scanPath'): Promise<void> {
  ...
  // 2) Native dialog when the server has one, server-side browser otherwise.
  const target = document.getElementById(targetInputId) as HTMLInputElement | null;
  const ok = pickFolder(target?.value.trim() || '', (path) => {
    if (target) target.value = path;
  });
  if (ok) return;

  // 3) Neither is available (dir-picker-fallback.js missing on this page)
  alert(
    _t(
