...
export async function loadBackupStatus(): Promise<void> {
  ...
  try {
    ...
    // Same reason as the listing above: a failed request is not a status. The
    // error body has none of these fields, so reading it anyway rendered
    // "Scheduler: off" and "Max generations: undefined" -- claims about the
    // user's backup settings that nothing had answered.
    if (!resp.ok) {
      el.textContent = '';
      return;
    }
