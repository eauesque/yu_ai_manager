...
import { isNaiSource, parseNovelAIComment } from '../../runtime-init/novelai/character-parse';
...
function extractGridCharacters(data: any): GridCharacter[] {
  ...
  if (isNaiSource(data.meta_source) && data.raw_meta_json) {
    const comment = parseNovelAIComment(data.raw_meta_json);
...
/**
 * Read the three-valued `use_coords` off the payload, falling back to the raw
 * metadata when the payload did not carry it.
 *
 * `undefined` from either source becomes `null`: "the image did not say" is a
 * state of its own, distinct from "the image said false".
 */
// eslint-disable-next-line @typescript-eslint/no-explicit-any
function extractCoordsMode(data: any): boolean | null {
  const payload = (data as { novelai_v4?: { use_coords?: unknown } }).novelai_v4;
  const fromPayload = payload?.use_coords;
  if (typeof fromPayload === 'boolean') return fromPayload;
  if (fromPayload === null) return null;
  const rawMeta = (data as { raw_meta_json?: unknown }).raw_meta_json;
  const comment = parseNovelAIComment(rawMeta);
  const fromRaw = (comment?.v4_prompt as { use_coords?: unknown } | undefined)?.use_coords;
  return typeof fromRaw === 'boolean' ? fromRaw : null;
}
...
export function renderCharacterGridForData(data: any): void {
  ...
  const coordsMode = extractCoordsMode(data);
  ...
  const doRender = () => {
    if (img.clientWidth && img.clientHeight) {
      void runtimeInitApi.renderCharacterGrid(stage, img, characters, coordsMode).then(() => {
