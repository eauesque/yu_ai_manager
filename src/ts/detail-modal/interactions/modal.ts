...
import { closeOverflowMenu } from '../content/toolbar/toolbar-collapse';
...
export function handleModalKeydown(e: KeyboardEvent, api: ModalApi): boolean {
  ...
  if (e.key === 'Escape') {
    ...
    // Innermost layer first: the toolbar overflow menu. Its own capture-phase
    // ESC handler lives in toolbar-collapse.ts but is registered on the first
    // modal open -- later than this one on the same target and phase -- so the
    // stopImmediatePropagation() below would swallow the key and close the
    // whole modal instead of just the menu.
    const overflow = document.getElementById('modalToolbarOverflow');
    if (overflow?.classList.contains('is-open')) {
      e.preventDefault(); closeOverflowMenu(); return true;
    }
