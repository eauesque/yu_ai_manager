...
import { pickFolder } from '../shared/pick-folder';
...
interface ServerStatusData {
  ...
  /** What the start-up version gate saw. Absent when it never ran (a URI
   *  database, or one that did not exist yet) -- the same key-absence
   *  convention stale_rebuild uses. Not rendered yet; `GET /api/server-info`
   *  is where an operator reads it today. */
  schema?: {
    expected?: number;
    actual?: number | null;
    drift?: 'match' | 'behind' | 'ahead' | 'unreadable';
    /** True only while answering queries over a database this build
     *  disagrees with -- the condition that is otherwise one boot log line. */
    serving_with_drift?: boolean;
    migrator_declared?: boolean;
    migrator_reason?: string;
    /** The other migration chain. `rust_drift: 'ahead'` is the condition
     *  behind exit 65 — a database written by a newer binary, which nothing
     *  migrates backwards. Reported here as the *before*: a machine already
     *  in that state cannot start, so it cannot answer this endpoint either. */
    rust_expected?: number;
    rust_actual?: number | null;
    rust_drift?: 'match' | 'behind' | 'ahead' | 'unreadable';
  };
  [key: string]: unknown;
}
...
interface FastModeBuild {
  ...
  decision?: {
    ...
    at?: number | null;
    // The verdict is written before yu-server runs, so an approval can still
    // be overridden by yu-server exiting EXIT_DB_SCHEMA_BEHIND (the database
    // is at an older schema and only Python can migrate it) or by --python.
    // This response is served by the Python server, so an approved verdict
    // arriving here is stale by construction.
    superseded_by_this_launch?: boolean;
  } | null;
...
function idleText(s: FastModeBuild): { text: string; color: string } {
  ...
  if (d.use_fast_mode) {
    // Approved, but this response came from the Python server, so the launch
    // did not end up on the Rust one. Saying "fast mode" here would contradict
    // the process the user is talking to.
    if (d.superseded_by_this_launch) {
      return {
        text: _t(
          'settings.fast_mode_superseded',
          '起動時は高速モードと判定されましたが、この起動は Python 版で動いています（データベースの移行が必要な場合など）',
        ),
        color: 'var(--status-warn,#7c4700)',
      };
    }
...
export function browseDbPath(): void {
  const input = document.getElementById('dbPathInput') as HTMLInputElement | null;
  if (!input) return;
  // Native dialog when the server has one, server-side browser otherwise.
  // (The POST this used to send was refused by *both* implementations -- Quart's
  // route registers GET only and the handler reads `request.args`, never a body.)
  const ok = pickFolder(input.value, (path) => {
    // Append /tags.db if a directory was selected
    input.value = path.endsWith('.db') ? path : path.replace(/[\\/]$/, '') + '/tags.db';
  });
  if (!ok) {
    alert(
      _t(
