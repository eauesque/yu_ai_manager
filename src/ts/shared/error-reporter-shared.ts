...
export function isBenignViewTransitionSkip(reason: unknown): boolean {
  ...
  // Chromium reports skipped/aborted view transitions with varying suffixes,
  // e.g. "Transition was aborted because of invalid state. ViewTransition opt-in disabled"
  // (cross-document transition where the destination document has not opted in yet).
  if (!/^Transition was (skipped|aborted)\b/.test(message)) return false;
  return !name || name === 'AbortError' || name === 'InvalidStateError';
}
