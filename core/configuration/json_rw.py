...
def candidate_config_paths(config_path: str | None = None) -> list[str]:
    ...
    # `_default_config_path()` resolves to config.toml whenever one exists, so
    # the branch above appends it and then the literal appends it again.
    # A duplicate is not merely redundant work: shadowed_config_paths() returns
    # everything after the first existing candidate, so the second copy makes
    # config.toml report itself as shadowed -- telling the user their settings
    # go somewhere nothing reads, about the very file being read.
    seen: set[str] = set()
    return [p for p in paths_to_try if not (p in seen or seen.add(p))]
