...
def register_profiles_crud_routes(bp):
    ...
    async def api_profiles_update(name: str):
        ...
        try:
            ...
            # `update_profile_metadata` raises ValueError both for an absent
            # profile and for a bad field set. Only the latter is a 400.
    ...
    async def api_profiles_delete(name: str):
        ...
        try:
            ...
            logger.exception("Profile delete failed for %s", name)
            return api_error("Profile delete failed", 500, code="delete_failed")
    ...
    async def api_profiles_duplicate(name: str):
        ...
        try:
            ...
            if "already exists" in message:
                return api_error(
                    f"Profile '{new_name}' already exists", 409, code="duplicate_failed"
                )
    ...
    async def api_profiles_rename(name: str):
        ...
        try:
            ...
            if "not found" in message:
                return api_error(f"Profile '{name}' not found", 404, code="profile_not_found")
            if "already exists" in message:
                return api_error(f"Profile '{new_name}' already exists", 409, code="rename_failed")
