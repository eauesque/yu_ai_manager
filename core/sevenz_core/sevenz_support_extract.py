"""7z-internal metadata extraction helpers."""
...
from .sevenz_support_core import read_bytes_from_7z
...
def extract_metadata_from_7z(archive_path: str, internal_path: str) -> dict[str, Any]:
    """Extract metadata from 7z entry bytes."""
    result: dict[str, Any] = {
        "meta_source": None,
        "format": None,
        "raw_prompt": None,
    ...
    try:
        if is_media_metadata_extension(internal_path):
            ...
            file_bytes = read_bytes_from_7z(archive_path, internal_path)
            apply_extractor_by_extension(result, internal_path, file_bytes)
    except Exception as e:
        logger.warning(f"Failed to extract metadata from 7z: {archive_path}!{internal_path}: {e}")
