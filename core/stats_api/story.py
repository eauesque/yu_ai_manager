...
def refresh_date_dependent(con, result: dict) -> dict:
    """Recompute only the fields of a story payload that depend on today.

    `streak_days` and `on_this_day` are both answers to "what is today?", but
    the story payload is cached under a signature made of row counts and
    mtimes alone. With the library untouched, both stayed frozen at whatever
    they were when the cache was written -- the streak panel stopped counting
    across midnight, and "on this day" kept naming the wrong day, until
    something happened to change the database.

    The rest of the payload (the monthly timeline and the events derived from
    it) is what the cache exists for, so it is kept. These two are a query
    each.
    """
    where = ai_image_where("f")
    result["streak_days"] = _streak_days(con, where)
    result["on_this_day"] = _on_this_day(con, where)
    return result


def local_date_key() -> str:
    """Today, in the configured timezone -- the cache's date component."""
    return configured_now().date().isoformat()
