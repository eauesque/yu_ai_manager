...
from core.stats_api.story import (
    build_story_stats,
    local_date_key,
    refresh_date_dependent,
)
...
def get_cached_stats_all(con) -> dict:
    ...
    if cached is not None:
        # Always report the flag, never omit it. Rust states it explicitly
        # (routes/stats.rs), and an absent key is ambiguous to a reader: it
        # cannot tell "fresh" from "this server does not report staleness".
        cached["_stale"] = False
        return cached
    ...
    def _rebuild():
        ...
        try:
            ...
            try:
                ...
                result["db_sig"] = db_sig
                result["cache_version"] = _CACHE_VERSION
                result["local_date"] = local_date_key()
...
_CACHE_ONLY_KEYS = ("local_date",)


def _served(payload: dict) -> dict:
    """Strip the cache's own bookkeeping before the payload goes out.

    `local_date` records when the date-dependent fields were computed. It
    belongs in the cache file, not in the response: the Rust port mirrors this
    body field for field, so a key that exists only on the Python side reads
    as a parity divergence -- which is exactly how it was caught.
    """
    if not isinstance(payload, dict):
        return payload
    if not any(key in payload for key in _CACHE_ONLY_KEYS):
        return payload
    return {key: value for key, value in payload.items() if key not in _CACHE_ONLY_KEYS}
...
def get_cached_story(con) -> dict:
    ...
    if cached is not None:
        cached["_stale"] = False  # see get_cached_stats_all
        if cached.get("local_date") != today:
            # The database has not changed, so everything expensive in here is
            # still valid -- but `streak_days` and `on_this_day` answer "what
            # is today?", and today is not what it was when this was written.
            # Recomputing the whole payload to fix two integers would throw
            # away the cache for no reason.
            refresh_date_dependent(con, cached)
            cached["local_date"] = today
            try:
                _write_cache(story_cache, cached)
            except OSError:
                _logger.debug("story cache date refresh write failed", exc_info=True)
    ...
    if stale is not None:
        ...
        stale["_stale"] = True
        if stale.get("local_date") != today:
            refresh_date_dependent(con, stale)
            stale["local_date"] = today
        return _served(stale)
