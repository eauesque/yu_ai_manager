"""Helper operations for archive (ZIP/7z) extract API."""
...
def _validate_internal_path(internal_path: str) -> bool:
    """Pre-validate archive internal path for safety (ZipSlip prevention).

    Rejects null bytes, absolute paths, and parent directory traversal (..).
    """
...
def _verify_extracted_path(extracted_path: str, extract_dir: str) -> bool:
    """Verify that the extracted path stays within extract_dir."""
...
def extract_archive_member(archive_path: str, internal_path: str) -> tuple[str, Any]:
    """Extract a member from ZIP, 7z, or RAR archive."""
    if _is_7z_archive(archive_path):
        return _extract_7z_member(archive_path, internal_path)
    if _is_rar_archive(archive_path):
        return _extract_rar_member(archive_path, internal_path)
    return _extract_zip_member(archive_path, internal_path)


def extract_zip_member(zip_path: str, internal_path: str) -> tuple[str, Any]:
    """Legacy alias — delegates to archive-type-aware extraction."""
    return extract_archive_member(zip_path, internal_path)
...
def _extract_zip_member(zip_path: str, internal_path: str) -> tuple[str, Any]:
    ...
    # Pre-validation: prevent path traversal
    ...
    # Post-validation: final check with realpath
