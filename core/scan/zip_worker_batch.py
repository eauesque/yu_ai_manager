"""ZIP-internal scan worker — batch operations."""
...
from core.zip_core.zip_support_core import batch_read_from_zip, batch_zip_info
...
def _try_cached_info(
    archive_cache, zip_path: str
) -> dict[str, tuple] | None:
    """Return cached per-member info_map if the archive is unchanged.

    Returns dict {member: (mtime, size)} or None.
    """
    ...
    try:
        ...
        raw = archive_cache.get_members_info(
            zip_path, int(st.st_mtime), st.st_size
        )
        if raw is not None:
...
def scan_batch_zip(
    ...
    *,
    ...
    """Scan multiple files inside a single ZIP with one archive open.

    Opens the ZIP once for mtime/size batch query, filters via DB
    prefetch, then opens once more for byte reads of changed entries.

    If *archive_cache* is provided and the archive is unchanged,
    Phase 1 (ZIP open for mtime/size) is skipped using cached info.
    """
    from core.models_core.models_files import get_file_rows_batch

    full_paths = [f"{zip_path}!{ip}" for ip in internal_paths]
    results: list[ScanResult] = [None] * len(internal_paths)

    # Phase 1: Try cached info first to avoid opening ZIP
    info_map = None
    if not force:
        info_map = _try_cached_info(archive_cache, zip_path)
        if info_map is not None:
            logger.debug("ZIP Phase 1 skip (cached): %s (%d members)", zip_path, len(internal_paths))

    if info_map is None:
        try:
            info_map = run_with_timeout(
                lambda: batch_zip_info(zip_path, internal_paths),
                timeout=ARCHIVE_SCAN_TIMEOUT,
                label=f"batch_info:{zip_path}",
            )
        except (TimeoutError, Exception) as e:
            logger.warning(f"ZIP batch info failed: {zip_path}: {e}")
            return results

    # Phase 2: DB prefetch — filter unchanged files
    db_rows = get_file_rows_batch(con, full_paths)
    from core.schema_core.schema import CURRENT_PARSER_VERSION

    needs_scan: list[int] = []  # indices into internal_paths
    ...
    if needs_backfill_list:
        ...
        try:
            backfill_bytes = run_with_timeout(
                lambda: batch_read_from_zip(zip_path, [internal_paths[i] for i in regular_backfill]),
                timeout=ARCHIVE_SCAN_TIMEOUT,
                label=f"batch_backfill:{zip_path}",
        ...
        if media_backfill:
            ...
            hash_updates.extend(
                collect_media_backfill_updates(
                    ...
                    full_paths,
                    db_rows,
                    lambda ip: extracted_zip_member_path(zip_path, ip),
                )
            )
        if hash_updates:
    ...
    # Phase 3: Batch read bytes for entries needing scan
    ...
    try:
        bytes_map = run_with_timeout(
            lambda: batch_read_from_zip(zip_path, [internal_paths[i] for i in regular_scan]),
            timeout=ARCHIVE_SCAN_TIMEOUT,
            label=f"batch_read:{zip_path}",
        ) if regular_scan else {}
    except (TimeoutError, Exception) as e:
        logger.warning(f"ZIP batch read failed: {zip_path}: {e}")
        bytes_map = {}
        ...
        if idx in media_scan:
            try:
                ...
                metadata, content_hash = extract_media_metadata_and_hash(
                    ip,
                    lambda inner: extracted_zip_member_path(zip_path, inner),
                    apply_extractor_by_extension,
