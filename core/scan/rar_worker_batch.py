"""RAR-internal scan worker — batch operations."""
...
from core.rar_core.rar_support_core import batch_rar_info, batch_read_from_rar
...
def scan_batch_rar(
    ...
    *,
    ...
    """Scan multiple files inside a single RAR with batch operations."""
    ...
    # Phase 1: Try cached info first
    info_map = None
    if not force:
        info_map = _try_cached_info_rar(archive_cache, archive_path)
        if info_map is not None:
            logger.debug("RAR Phase 1 skip (cached): %s (%d members)", archive_path, len(internal_paths))

    if info_map is None:
        try:
            info_map = run_with_timeout(
                lambda: batch_rar_info(archive_path, internal_paths),
                timeout=ARCHIVE_SCAN_TIMEOUT,
                label=f"batch_info:{archive_path}",
            )
        except (TimeoutError, Exception) as e:
            logger.warning(f"RAR batch info failed: {archive_path}: {e}")
            return results
    ...
    if needs_backfill_list:
        ...
        try:
            backfill_bytes = run_with_timeout(
                lambda: batch_read_from_rar(archive_path, [internal_paths[i] for i in regular_backfill]),
                timeout=ARCHIVE_SCAN_TIMEOUT,
        ...
        if media_backfill:
            ...
            hash_updates.extend(
                collect_media_backfill_updates(
                    ...
                    full_paths,
                    db_rows,
                    lambda ip: extracted_rar_member_path(archive_path, ip),
                )
            )
        if hash_updates:
    ...
    try:
        bytes_map = run_with_timeout(
            lambda: batch_read_from_rar(archive_path, [internal_paths[i] for i in regular_scan]),
            timeout=ARCHIVE_SCAN_TIMEOUT,
            label=f"batch_read:{archive_path}",
        ) if regular_scan else {}
    except (TimeoutError, Exception) as e:
        logger.warning(f"RAR batch read failed: {archive_path}: {e}")
        bytes_map = {}

    # Phase 4: Process each entry
    rows_by_file_id: dict[int, list[tuple[int, int, float, str]]] = {}
    entry_data: list[tuple] = []
        ...
        if idx in media_scan:
            try:
                ...
                metadata, content_hash = extract_media_metadata_and_hash(
                    ip,
                    lambda inner: extracted_rar_member_path(archive_path, inner),
                    apply_extractor_by_extension,
