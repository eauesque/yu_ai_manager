"""7z-internal scan worker — single-entry operations."""
...
from core.sevenz_core.sevenz_support import read_bytes_from_7z
from core.sevenz_core.sevenz_support_core import get_mtime_and_size_from_7z
...
def _get_7z_info(archive_path: str, internal_path: str) -> tuple[int, int]:
    """Get mtime and size from 7z entry in a single archive open."""
    return get_mtime_and_size_from_7z(archive_path, internal_path)


def _backfill_7z_hash(
    con: sqlite3.Connection, archive_path: str, internal_path: str,
    file_id: int, old_hash: str | None, full_path: str,
) -> ScanResult:
    """Compute missing hash for an unchanged 7z member (BUG-62 extension).
    ...
    try:
        ...
        if is_media_metadata_extension(internal_path):
            with extracted_7z_member_path(archive_path, internal_path) as extracted:
                content_hash = file_etag(extracted)
        else:
            file_bytes = run_with_timeout(
                lambda: read_bytes_from_7z(archive_path, internal_path),
...
def scan_one_7z(con: sqlite3.Connection, archive_full_path: str, config: dict[str, Any], force: bool, *, skip_backfill: bool = False) -> ScanResult:
    """Scan a file inside 7z archive."""
    ...
    try:
        raw_mtime, size = run_with_timeout(
            lambda: _get_7z_info(archive_path, internal_path),
            timeout=ARCHIVE_SCAN_TIMEOUT,
            label=archive_full_path,
        )
    except TimeoutError:
        logger.warning(f"7z info timed out ({ARCHIVE_SCAN_TIMEOUT}s): {archive_full_path}")
        return None
    except Exception as e:
        logger.warning(f"Failed to get 7z file info: {archive_full_path}: {type(e).__name__}: {e}")
    ...
    if not should_scan:
        # BUG-62: Backfill hash for unchanged 7z members with NULL hash
        if existing and not skip_backfill:
            return _backfill_7z_hash(con, archive_path, internal_path, existing[0], existing[4], path_s)
        return None
    ...
    if is_media_metadata_extension(internal_path):
        try:
            ...
            with extracted_7z_member_path(archive_path, internal_path) as extracted:
                ...
                try:
                    ...
                    apply_extractor_by_extension(metadata, internal_path, metadata_source)
                    _set_a1111_tag_source(metadata)
                except Exception as e:
                    logger.warning(f"Failed to extract metadata from 7z: {archive_full_path}: {e}")
                    metadata = {"meta_source": "7z_error", "format": "unknown",
                                "raw_prompt": None, "raw_negative": None, "raw_meta_json": None}
                content_hash = file_etag(extracted)
        except TimeoutError:
            logger.warning(f"7z read timed out ({ARCHIVE_SCAN_TIMEOUT}s): {archive_full_path}")
            metadata = {"meta_source": "7z_error", "format": "unknown",
                        "raw_prompt": None, "raw_negative": None, "raw_meta_json": None}
            content_hash = None
        except Exception as e:
            logger.warning(f"Failed to read from 7z: {archive_full_path}: {e}")
            metadata = {"meta_source": "7z_error", "format": "unknown",
                        "raw_prompt": None, "raw_negative": None, "raw_meta_json": None}
            content_hash = None
    else:
        ...
        # Metadata extraction (reuse already-read bytes)
