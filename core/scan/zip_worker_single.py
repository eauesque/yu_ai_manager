"""ZIP-internal scan worker — single-entry operations."""
...
from core.zip_core.zip_support_core import get_mtime_and_size_from_zip
from core.zip_core.zip_support_extract import _set_a1111_tag_source
from core.zip_core.zip_support_extract_dispatch import apply_extractor_by_extension, is_media_metadata_extension
...
def _get_zip_info(zip_path: str, internal_path: str) -> tuple[int, int]:
    """Get mtime and size from ZIP entry in a single archive open."""
    return get_mtime_and_size_from_zip(zip_path, internal_path)


def _backfill_zip_hash(
    con: sqlite3.Connection, zip_path: str, internal_path: str,
    file_id: int, old_hash: str | None, full_path: str,
) -> ScanResult:
    """Compute missing hash for an unchanged ZIP member (BUG-62 extension).

    Only fills NULL hash — no metadata re-extraction.
    """
    ...
    try:
        if is_media_metadata_extension(internal_path):
            ...
            file_bytes = run_with_timeout(
                lambda: read_bytes_from_zip(zip_path, internal_path),
...
def scan_one_zip(con: sqlite3.Connection, zip_full_path: str, config: dict[str, Any], force: bool, *, skip_backfill: bool = False) -> ScanResult:
    """Scan a file inside ZIP."""
    from core.helpers_core.helpers_text_path import split_archive_path
    zip_path, internal_path = split_archive_path(zip_full_path)

    try:
        raw_mtime, size = run_with_timeout(
            lambda: _get_zip_info(zip_path, internal_path),
            timeout=ARCHIVE_SCAN_TIMEOUT,
            label=zip_full_path,
        )
    except TimeoutError:
        logger.warning(f"ZIP info timed out ({ARCHIVE_SCAN_TIMEOUT}s): {zip_full_path}")
        return None
    except Exception as e:
        logger.warning(f"Failed to get ZIP file info: {zip_full_path}: {type(e).__name__}: {e}")
        logger.warning(f"  zip_path={zip_path}  internal_path={internal_path!r}")
        return None

    path_s = zip_full_path
    ...
    if not should_scan:
        # BUG-62: Backfill hash for unchanged ZIP members with NULL hash
        if existing and not skip_backfill:
            return _backfill_zip_hash(con, zip_path, internal_path, existing[0], existing[4], path_s)
        return None
    ...
    if is_media_metadata_extension(internal_path):
        try:
            ...
            logger.warning(f"ZIP read timed out ({ARCHIVE_SCAN_TIMEOUT}s): {zip_full_path}")
            metadata = {"meta_source": "zip_error", "format": "unknown",
                        "raw_prompt": None, "raw_negative": None, "raw_meta_json": None}
            content_hash = None
        except Exception as e:
            logger.warning(f"Failed to read from ZIP: {zip_full_path}: {e}")
            metadata = {"meta_source": "zip_error", "format": "unknown",
                        "raw_prompt": None, "raw_negative": None, "raw_meta_json": None}
            content_hash = None
    else:
        ...
        if file_bytes is not None:
            try:
                ...
                logger.warning(f"Failed to extract metadata from ZIP: {zip_full_path}: {e}")
                metadata = {"meta_source": "zip_error", "format": "unknown",
                            "raw_prompt": None, "raw_negative": None, "raw_meta_json": None}
        else:
            metadata = {"meta_source": "zip_error", "format": "unknown",
    ...
    meta_source = metadata.get('meta_source', 'zip_unknown')
    ...
    # Correct mtime if archive timestamps are in UTC
