...
#
# The environment override exists so a test can reach the skip branch without
# writing 500MB; production never sets it. Reading it at call time rather than
# at import keeps it out of the fast path's constant folding.
_BACKUP_SKIP_THRESHOLD_BYTES = 500 * 1024 * 1024  # 500 MB


def _backup_skip_threshold_bytes() -> int:
    override = os.environ.get("YU_TEST_BACKUP_SKIP_THRESHOLD_BYTES")
    if override:
        try:
            return int(override)
        except ValueError:
            logger.warning("Ignoring non-numeric YU_TEST_BACKUP_SKIP_THRESHOLD_BYTES")
    return _BACKUP_SKIP_THRESHOLD_BYTES
...
class BackupOutcome(StrEnum):
    """What `_backup_before_migrate` actually did.

    It used to return None from every branch, so a caller could not tell a
    backup that was made from one that was skipped for size or one that
    failed outright. An unattended migration needs that distinction: nobody
    is watching, the write is one-way, and the largest databases are exactly
    the ones the size rule leaves uncovered.
    """
...
class BackupUnavailable(RuntimeError):
    """Raised by `migrate_db(require_backup=True)` when no backup exists."""
...
def _backup_before_migrate(
    ...
    Returns what happened; callers that do not care may ignore it.
    """
    try:
        db_path_str = con.execute("PRAGMA database_list").fetchone()[2]
        if not db_path_str or db_path_str == ":memory:" or db_path_str == "":
            return BackupOutcome.NOT_APPLICABLE
        db_path = Path(db_path_str)
        if not db_path.exists():
            return BackupOutcome.NOT_APPLICABLE
    except Exception as exc:
        logger.warning("Failed to resolve DB path for backup: %s", exc)
        # Not "not applicable": the path could not be read, so whether a
        # backup is possible is unknown -- and unknown must not pass for a
        # caller that requires one.
    ...
    try:
        ...
        db_size = 0
    threshold = _backup_skip_threshold_bytes()
    if db_size >= threshold:
        ...
        return BackupOutcome.SKIPPED_LARGE
    ...
    try:
        ...
        if backup_path.exists():
            ...
            # Named for this exact source version, so it is a backup of this
            # database at this version -- unlike the new-system path, nothing
            # matched size or mtime, which is why REUSED is reported rather
            # than MADE.
            return BackupOutcome.REUSED
...
def migrate_db(
    ...
    *,
    ...
    ``require_backup`` refuses to migrate unless a pre-migration backup
    exists. It defaults to False so every interactive caller keeps the
    behaviour it has always had; the unattended entry point turns it on,
    because there nobody can notice a skipped backup and decide.

    ``db_key`` is the key the database is encrypted with, and is needed for
    the backup, not for the migration: the caller already holds an open
    connection, but the online-backup path opens the file again. Omitting it
    for an operator-keyed database makes that backup fail and fall back to a
    plain copy -- which is why require_backup alone is not enough.

    ``backup_dir`` overrides where the backup lands. Left unset it is
    resolved from config, which is right for a running server and wrong for a
    migrator told to operate on one specific file: the configured database
    and the one being migrated need not be the same.
    """
    ...
    backup = _backup_before_migrate(con, current_version, db_key, backup_dir)
    if require_backup and backup in (BackupOutcome.SKIPPED_LARGE, BackupOutcome.FAILED):
        # Raised before apply_pending, so the database is untouched.
        raise BackupUnavailable(
            f"Refusing to migrate v{current_version} -> v{CURRENT_SCHEMA_VERSION} "
            f"without a pre-migration backup (backup outcome: {backup.value})"
        )
    ...
    apply_pending(con, current_version, db_key)
