...
def apply_migration_55(con, db_key: str | None = None) -> None:
    """Create vectors.db, migrate file_vectors as float16, drop from tags.db.

    ``db_key`` is the key tags.db was opened with. vectors.db must get the
    same one: the server opens both with a single key, so a vectors.db
    written under the built-in constant is unreadable to it and gets
    deleted and recreated empty -- the vectors this migration just spent
    its time converting disappear without a backup or a log line.
    """
    import numpy as np
