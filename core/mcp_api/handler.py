...
# Keep in sync with crates/yu-server/src/mcp/dispatch.rs.
...
def _negotiated_protocol_version(params: object) -> str:
    ...
    return requested_version if requested_version in SUPPORTED_VERSIONS else LATEST_VERSION


def _inject_result_type(response: dict | None) -> dict | None:
    if isinstance(response, dict) and isinstance(response.get("result"), dict):
        response["result"].setdefault("resultType", "complete")
    return response


def _handle_jsonrpc(session_id: str, msg: dict, declared_2026: bool = False) -> dict | None:
    response = _handle_jsonrpc_inner(session_id, msg)
    return _inject_result_type(response) if declared_2026 else response


def _handle_jsonrpc_inner(session_id: str, msg: dict) -> dict | None:
    ...
    if method == "server/discover":
        return {
            ...
            "result": {
                ...
                "serverInfo": {"name": "yu-ai-manager", "version": "1.0.0"},
...
def _handle_tool_call(mcp_srv, params: dict, msg_id) -> dict:
    ...
    try:
        from mcp_server.facade import unwrap_facade
        from mcp_server.interceptor import check_kill_switch

        # A facade call carries the real tool in arguments["action"]; record the
        # effective name so the journal does not fill up with "yu_*".
        effective_name, _ = unwrap_facade(tool_name, arguments)
        kill_msg = check_kill_switch(effective_name)
        if kill_msg:
            return {
                "jsonrpc": "2.0", "id": msg_id,
