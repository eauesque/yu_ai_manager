...
def build_webui_parser() -> argparse.ArgumentParser:
    ...
    # Accepted and ignored here: `--python` selects *which* server starts, and
    # by the time this parser runs that choice is already made (this is the
    # Python one). It must still be declared, because the launcher forwards
    # every argument verbatim and argparse rejects what it does not know --
    # so omitting it would make `--python` the one flag that cannot reach the
    # server it asks for.
    parser.add_argument(
        "--python",
        action="store_true",
        help="Force the Python server instead of the Rust one (read by the launcher)",
    )
    return parser
