...
def _decision(root: Path) -> dict[str, Any] | None:
    ...
    return {
        ...
        "at": data.get("at"),
        # The record is written at --resolve time, before yu-server runs, so it
        # can outlive its own verdict: a launch approved here still falls back
        # to Python when yu-server exits EXIT_DB_SCHEMA_BEHIND (the database is
        # at an older schema than the binary and only Python can migrate it),
        # or when the user passed --python. This module is Python, so it
        # executing at all proves the Python server is the one serving --
        # which makes an "approved" record on this response stale by
        # construction. Say so rather than showing a verdict that contradicts
        # the process answering the request.
        "superseded_by_this_launch": bool(data.get("use_fast_mode")),
    }
