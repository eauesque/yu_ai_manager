...
def cleanup_dedupe_paths(con: sqlite3.Connection, dry_run: bool = False) -> int:
    ...
    # file_ocr_results/file_translations have no ON DELETE CASCADE to files
    # (unlike templates/file_tags), so a duplicate file_id with OCR data
    # raises a foreign key violation on DELETE FROM files unless cleaned up
    # explicitly first. Mirrors schema_migrate_steps_89.py's dedupe delete.
    if table_has_column(con, "file_ocr_results", "file_id"):
        _chunked_in_execute(
            con,
        ...
        _chunked_in_execute(con, "DELETE FROM file_ocr_results WHERE file_id", "", dup_ids)
