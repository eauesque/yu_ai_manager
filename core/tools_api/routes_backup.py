...
def register_tools_backup_routes(bp):
    ...
    async def api_tools_backup_download():
        ...
        def _prepare_backup():
            ...
            # Snapshot to a tempfile, read it out, drop it here. Quart's Response
            # has no Flask-style `call_on_close`, so a deferred unlink is not just
            # a leak -- it raises AttributeError and turns the request into a 500.
            # The Rust side already works this way: db_backup/routes.rs's
            # backup_download reads the temp copy into memory and removes it
            # before responding.
            # ponytail: whole DB in memory; stream it if the DB outgrows RAM.
            tmp = tempfile.NamedTemporaryFile(delete=False, suffix=".db")  # noqa: SIM115 — closed below, unlinked in finally
            tmp.close()
            try:
        ...
        data, filename = await run_db_sync(_prepare_backup)
        if data is None:
    ...
    async def api_tools_restore():
        ...
        # Quart's FileStorage.save is a coroutine (unlike Werkzeug's). Calling
        # it without awaiting wrote nothing at all, so the temp file stayed
        # empty and every restore ever attempted through this route died at
        # "Invalid database: 'files' table not found" -- the route could not
        # succeed. Measured: quart 0.20.0,
        # inspect.iscoroutinefunction(FileStorage.save) is True.
        ...
        def _validate_and_restore():
            ...
            try:
                ...
                try:
                    ...
                    uploaded_version = row[0] if row else None
                except Exception:
                    # A database without a readable schema_version is still
                    # restorable: `files` is there, so it is a yu database.
                    # Refusing here would take away the recovery instead of
                    # protecting it. Same call as the managed restore path.
            ...
            # The same rule the two managed restore paths already apply
            # (extensions/builtin_backup/core_impl/backup_ops.py::restore_backup,
            # crates/yu-server/src/db_backup/routes.rs::backup_restore). This
            # one was missed, and it is the *least* trusted of the three: the
            # file arrives by upload rather than from our own backup directory.
            #
            # Only ahead is refused. An older database is a legitimate recovery
            # that the launchers migrate on the next start; refusing it would
            # take away the recovery. Nothing migrates backwards, so a newer one
            # cannot be repaired by anybody -- and refusing is the reversible
            # answer: upgrade, then restore.
            if uploaded_version is not None and uploaded_version > CURRENT_SCHEMA_VERSION:
                tmp_path.unlink(missing_ok=True)
                return {
                    "error": (
                        f"This database is at schema v{uploaded_version}, which is newer "
                        f"than this build (v{CURRENT_SCHEMA_VERSION}). Restoring it would "
                        "leave a database this build cannot open, and no migration goes "
                        "backwards. Use a newer build, then restore. Nothing has been "
                        "modified."
                    )
                }, 400
            ...
            # Say it in the response, not only the log: the caller is a UI that
            # reports "restored successfully", and the database it just wrote
            # needs a migration before anything can open it.
            if uploaded_version is not None and uploaded_version < CURRENT_SCHEMA_VERSION:
                notice = (
                    f"The restored database is at schema v{uploaded_version}; this build "
                    f"needs v{CURRENT_SCHEMA_VERSION}. Starting through the launcher "
                    "migrates it automatically. A headless deployment (systemd, a bare "
                    "yu-server) must run the migration once before it will start."
                )
                logger.warning("%s", notice)
                payload["schema_version"] = uploaded_version
