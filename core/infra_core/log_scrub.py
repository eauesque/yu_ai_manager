...
# A freshly issued API key (`POST /api/apikeys`) is readable exactly once, and a
# bare `sk_...` carries no surrounding key name or scheme for the rules above to
# key off. Kept in step with `crates/yu-server/src/logs/scrub.rs`.
_API_KEY = re.compile(r"sk_[0-9a-f]{32}")
...
def scrub_secrets(text: str) -> str:
    ...
    out = _KV.sub(lambda m: f"{m.group('key')}{m.group('sep')}{MASK}", out)
    return _API_KEY.sub(MASK, out)
