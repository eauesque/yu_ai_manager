...
from core.collection_api.list_cache import invalidate_list as _invalidate_list
...
def invalidate_favorites_caches() -> None:
    """Clear every cache a favorites mutation invalidates.

    Call from any mutation path (toggle / batch add / batch remove).

    Two caches, not one:

    - ``/api/favorites/check``: the affected file_id may sit in any cached
      batch, so we wipe the lot — cheaper than tracking which
      (ids, collection_id) tuples touched it.
    - the collections list: a collection's ``count`` is ``COUNT(favorites)``,
      so every favorites mutation moves it. Only create/update/delete/reorder
      used to drop that cache, which left ``GET /api/collections`` reporting a
      stale count for the whole 3s window after a favorite was added or
      removed — the user saw the item land but the sidebar count stand still.
      Rust reads the DB on every request (routes/collections.rs), and this was
      the only body difference left against it once the comparison was
      un-suppressed.
    """
