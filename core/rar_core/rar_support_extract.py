"""RAR-internal metadata extraction helpers."""
...
from .rar_support_core import read_bytes_from_rar
...
def extract_metadata_from_rar(archive_path: str, internal_path: str) -> dict[str, Any]:
    """Extract metadata from RAR entry bytes."""
    result: dict[str, Any] = {
        "meta_source": None,
        "format": None,
        "raw_prompt": None,
    ...
    try:
        if is_media_metadata_extension(internal_path):
            ...
            file_bytes = read_bytes_from_rar(archive_path, internal_path)
            apply_extractor_by_extension(result, internal_path, file_bytes)
    except Exception as e:
        logger.warning(f"Failed to extract metadata from RAR: {archive_path}!{internal_path}: {e}")
