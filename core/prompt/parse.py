...
def _reject_non_finite(value: str) -> Any:
    """Mirror serde_json, which rejects NaN/Infinity outright.

    Python's json.loads accepts them by default. Leaving that alone would make
    the two implementations disagree on malformed metadata: Rust returns None
    for the whole blob while Python would hand back a value.
    """
    raise ValueError(f"non-finite constant in metadata: {value}")


def _tri_bool(value: Any) -> bool | None:
    """Read a three-valued flag: a real bool stays, anything else is unknown.

    `bool("false")` is True in Python, so a string must not be coerced. Absent
    and non-bool both mean "the image did not say", which is not the same as
    "the image said false".
    """
    return value if isinstance(value, bool) else None


def _norm_center(center: Any) -> Any:
    """Normalise -0.0 to 0.0 so both implementations serialise alike.

    json.dumps(-0.0) is "-0.0" while the Rust side emits "0.0"; left alone that
    shows up as a parity diff with no semantic difference behind it.
    """
    if not isinstance(center, dict):
        return center
    return {k: (0.0 if isinstance(v, float) and v == 0.0 else v) for k, v in center.items()}


def _centers(char: Any) -> list[Any]:
    if not isinstance(char, dict):
        return []
    return [_norm_center(c) for c in char.get("centers", [])]


def parse_novelai_v4_metadata(raw_meta_json: str) -> dict[str, Any] | None:
    """Parse NovelAI V4/V5 metadata JSON.
    ...
    Rust scanner stores the bare ``Comment`` chunk itself, whose ``v4_prompt``
    sits at the top level. Mirrors crates/meta-extract/src/detail.rs.

    ``has_v4`` tells the caller whether a v4 caption was present. The parameter
    block below runs for v3 too -- NovelAI writes steps/sampler/seed/width/height
    either way -- so the v4 gate belongs to the consumer, not here. Rejecting
    early would drop a v3 image's parameters and resolution.

    ``use_coords`` / ``negative_use_coords`` / ``use_order`` are three-valued:
    True, False, or None for "the image did not say". V5 replaced the 5x5 grid
    with free coordinates and uses ``use_coords`` to say which applies; the
    coordinate values alone cannot tell the two apart.
    """
    try:
        parsed = json.loads(raw_meta_json, parse_constant=_reject_non_finite)
        if not isinstance(parsed, dict):
            return None
        inner = parsed.get("Comment")
        data = (
            json.loads(inner, parse_constant=_reject_non_finite)
            if isinstance(inner, str)
            else parsed
        )
        if not isinstance(data, dict):
            return None
        v4_pos = data.get("v4_prompt") if isinstance(data.get("v4_prompt"), dict) else {}
        v4_neg = (
            data.get("v4_negative_prompt")
            if isinstance(data.get("v4_negative_prompt"), dict)
            else {}
        )
        result = {
            "base_caption": "",
            ...
            "parameters": {},
            "vibe_transfer": None,
            "has_v4": "v4_prompt" in data or "v4_negative_prompt" in data,
            "use_coords": _tri_bool(v4_pos.get("use_coords")),
            "negative_use_coords": _tri_bool(v4_neg.get("use_coords")),
            "use_order": _tri_bool(v4_pos.get("use_order")),
        }
        ...
        if "v4_prompt" in data:
            ...
            for char in caption.get("char_captions", []):
                result["character_prompts"].append({"prompt": char.get("char_caption", ""), "positions": _centers(char)})
        ...
        if "v4_negative_prompt" in data:
            ...
            for char in caption.get("char_captions", []):
                result["negative_characters"].append({"prompt": char.get("char_caption", ""), "positions": _centers(char)})
